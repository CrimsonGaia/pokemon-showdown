...
async function runParallelWithLimit(items: string[], max: number, fn: (item: string) => Promise<void>) {
	...
	for (const item of items) {
		...
		if (max <= runningPromises.size) { await Promise.race(runningPromises.values()); }
	}
...
export const Repl = new class {
	...
	setupListeners(filename: string) {
		...
		process.once('exit', code => {
			for (const s of Repl.socketPathnames) { try { fs.unlinkSync(s); } catch {} }
			if (code === 129 || code === 130) { process.exitCode = 0; }
		});
		if (!process.listeners('SIGHUP').length) { process.once('SIGHUP', () => process.exit(128 + 1)); }
		if (!process.listeners('SIGINT').length) { process.once('SIGINT', () => process.exit(128 + 2)); }
		(global as any).heapdump = (targetPath?: string) => {
			if (!targetPath) targetPath = `${filename}-${new Date().toISOString()}`;
			let handler;
			try { handler = require('node-oom-heapdump')(); } 
			catch (e: any) {
	...
	// Delete old sockets in the REPL directory (presumably from a crashed previous launch of PS).
	async cleanup() {
		const config = typeof Config !== 'undefined' ? Config : {};
		if (!config.repl) return;
		// Clean up old REPL sockets.
		const directory = path.dirname(path.resolve(FS.ROOT_PATH, config.replsocketprefix || 'logs/repl', 'app'));
		...
		await runParallelWithLimit(files, MAX_CONCURRENT_CLEANUP_SOCKETS, async (file: string) => {
			...
			await new Promise((resolve, reject) => {
				const socket = net.connect(pathname, () => {
					...
					socket.destroy();
					resolve(null);
				}).on('error', () => { resolve(fs.promises.unlink(pathname).catch(err => null)); });
			});
		});
	}
	/**
	 * Starts a REPL server, using a UNIX socket for IPC. The eval function
	 * parameter is passed in because there is no other way to access a file's non-global context.
	 */
	start(filename: string, evalFunction: EvalType) {
		...
		// TODO: Windows does support the REPL when using named pipes. For now, this only supports UNIX sockets.
		...
		const server = net.createServer(socket => {
			repl.start({
				...
				eval(cmd, context, unusedFilename, callback) {
					try { return callback(null, evalFunction(cmd)); } 
					catch (e: any) { return callback(e, undefined); }
				},
			}).on('exit', () => socket.end());
		...
		try {
			...
			server.once('error', (err: NodeJS.ErrnoException) => {
				server.close();
				if (err.code === "EADDRINUSE") { fs.unlink(pathname, _err => { if (_err && _err.code !== "ENOENT") { crashlogger(_err, `REPL: ${filename}`); } }); } 
				else if (err.code === "EACCES") { if (process.platform !== 'win32') { console.error(`Could not start REPL server "${filename}": Your filesystem doesn't support Unix sockets (everything else will still work)`); } } 
				else { crashlogger(err, `REPL: ${filename}`); }
			});
			server.once('close', () => { Repl.socketPathnames.delete(pathname); });
		} catch (err) { console.error(`Could not start REPL server "${filename}": ${err}`); }
	}
