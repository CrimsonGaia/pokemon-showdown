...
export class StaticServer {
	...
	constructor(root?: Options | string | null, options?: Options) {
		...
		if (this.options.cacheTime !== undefined) { this.cacheTime = this.options.cacheTime; }
		if (this.options.defaultExtension) { this.defaultExtension = `.${this.options.defaultExtension}`; }
		if (this.options.serverInfo !== null) { this.defaultHeaders['server'] = this.options.serverInfo || SERVER_INFO; }
		for (const k in this.options.headers) { this.defaultHeaders[k] = this.options.headers[k]; }
	}
	async serveDir(pathname: string, req: http.IncomingMessage, res: http.ServerResponse): Promise<Result> {
		...
		try {
			...
			if (originalPathname.length && !originalPathname.endsWith('/')) { return this.getResult(301, { 'Location': originalPathname + '/' }); } 
			else { return this.respond(status, headers, htmlIndex, stat, req, res); }
		} catch { return this.getResult(404); }
	}
	async serveFile(
	...
	getResult(status: number, headers: Headers = {}, alreadySent = false): Result {
		if (this.defaultHeaders['server']) { headers['server'] ||= this.defaultHeaders['server']; }
		return {
			status,
			headers,
	...
	finish(result: Result, req: http.IncomingMessage, res: http.ServerResponse, errorCallback?: ErrorCallback): Result {
		// If `alreadySent`, it's been taken care of in `this.stream`.
		if (!result.alreadySent && !errorCallback?.(result)) {
			res.writeHead(result.status, result.headers);
			if (result.status >= 400 && req.method !== 'HEAD') { res.write(`${result.status} ${result.message}`); }
			res.end();
		}
		return result;
	}
	async servePath(pathname: string, status: number, headers: Headers, req: http.IncomingMessage, res: http.ServerResponse): Promise<Result> {
		pathname = this.resolve(pathname);
		// Make sure we're not trying to access a file outside of the root.
		if (!pathname.startsWith(this.root)) { return this.getResult(403); } // Forbidden
		try {
			const stat = await fsP.stat(pathname);
			if (stat.isFile()) { return this.respond(status, headers, pathname, stat, req, res); } // Stream a single file.
			else if (stat.isDirectory()) { return this.serveDir(pathname, req, res); } 
			else { return this.getResult(400); }
		} catch { // possibly not found, check default extension
			if (this.defaultExtension) {
				try {
					const stat = await fsP.stat(pathname + this.defaultExtension);
					if (stat.isFile()) { return this.respond(status, headers, pathname + this.defaultExtension, stat, req, res); } 
					else { return this.getResult(400); }
				} catch { return this.getResult(404); } // really not found
			} else { return this.getResult(404); }
		}
	}
	resolve(pathname: string) { return path.resolve(path.join(this.root, pathname)); }
	async serve(req: http.IncomingMessage, res: http.ServerResponse, errorCallback?: ErrorCallback): Promise<Result> {
		let pathname;
		try { pathname = decodeURIComponent(new URL(req.url!, 'http://localhost').pathname); } 
		catch { return this.finish(this.getResult(400), req, res, errorCallback); }
		const result = await this.servePath(pathname, 200, {}, req, res);
		return this.finish(result, req, res, errorCallback);
	}
	/** Check if we should consider sending a gzip version of the file based on the file content type and client's Accept-Encoding header value. */
	gzipOk(req: http.IncomingMessage, contentType: string) {
		const enable = this.options.gzip;
		if (enable === true || ((enable instanceof RegExp) && enable.test(contentType))) { return req.headers['accept-encoding']?.includes('gzip'); }
		return false;
	}
	/** Send a gzipped version of the file if the options and the client indicate gzip is enabled and we find a .gz file matching the static resource requested. */
	respondGzip(
		status: number, contentType: string, _headers: Headers, file: string, stat: fs.Stats,
		req: http.IncomingMessage, res: http.ServerResponse
	): Promise<Result> {
		if (!this.gzipOk(req, contentType)) { return this.respondNoGzip(status, contentType, _headers, file, stat, req, res); } // Client doesn't want gzip
	...
	parseByteRange(req: http.IncomingMessage, stat: fs.Stats) {
		...
		if (rangeHeader) {
			if (rangeHeader.startsWith(flavor) && !rangeHeader.includes(',')) {
				...
				} else if (!isNaN(byteRange.from) && isNaN(byteRange.to)) { byteRange.to = stat.size ? stat.size - 1 : 0; }
				/* General byte range validation */
				if (!isNaN(byteRange.from) && !isNaN(byteRange.to) && 0 <= byteRange.from && byteRange.from <= byteRange.to) { byteRange.valid = true; } 
				else { if (DEBUG) console.warn('Request contains invalid range header: ', splitRangeHeader); }
			} else { if (DEBUG) console.warn('Request contains unsupported range header: ', rangeHeader); }
		}
		return byteRange;
	}
	async respondNoGzip(
		...
		if (byteRange.valid) {
			if (byteRange.to < length) {
				...
				if (DEBUG) { console.warn('Range request exceeds file boundaries, goes until byte no', byteRange.to, 'against file size of', length, 'bytes'); }
			}
		}
		/* In any case, check for unhandled byte range headers */
		if (!byteRange.valid && req.headers['range']) { if (DEBUG) console.error(new Error('Range request present but invalid, might serve whole file instead')); }
		...
		// Conditional GET
		// If the "If-Modified-Since" or "If-None-Match" headers match the conditions, send a 304 Not Modified.
		if ((clientMTime || clientETag) &&
			(!clientETag || clientETag === headers['Etag']) &&
			(!clientMTime || clientMTime >= mtime)) {
			// 304 response should not contain entity headers
			for (const entityHeader of ['Content-Encoding', 'Content-Language', 'Content-Length', 'Content-Location', 'Content-MD5', 'Content-Range', 'Content-Type', 'Expires', 'Last-Modified',]) { delete headers[entityHeader]; }
			return this.getResult(304, headers);
		} else if (req.method === 'HEAD') { return this.getResult(status, headers); } 
		else {
			res.writeHead(status, headers);
			try {
				await this.stream(file, length, startByte, res);
				return this.getResult(status, headers, true);
			} catch { return this.getResult(500, {}, true); } 
		}
	}
	respond(
		...
		if (this.options.gzip) { return this.respondGzip(status, contentType, _headers, file, stat, req, res); } 
		else { return this.respondNoGzip(status, contentType, _headers, file, stat, req, res); }
	}
	stream(file: string, length: number, startByte: number, res: http.ServerResponse): Promise<number> {
		return new Promise<number>((resolve, reject) => {
			...
			fs.createReadStream(file, {
				...
				// Bounds check the incoming chunk and offset, as copying a buffer from an invalid offset will throw an error and crash
				if (chunk.length && offset < length && offset >= 0) { offset += chunk.length; }
			}).on('close', () => {
				res.end();
				resolve(offset);
			}).on('error', err => {
	...
	setCacheHeaders(_headers: Headers): Headers {
		if (typeof this.cacheTime === 'number') { _headers['cache-control'] = `max-age=${this.cacheTime}`; }
		return _headers;
	}
}