 ...
 * It'll always be a judgment call whether or not a function goes into a "catch-all" library like this, so here are some guidelines:
 * - It must not have any dependencies
 * - It must conceivably have a use in a wide variety of projects, not just Pokémon (if it's Pokémon-specific, Dex is probably a good place for it)
 * - A lot of Chat functions are kind of iffy, but I'm going to say for now that if it's English-specific, it should be left out of here.
 */
 ...
 * String(str), or str.toString(), Utils.getString is guaranteed not to crash.
 *
 ...
 * getString simply returns '' if the passed variable isn't a string or a number.
 */
export function getString(str: any): string { return (typeof str === 'string' || typeof str === 'number') ? `${str}` : ''; }
...
export function visualize(value: any, depth = 0): string {
	...
	if (typeof value === 'string') { return `"${value}"`; } // NOT ESCAPED
	...
	if (value.toString) {
		try {
			...
			if (typeof stringValue === 'string' && stringValue !== '[object Object]' && stringValue !== `[object ${constructor}]`) { return `${constructor}(${stringValue})`; }
		} catch {}
	}
	let buf = '';
...
export function compare(a: Comparable, b: Comparable): number {
	...
	if ('reverse' in a) { return compare((b as { reverse: string }).reverse, a.reverse); }
 ...
 * (numbers low to high, strings A-Z, booleans true-first, arrays in order).
 * Note that array.sort() only works on strings, not numbers, so you'll need this to sort numbers.
 */
 ...
 */
export function escapeHTMLForceWrap(text: string): string { return escapeHTML(forceWrap(text)).replace(/\u200B/g, '<wbr />'); }
/**
...
export function deepFreeze<T>(obj: T): T {
	...
	Object.freeze(obj);
	if (Array.isArray(obj)) { for (const elem of obj) deepFreeze(elem); } 
	else { for (const elem of Object.values(obj)) deepFreeze(elem); }
	return obj;
}
...
export function waitUntil(time: number): Promise<void> { return new Promise(resolve => { setTimeout(() => resolve(), time - Date.now()); }); }
...
export function bufWriteHex(buf: Uint8Array, hex: string, offset = 0) {
	...
	for (let i = 0; i < size; i++) { buf[offset + i] = parseInt(hex.slice(i * 2, i * 2 + 2).padEnd(2, '0'), 16); }
}
export function bufReadHex(buf: Uint8Array, start = 0, end?: number) { return [...buf.slice(start, end)].map(val => val.toString(16).padStart(2, '0')).join(''); }
export class Multiset<T> extends Map<T, number> {
	override get(key: T) { return super.get(key) ?? 0; }
	add(key: T) {
