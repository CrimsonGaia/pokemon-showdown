...
export interface PostData { [key: string]: string | number; }
...
export class NetStream extends Streams.ReadWriteStream {
	...
	makeRequest(opts: NetRequestOptions | null) {
		...
		if (opts.query) { this.uri += (this.uri.includes('?') ? '&' : '?') + NetStream.encodeQuery(opts.query); }
		if (body) {
			if (!opts.headers) opts.headers = {};
			if (!opts.headers['Content-Length']) { opts.headers['Content-Length'] = Buffer.byteLength(body); }
		}
		...
		this.response = new Promise(resolve => { resolveResponse = resolve; });
		const request = net.request(this.uri, opts, response => {
			...
			response.on('data', data => { this.push(data); });
			response.on('error', error => { if (!this.atEOF) this.pushError(error, true); });
			response.on('end', () => {
		...
		request.on('error', error => { if (!this.atEOF) this.pushError(error, true); });
		...
		if (body) {
			request.write(body);
			request.end();
			if (opts.writable) { throw new Error(`options.body is what you would have written to a NetStream - you must choose one or the other`); }
		} else if (opts.writable) { this.nodeWritableStream = request; } 
		else { request.end(); }
		return request;
	}
	...
	override _write(data: string | Buffer): Promise<void> | void {
		if (!this.nodeWritableStream) { throw new Error("You must specify opts.writable to write to a request."); }
	...
	override _read() { this.nodeReadableStream?.resume(); }
	override _pause() { this.nodeReadableStream?.pause(); }
}
export class NetRequest {
	...
	getStream(opts: NetRequestOptions = {}) {
		if (typeof Config !== 'undefined' && Config.noNetRequests) { throw new Error(`Net requests are disabled.`); }
	...
	async get(opts: NetRequestOptions = {}): Promise<string> {
		...
		if (response && response.statusCode !== 200) { throw new HttpError(response.statusMessage || "Connection error", response.statusCode, await stream.readAll()); }
	...
	post(opts: NetRequestOptions = {}, body?: PostData | string) {
		...
		return this.get({ ...opts, method: 'POST', body, });
	}
}
export const Net = Object.assign((path: string) => new NetRequest(path), { NetRequest, NetStream, });