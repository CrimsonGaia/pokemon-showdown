 ...
 * The Node.js standard library's Streams are really hard to use. This offers a better stream API.
...
export class ReadStream {
	...
	constructor(optionsOrStreamLike: { [k: string]: any } | NodeJS.ReadableStream | string | Buffer = {}) {
		...
		this.nextPush = new Promise(resolve => { this.nextPushResolver = resolve; });
		this.awaitingPush = false;
		let options: { [k: string]: any };
		if (typeof optionsOrStreamLike === 'string') { options = { buffer: optionsOrStreamLike }; } 
		else if (optionsOrStreamLike instanceof Buffer) { options = { buffer: optionsOrStreamLike }; } 
		else if (typeof (optionsOrStreamLike as any)._readableState === 'object') { options = { nodeStream: optionsOrStreamLike as NodeJS.ReadableStream }; } 
		else { options = optionsOrStreamLike; }
		if (options.nodeStream) {
			const nodeStream: NodeJS.ReadableStream = options.nodeStream;
			this.nodeReadableStream = nodeStream;
			nodeStream.on('data', data => { this.push(data); });
			nodeStream.on('end', () => { this.pushEnd(); });
			options.read = function (this: ReadStream, unusedBytes: number) { this.nodeReadableStream!.resume(); };
			options.pause = function (this: ReadStream, unusedBytes: number) { this.nodeReadableStream!.pause(); };
		}
	...
	get bufSize() { return this.bufEnd - this.bufStart; }
	moveBuf() {
		if (this.bufStart !== this.bufEnd) { this.buf.copy(this.buf, 0, this.bufStart, this.bufEnd); }
	...
	ensureCapacity(additionalCapacity: number) {
		...
		if (capacity <= this.bufCapacity) { return this.moveBuf(); }
	...
	peekError() { if (this.errorBuf) { throw this.errorBuf[0]; } }
	resolvePush() {
	...
	_read(size = 0): void | Promise<void> { throw new Error(`ReadStream needs to be subclassed and the _read function needs to be implemented.`); }
	_destroy(): void | Promise<void> {}
	_pause() {}
	/**
	 * Reads until the internal buffer is non-empty. Does nothing if the internal buffer is already non-empty.
	 * If `byteCount` is a number, instead read until the internal buffer contains at least `byteCount` bytes.
	...
	async read(byteCount: number | string | null = null, encoding: BufferEncoding = this.encoding) {
		...
		// if the buffer is written to after peek but before clearing the buffer, the write will be lost forever
		const out = this.peek(byteCount, encoding);
		if (out && typeof out !== 'string') { throw new Error("Race condition; you must not read before a previous read has completed"); }
	...
	async readBuffer(byteCount: number | null = null) {
		await this.loadIntoBuffer(byteCount, true);
		// This MUST NOT be awaited: we must synchronously clear the buffer after peeking (see `read`)
		const out = this.peekBuffer(byteCount);
		if (out && (out as Promise<unknown>).then) { throw new Error("Race condition; you must not read before a previous read has completed"); }
		if (byteCount === null || byteCount >= this.bufSize) {
			this.bufStart = 0;
			this.bufEnd = 0;
		} else { this.bufStart += byteCount; }
		return out;
	}
	...
	async readAll(encoding: BufferEncoding = this.encoding) { return (await this.read(Infinity, encoding)) || ''; }

	peekAll(encoding: BufferEncoding = this.encoding) { return this.peek(Infinity, encoding); }
	...
	async pipeTo(outStream: WriteStream, options: { noEnd?: boolean } = {}) {
		let next;
		while ((next = await this.next(), !next.done)) { await outStream.write(next.value); }
...
export class WriteStream {
	...
	constructor(optionsOrStream: WriteStreamOptions | NodeJS.WritableStream = {}) {
		...
		if ((options as any)._writableState) { options = { nodeStream: optionsOrStream as NodeJS.WritableStream }; }
		if (options.nodeStream) {
			...
			if (nodeStream !== process.stdout && nodeStream !== process.stderr) { options.writeEnd = function () { return new Promise<void>(resolve => { this.nodeWritableStream!.end(() => resolve()); }); }; }
		}
		if (options.write) this._write = options.write;
		if (options.writeEnd) this._writeEnd = options.writeEnd;
	}
	write(chunk: Buffer | string): void | Promise<void> { return this._write(chunk); }
	writeLine(chunk: string): void | Promise<void> {
		if (chunk === null) { return this.writeEnd(); }
		return this.write(chunk + '\n');
	}
	_write(chunk: Buffer | string): void | Promise<void> { throw new Error(`WriteStream needs to be subclassed and the _write function needs to be implemented.`); }
...
export class ReadWriteStream extends ReadStream implements WriteStream {
	...
	write(chunk: Buffer | string): Promise<void> | void { return this._write(chunk); }
	writeLine(chunk: string): Promise<void> | void { return this.write(chunk + '\n'); }
	_write(chunk: Buffer | string): Promise<void> | void { throw new Error(`WriteStream needs to be subclassed and the _write function needs to be implemented.`); }
	// In a ReadWriteStream, `_read` does not need to be implemented, because it's valid for the read stream buffer to be filled only by `_write`.
	override _read(size?: number) {}
	_writeEnd(): void | Promise<void> {}
	async writeEnd() { return this._writeEnd(); }
}
...
export class ObjectReadStream<T> {
	...
	constructor(optionsOrStreamLike: ObjectReadStreamOptions<T> | NodeJS.ReadableStream | T[] = {}) {
		...
		if (Array.isArray(optionsOrStreamLike)) { options = { buffer: optionsOrStreamLike }; } 
		else if (typeof (optionsOrStreamLike as any)._readableState === 'object') { options = { nodeStream: optionsOrStreamLike as NodeJS.ReadableStream }; } 
		else { options = optionsOrStreamLike as ObjectReadStreamOptions<T>; }
		if ((options as any).nodeStream) {
			...
			nodeStream.on('end', () => { this.pushEnd(); });
			options = {
				read() { this.nodeReadableStream!.resume(); },
				pause() { this.nodeReadableStream!.pause(); },
			};
		}
		if (options.read) this._read = options.read;
		if (options.pause) this._pause = options.pause;
	...
	async loadIntoBuffer(count: number | true = 1, readError?: boolean) {
		...
		while (!this.errorBuf && !this.atEOF && this.buf.length < this.readSize) {
			...
			if (readResult) { await readResult; } 
...
export class ObjectWriteStream<T> {
	...
	constructor(optionsOrStream: ObjectWriteStreamOptions<T> | NodeJS.WritableStream = {}) {
		...
		if (options._writableState) {  options = { nodeStream: optionsOrStream as NodeJS.WritableStream }; }
		if (options.nodeStream) {
			const nodeStream: NodeJS.WritableStream = options.nodeStream;
			this.nodeWritableStream = nodeStream;
			options.write = function (data: T) {
				const result = this.nodeWritableStream!.write(data as unknown as string);
				if (result === false) { return new Promise<void>(resolve => { this.nodeWritableStream!.once('drain', () => { resolve(); }); }); }
			};
			// Prior to Node v10.12.0, attempting to close STDOUT or STDERR will throw
			if (nodeStream !== process.stdout && nodeStream !== process.stderr) { options.writeEnd = function () { return new Promise<void>(resolve => { this.nodeWritableStream!.end(() => resolve()); }); }; }
		}
	...
	_write(elem: T): void | Promise<void> { throw new Error(`WriteStream needs to be subclassed and the _write function needs to be implemented.`); }
	_writeEnd(): void | Promise<void> {}
	async writeEnd(elem?: T): Promise<void> {
		if (elem !== undefined) { await this.write(elem); }
		return this._writeEnd();
	}
}
...
export class ObjectReadWriteStream<T> extends ObjectReadStream<T> implements ObjectWriteStream<T> {
	...
	write(elem: T): void | Promise<void> { return this._write(elem); }
	_write(elem: T): void | Promise<void> { throw new Error(`WriteStream needs to be subclassed and the _write function needs to be implemented.`); }
...
export function readAll(nodeStream: NodeJS.ReadableStream, encoding?: any) { return new ReadStream(nodeStream).readAll(encoding); }
export function stdin() { return new ReadStream(process.stdin); }
export function stdout() { return new WriteStream(process.stdout); }
export function stdpipe(stream: WriteStream | ReadStream | ReadWriteStream) {
	const promises = [];
	if ((stream as ReadStream | WriteStream & { pipeTo: undefined }).pipeTo) { promises.push((stream as ReadStream).pipeTo(stdout())); }
	if ((stream as WriteStream | ReadStream & { write: undefined }).write) { promises.push(stdin().pipeTo(stream as WriteStream)); }
