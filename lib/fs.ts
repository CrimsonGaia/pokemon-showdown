...
if (!global.__fsState) { global.__fsState = { pendingUpdates: new Map(), }; }
export class FSPath {
	path: string;
	constructor(path: string) { this.path = pathModule.resolve(ROOT_PATH, path); }
	parentDir() { return new FSPath(pathModule.dirname(this.path)); }
	read(options: AnyObject | BufferEncoding = 'utf8'): Promise<string> {
		if (typeof options !== 'string' && options.encoding === undefined) { options.encoding = 'utf8'; }
		return new Promise((resolve, reject) => { fs.readFile(this.path, options, (err, data) => { err ? reject(err) : resolve(data as string); }); });
	}
	readSync(options: AnyObject | string = 'utf8'): string {
		if (typeof options !== 'string' && options.encoding === undefined) { options.encoding = 'utf8'; }
		return fs.readFileSync(this.path, options as { encoding: 'utf8' });
	}
	readBuffer(options: AnyObject | BufferEncoding = {}): Promise<Buffer> { return new Promise((resolve, reject) => { fs.readFile(this.path, options, (err, data) => { err ? reject(err) : resolve(data as Buffer); }); }); }
	readBufferSync(options: AnyObject | string = {}) { return fs.readFileSync(this.path, options as { encoding: null }); }
	exists(): Promise<boolean> { return new Promise(resolve => { fs.exists(this.path, exists => { resolve(exists); }); }); }
	existsSync() { return fs.existsSync(this.path); }
	...
	readIfExistsSync() {
		try { return fs.readFileSync(this.path, 'utf8'); } 
	 ...
	 * Does not protect against simultaneous writing; use writeUpdate for that.
	 */
	...
	checkNextUpdate() {
		...
		if (!dataFetcher || !options) { // no pending update
	...
	append(data: string | Buffer, options: AnyObject = {}) {
		if (global.Config?.nofswriting) return Promise.resolve();
		return new Promise<void>((resolve, reject) => { fs.appendFile(this.path, data, options, err => { err ? reject(err) : resolve(); }); });
	}
	...
	symlinkTo(target: string) {
		if (global.Config?.nofswriting) return Promise.resolve();
		return new Promise<void>((resolve, reject) => { fs.symlink(target, this.path, err => { err ? reject(err) : resolve(); }); });
	}
	...
	copyFile(dest: string) {
		if (global.Config?.nofswriting) return Promise.resolve();
		return new Promise<void>((resolve, reject) => { fs.copyFile(this.path, dest, err => { err ? reject(err) : resolve(); }); });
	}
	rename(target: string) {
		if (global.Config?.nofswriting) return Promise.resolve();
		return new Promise<void>((resolve, reject) => { fs.rename(this.path, target, err => { err ? reject(err) : resolve(); }); });
	}
	renameSync(target: string) {
		if (global.Config?.nofswriting) return;
		return fs.renameSync(this.path, target);
	}
	readdir(): Promise<string[]> { return new Promise((resolve, reject) => { fs.readdir(this.path, (err, data) => { err ? reject(err) : resolve(data); }); }); }

	readdirSync() { return fs.readdirSync(this.path); }
	...
	createReadStream() { return new FileReadStream(this.path); }
	createWriteStream(options = {}): WriteStream {
		if (global.Config?.nofswriting) { return new WriteStream({ write() {} }); }
	...
	async rmdir(recursive?: boolean) {
		if (global.Config?.nofswriting) return Promise.resolve();
		return new Promise<void>((resolve, reject) => { fs.rmdir(this.path, { recursive }, err => { err ? reject(err) : resolve(); }); });
	}
	...
	mkdir(mode: string | number = 0o755) {
		if (global.Config?.nofswriting) return Promise.resolve();
		return new Promise<void>((resolve, reject) => { fs.mkdir(this.path, mode, err => { err ? reject(err) : resolve(); }); });
	}
	...
	mkdirIfNonexistentSync(mode: string | number = 0o755) {
		if (global.Config?.nofswriting) return;
		try { fs.mkdirSync(this.path, mode); } 
	...
	async mkdirp(mode: string | number = 0o755) {
		try { await this.mkdirIfNonexistent(mode); } 
		catch (err: any) {
	...
	mkdirpSync(mode: string | number = 0o755) {
		try { this.mkdirIfNonexistentSync(mode); } 
		catch (err: any) {
			if (err.code !== 'ENOENT') throw err;
	...
	onModify(callback: () => void) { fs.watchFile(this.path, (curr, prev) => { if (curr.mtime > prev.mtime) return callback(); }); }
	/** Clears callbacks added with onModify(). */
	unwatch() { fs.unwatchFile(this.path); }
	async isFile() { return new Promise<boolean>((resolve, reject) => { fs.stat(this.path, (err, stats) => { err ? reject(err) : resolve(stats.isFile()); }); }); }
	isFileSync() { return fs.statSync(this.path).isFile(); }
	async isDirectory() { return new Promise<boolean>((resolve, reject) => { fs.stat(this.path, (err, stats) => { err ? reject(err) : resolve(stats.isDirectory()); }); }); }
	isDirectorySync() { return fs.statSync(this.path).isDirectory(); }
	async realpath() { return new Promise<string>((resolve, reject) => { fs.realpath(this.path, (err, path) => { err ? reject(err) : resolve(path); }); }); }
	realpathSync() { return fs.realpathSync(this.path); }
}
class FileReadStream extends ReadStream {
	fd: Promise<number>;
	constructor(file: string) {
		super();
		this.fd = new Promise((resolve, reject) => { fs.open(file, 'r', (err, fd) => err ? reject(err) : resolve(fd)); });
		this.atEOF = false;
	}
	...
	override _destroy() { return new Promise<void>(resolve => { void this.fd.then(fd => { fs.close(fd, () => resolve()); }); }); }
}
function getFs(path: string) { return new FSPath(path); }
export const FS = Object.assign(getFs, { FileReadStream, FSPath, ROOT_PATH, });