...
const logPath = path.resolve( // not sure why this is necessary, but in Windows testing it was
...
function appendCause(error: any) {
	let stack = ``;
	if (typeof error.cause === 'string') { stack += `\n\n[cause]: ${error.cause}\n`; } 
	else {
...
// Logs when a crash happens to console, then e-mails those who are configured to receive them.
export function crashlogger(
	...
	if ((error as any)?.cause) { stack += appendCause(error as Error); }
	if (data) {
		stack += `\n\nAdditional information:\n`;
		for (const k in data) { stack += `  ${k} = ${data[k]}\n`; }
	}
	console.error(`\nCRASH: ${stack}\n`);
	...
	out.on('open', () => {
		out.write(`\n${stack}\n`);
		out.end();
	}).on('error', (err: Error) => { console.error(`\nSUBCRASH: ${err.stack}\n`); });
	...
	if (emailOpts && ((datenow - lastCrashLog) > CRASH_EMAIL_THROTTLE)) {
		...
		if (transport) { text += `again with this stack trace:\n${stack}`; } 
		else {
			try { transport = require('nodemailer').createTransport(emailOpts.options); } 
			catch { throw new Error("Failed to start nodemailer; are you sure you've configured Config.crashguardemail correctly?"); }
		...
		transport.sendMail({
			...
			text,
		}, (err: Error | null) => { if (err) console.error(`Error sending email: ${err}`); });
	}
	return null;
}