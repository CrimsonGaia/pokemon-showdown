...
console.log('[loginserver] CONFIG CHECK', {
  port: Config.port,
...
export class ActionContext {
	...
	isTrustedProxy(ip: string) {
		...
		return (
			...
			Config.trustedproxies.some((f: string) => IPTools.checkPattern(f, ip)) ||
...
if (require.main === module) {
	// Start the loginserver HTTP listener
	// Uses Config.port / Config.bindaddress from your config.js
	// eslint-disable-next-line no-new
	new Server();
	console.log(`[loginserver] listening on ${Config.bindaddress || '0.0.0.0'}:${Config.port || 8000}`);
}