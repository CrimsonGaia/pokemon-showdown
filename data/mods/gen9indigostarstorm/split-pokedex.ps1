# Split pokedex.ts into smaller files by generation pairs so it can actually load in time
$content = Get-Content "pokedex.ts" -Raw
$lines = Get-Content "pokedex.ts"
$regions = @()
for ($i = 0; $i -lt $lines.Count; $i++) { if ($lines[$i] -match '//region Gen (\d+)') { $regions += @{ Gen = [int]$Matches[1]; Line = $i }  } }
...
foreach ($split in $splits) {
    ...
    for ($i = 1; $i -lt $lines.Count; $i++) {
        ...
        elseif ($line -match '//endregion') {
            if ($inRegion) { $output += $line }
            $inRegion = $false
        }
        elseif ($inRegion) { $output += $line }
        elseif ($line -match '^\s*}\s*;\s*$') { $output += $line }
    }
    if ($output[-1] -notmatch '^\s*}\s*;\s*$') { $output += "};" }
