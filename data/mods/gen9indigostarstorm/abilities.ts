...
export const Abilities: import('../../sim/dex-abilities').AbilityDataTable = {
	...
	aerodynamic: { // airborneness implemented in sim/pokemon.js:Pokemon#isGrounded
		...
		onTryHit(damage, target, source, move) { if (move.flags?.wind) { this.boost({spe: 1}, target, target, this.effect); } },
		flags: {breakable: 1},
		name: "Aerodynamic",
	...
	blazingvortex: {
		onStart(source) {
			...
			this.field.addPseudoWeather('seaoffire', source, source.getAbility(slot as 1 | 2), 4);
		},
		flags: {},
		name: "Blazing Vortex",
	...
	fluxconduit: {
		...
		onRedirectTarget(target, source, source2, move) { 
			...
			if (!pokemon.knownType || pokemon.hasType('Steel' && target !== this.effectState.target && this.effectState.target.isAdjacent(target))) { return this.effectState.target; }
	...
	gravitywell: {
		onStart(source) {
			...
			this.field.addPseudoWeather('gravity', source, source.getAbility(slot as 1 | 2), 5);
		},
		flags: {},
		name: "Gravity Well",
	...
	karmicreversal: {
		onStart(source) {
			const slot = source.ability1 === 'karmicreversal' ? 1 : 2;
			this.field.setRoom('inverseroom', source, source.getAbility(slot as 1 | 2));
		},
		flags: {},
	...
	longsnout: {
		...
		onModifyMove(move, pokemon) { 
			if (move.flags?.sweep && move.target === 'normal') {
	...
	mossarmor: {
		onDamagingHit(damage, target, source, move) { 
			if (!target.mossArmorBroken) { 
				if (move.crit || move.type === 'Bug' || move.type === 'Fire' || move.type === 'Poison' || move.flags?.slice) { // Break armor if hit by critical hit or a Bug/Fire/Poison or Slice move
	...
	mudarmor: {
		onDamagingHit(damage, target, source, move) { 
			if (!target.mudArmorBroken) { 
				if (move.crit || move.type === 'Grass' || move.type === 'Ice' || move.type === 'Water') { // Break armor if hit by critical hit or Grass/Ice/Water move
		...
		onModifyMove(move, pokemon) { 
			if (move.flags?.pierce && this.randomChance(1, 5)) { 
				...
				if (this.checkMoveMakesContact(move, source, target, !source.isAlly(target))) {
					...
					source.setAbility('neurotoxin', target, null, false, false, 1);
					if (oldAbility && oldAbility !== 'neurotoxin') { this.add('-activate', target, 'ability: Neurotoxin', this.dex.abilities.get(oldAbility).name, `[of] ${source}`); }
				}
			} 
		},
	...
	rainbowwings: {
		onStart(source) {
			...
			this.field.addPseudoWeather('rainbow', source, source.getAbility(slot as 1 | 2), 4);
		},
	...
	seedgift: {
		...
		onStart(pokemon) {
			const abilityState =
				pokemon.ability1 === 'seedgift' ? pokemon.abilityState1 :
				pokemon.ability2 === 'seedgift' ? pokemon.abilityState2 :
	...
	soothingvoice: {
		onSourceDamagingHit(damage, target, source, move) {
			...
			if (this.move.flags.sound(move, target, source)) { 
		...
		flags: {},
		name: "Soothing Voice",
		shortDesc: "This Pokemon's Sound moves become Poison type and have 1.2x power, and a 20% chance to Poison the target. If poisoned by this effect, also lower their Special Attack -1 stage.",
		rating: 1.5,
		num: 1099,
	},
	souleater: {
	...
	spellhorizon: {
		onStart(source) {
			const slot = source.ability1 === 'spellhorizon' ? 1 : 2;
			this.field.setRoom('magicroom', source, source.getAbility(slot as 1 | 2));
		},
		flags: {},
		name: "Spell Horizon",
	...
	starterdough: {
		onStart(pokemon) {
			const abilityState =
				pokemon.ability1 === 'starterdough' ? pokemon.abilityState1 :
				pokemon.ability2 === 'starterdough' ? pokemon.abilityState2 :
				pokemon.abilityState;
		...
		onTryHit(target, source, move) {
			if (target !== source && move.type === 'Fire') {
				const abilityState =
					target.ability1 === 'starterdough' ? target.abilityState1 :
					target.ability2 === 'starterdough' ? target.abilityState2 :
					target.abilityState;
	...
	surgingmigraine: {
		onStart(pokemon) {
			const slot = pokemon.ability1 === 'surgingmigraine' ? 1 : 2;
			this.field.setRoom('wonderroom', pokemon, pokemon.getAbility(slot as 1 | 2));
	...
	teraformblade: {
		onModifyMove(move) { if (move.flags?.slice) { move.infiltrates = true; } },
	...
	volvation: {
		...
		onSourceModifyAtk(atk, attacker, defender, move) { 
			if (move.flags && (move.flags.bullet || move.flags.bite || move.flags.bomb || move.flags.claw || move.flags.explosive || move.flags.kick || move.flags.pierce || move.flags.punch || move.flags.slice)) { 
	...
	webarmor: {
		...
		onDamagingHit(damage, target, source, move) { 
			if (!target.webArmorBroken && (move.type === 'Fire' || move.flags?.slice)) {
	...
	esperwing: {
		...
		onStart(pokemon) {
			const terrain = this.field.isTerrain();
	...
	indomitablespirit: {
		onBasePower(basePower, attacker, defender, move) { 
			if (move.flags?.aura || move.flags?.dance || move.flags?.heal || move.flags?.kick || move.flags?.punch || move.flags?.slice || move.flags?.sweep || move.flags?.throwing) { 
	...
	cloudnine: {
		...
		onStart(pokemon) {
			const abilityState =
				pokemon.ability1 === 'cloudnine' ? pokemon.abilityState1 :
				pokemon.ability2 === 'cloudnine' ? pokemon.abilityState2 :
				pokemon.abilityState;
	...
	airlock: {
		...
		onStart(pokemon) {
			const abilityState =
				pokemon.ability1 === 'airlock' ? pokemon.abilityState1 :
				pokemon.ability2 === 'airlock' ? pokemon.abilityState2 :
				pokemon.abilityState;
			abilityState.ending = false;
			this.eachEvent('WeatherChange', this.effect);
		},
	...
	anticipation: {
		onStart(pokemon) { for (const target of pokemon.foes()) { for (const moveSlot of target.moveSlots) {
					...
					if (this.dex.getEffectiveness(moveType, pokemon) > 0) { 
		...
		onModifyMove(move, attacker, defender) {
			if (!defender || !defender.hasAbility('anticipation')) return;
	...
	gluttony: {
		onStart(pokemon) {
			const abilityState =
				pokemon.ability1 === 'gluttony' ? pokemon.abilityState1 :
				pokemon.ability2 === 'gluttony' ? pokemon.abilityState2 :
				pokemon.abilityState;
	...
	motordrive: {
		...
		onAddVolatile(status, pokemon) { 
			if (status.id === 'charge' && !pokemon.volatiles['motordrivechargeboosted']) {
				this.boost({ spe: 1 }, pokemon, pokemon, null, true);
				pokemon.volatiles['motordrivechargeboosted'] = true;
			}
		},
	...
	illusion: { // yes, you can Illusion an active pokemon but only if it's to your right
		...
		onDamagingHit(damage, target, source, move) {
			if (target.illusion) {
				const illusionState =
					target.ability1 === 'illusion' ? target.abilityState1 :
					target.ability2 === 'illusion' ? target.abilityState2 :
					target.abilityState;
	...
	telepathy: {
		...
		onModifyAccuracy(accuracy, target, source, move) {
			...
			if (telepathyUser && telepathyUser.moveThisTurnResult === false) { 
				this.add('-activate', target, 'ability: Telepathy');
				return false; 
			}
		},
	...
	furcoat: {
		...
		onSourceModifyDamage(damage, source, target, move) { 
			if (move.flags?.claw || move.flags?.slice) { 
	...
	fullmetalbody: {
		...
		onSourceModifyDamage(damage, source, target, move) { 
			if ( (move.flags?.bullet || move.flags?.kick || move.flags?.pierce || move.flags?.punch || move.flags?.slice) ) { 
	...
	receiver: {
		onBasePowerPriority: 8,
		onBasePower(basePower, attacker, defender, move) {
			if (move.flags && move.flags.throwing) { 
	...
	surgesurfer: {
		onModifySpe(spe, pokemon) { if (this.field.isTerrain()) { return this.chainModify(2); } },
		onModifyMove(move, pokemon) { 
			if (move.id === 'surf')  {
	...
	neutralizinggas: { // Ability suppression implemented in sim/pokemon.ts:Pokemon#ignoringAbility
		...
		onSwitchIn(pokemon) {
			...
			const ngState =
				pokemon.ability1 === 'neutralizinggas' ? pokemon.abilityState1 :
				pokemon.ability2 === 'neutralizinggas' ? pokemon.abilityState2 :
				pokemon.abilityState;
		...
		onEnd(source) {
			...
			const ngState =
				source.ability1 === 'neutralizinggas' ? source.abilityState1 :
				source.ability2 === 'neutralizinggas' ? source.abilityState2 :
				source.abilityState;
	...
	wellbakedbody: {
		onTryHit(target, source, move) { if (target !== source && move.type === 'Fire') { 
			if (!this.boost({ def: 2 })) { 
