export const Abilities: import('../sim/dex-abilities').AbilityDataTable = {
	...
	adrenelinerush: {
		onDamagingHit(damage, target, source, move) {
			if (move.basePower >= 120) { this.boost({atk: 3, spe: 3}, target, target, this.effect); }
			else if (move.basePower >= 90) { this.boost({atk: 2, spe: 2}, target, target, this.effect); }
			else if (move.basePower >= 75) { this.boost({atk: 1, spe: 1}, target, target, this.effect); }
		},	
		flags: {},
		name: "Adreneline Rush",
		shortDesc: "When hit by a damaging move with 75+ base power: +1 Attack, +1 Speed. 90+ base power: +2 Attack, +2 Speed. 120+ base power: +3 Attack, +3 Speed.",
		rating: 3,
		num: 1031,
	},
	aerodynamic: { // airborneness implemented in sim/pokemon.js:Pokemon#isGrounded
		...
		onTryHit(target, source, move) { if (move.flags?.wind) { this.boost({spe: 1}, target, target, this.effect); } },
		flags: { breakable: 1 },
		name: "Aerodynamic",
		shortDesc: "User becomes airborne and immune to the negative effects of Delta Stream, Turbulent Winds. Gain Speed +1 when hit by Wind effects.",
		rating: 2.5,
		num: 1001,
	},
	amplifier: {
		onBasePower(basePower, attacker, defender, move) { if (move.flags?.sound) return this.chainModify(1.3); },
		onTryHitPriority: 1,
	...
	astralaspect: {
		...
		onModifyAtk(atk, pokemon) { if ((this.field.isWeather('sunnyday') || this.field.isWeather('desolateland') || this.field.isWeather('eclipse')) && pokemon.hasAbility('astralaspect')) { return this.chainModify(1.15); } },
		onModifyDef(def, pokemon) { if ((this.field.isWeather('sunnyday') || this.field.isWeather('desolateland') || this.field.isWeather('eclipse')) && pokemon.hasAbility('astralaspect')) { return this.chainModify(1.15); } },
		onModifySpA(spa, pokemon) { if ((this.field.isWeather('sunnyday') || this.field.isWeather('desolateland') || this.field.isWeather('eclipse')) && pokemon.hasAbility('astralaspect')) { return this.chainModify(1.15); } },
		onModifySpD(spd, pokemon) { if ((this.field.isWeather('sunnyday') || this.field.isWeather('desolateland') || this.field.isWeather('eclipse')) && pokemon.hasAbility('astralaspect')) { return this.chainModify(1.15); } },
		onModifySpe(spe, pokemon) { if ((this.field.isWeather('sunnyday') || this.field.isWeather('desolateland') || this.field.isWeather('eclipse')) && pokemon.hasAbility('astralaspect')) { return this.chainModify(1.15); } },
		onResidual(pokemon) { if (pokemon.hasAbility('astralaspect') && (this.field.isWeather('sunnyday') || this.field.isWeather('desolateland') || this.field.isWeather('eclipse'))) { this.heal(pokemon.baseMaxhp / 16, pokemon, pokemon); } },
		onTryHitPriority: 1,
		onTryHit(target, source, move) {
			if ((move.flags?.aura || move.flags?.lunar || move.flags?.solar) && target !== source) {
				this.add('-immune', target, '[from] ability: Astral Aspect');
				const oldhp = target.hp;
				target.hp = Math.min(target.maxhp, target.hp + Math.floor(target.baseMaxhp / 4));
				if (target.hp > oldhp) { this.add('-heal', target, target.getHealth); }
				return null;
			}
		},
		flags: { breakable: 1 },
	...
	balloonphysics: {
		...
		onTryHit(target, source, move) {
			if ((move.flags?.launch || move.flags?.sweep) && target !== source) {
				...
				target.addVolatile('balloonphysics');
				return null;
			}
		},
		onAfterMove(source, target, move) { if (move.flags?.airborne) { source.addVolatile('balloonphysics'); } },
		condition: {
			duration: 2,
	...
	blazingvortex: {
		onStart(source) {
			...
			const slot = source.ability1 === 'blazingvortex' ? 1 : 2;
			this.field.addPseudoWeather('seaoffire', source, source.getAbility(slot as 1 | 2));
		},
		flags: {},
		name: "Blazing Vortex",
	...
	dreameater: {
		...
		flags: {},
		name: "Dream Eater",
		shortDesc: "At end of turn, damages sleeping foes for 1/12 max HP and heals this Pokemon by the same amount.",
		rating: 2.5,
		num: 1016,
	},
	eclipse: {
		onStart(source) { this.field.setWeather('eclipse'); },
		onAnySetWeather(target, source, weather) {
			const strongWeathers = ['desolateland', 'primordialsea', 'deltastream', 'eclipse'];
			if (this.field.getWeather().id === 'eclipse' && !strongWeathers.includes(weather.id)) return false;
		},
		onEnd(pokemon) {
		...
		flags: {},
		name: "Eclipse",
		shortDesc: "Sets Eclipse while user is on the field. Eclipse overpowers all weathers and primal weathers, replaces Delta Stream.",
		rating: 4,
		num: 1090,
	},
	eelevate: { // airborneness implemented in sim/pokemon.js:Pokemon#isGrounded
		...
		flags: { breakable: 1 },
		name: "Eelevate",
		shortDesc: "User becomes airborne. When user kills a pokemon with a damaging move: +1 to its highest stat.",
		rating: 4.5,
		num: 1107,
	},
	elementalaffinity: {
	...
	enchantingvoice: {
		onModifyTypePriority: -1,
		onModifyType(move, pokemon) { if (move.flags?.sound) { move.type = 'Fairy'; } },
		onBasePower(basePower, attacker, defender, move) { if (move.flags?.sound) { return this.chainModify(1.2); } },
		flags: {},
	...
	feareater: {
		onUpdate(pokemon) {
			if (pokemon.status === 'fear') {
				this.add('-activate', pokemon, 'ability: Fear Eater');
				pokemon.cureStatus();
			}
		},
		onSetStatus(status, target, source, effect) {
			if (status.id !== 'fear') return;
			if ((effect as Move)?.status) { this.add('-immune', target, '[from] ability: Fear Eater'); }
			return false;
		},
		onResidualOrder: 28,
		...
		flags: { breakable: 1 },
		name: "Fear Eater",
		shortDesc: "Immune to Fear. At end of turn, drain 1/16HP from all pokemon on the field that are Afraid",
		rating: 3,
		num: 1104,
	},
	firemane: {
		onStart(pokemon) {
			pokemon.setStatus('aura', pokemon, {
				auraAbility: 'kinglyauthority',
				auraName: 'Kingly Authority',
				auraDuration: 3,
			} as any);
		},
		...
		flags: {},
		name: "Fire Mane",
		shortDesc: "Boosts power of user's Fire type moves by 1.5x",
		rating: 3.5,
		num: 316,
	},
	flameemperor: {
		...
		onDamage(damage, target, source, effect) { 
			if (effect && (effect.id === 'recoil' || effect.id === 'crash')) { 
				this.debug('Reckless halves recoil/crash damage');
				return Math.floor(damage / 2);
			}
		},
		onBasePowerPriority: 23,
		onBasePower(basePower, attacker, defender, move) { 
			if (move.recoil || move.hasCrashDamage || move.flags?.pierce) { 
				this.debug('flameemp boost');
		...
		flags: { breakable: 1 },
		name: "Flame Emperor",
		shortDesc: "User's Contact moves lose Contact, but gain Weapon. 1.1x damage on recoil, Crash, and Pierce moves, halves recoil and crash damage. Sets the Kingly Authority Aura for 3 turns.",
		rating: 5,
		num: 1092,
	},
	flamepads: {
	...
	gravitywell: {
		onStart(source) {
			...
			const slot = source.ability1 === 'gravitywell' ? 1 : 2;
			this.field.addPseudoWeather('gravity', source, source.getAbility(slot as 1 | 2));
		},
		flags: {},
		name: "Gravity Well",
	...
	hardtopcarapace: {
		onTryHitPriority: 1,
		onTryHit(target, source, move) { 
			if (move && (move.flags?.bomb || move.flags?.crush)) { 
				this.add('-immune', target, '[from] ability: Hardtop Carapace');
				return null;
			}
		},
		onModifyGuardAction(guardActionId, pokemon) { if (guardActionId === 'guard') return 'mirrorshield'; },
		flags: { breakable: 1 },
		name: "Hardtop Carapace",
		shortDesc: "Immune to Bomb and Crush moves. If user's Guard Action is 'Guard' upgrade it to 'Mirror Shield'",
		rating: 2.5,
		num: 1024,
	},
	hellfire: { // Partially implemented in Pokemon.effectiveWeather() in sim/pokemon.ts
		...
		name: "Hellfire",
		shortDesc: "This Pokemon is always treated as if Sun is active. Under Sun, boosts SpAtk 1.3x, user takes 1/10HP at end of turn. Immune to Sea of Fire, Wyldfire",
		rating: 4,
		num: 1089,
	},
	herbalmedicine: {
	...
	icestilts: { // airborneness implemented in sim/pokemon.js:Pokemon#isGrounded
		...
		flags: {},
		name: "Ice Stilts",
		shortDesc: "User is Immune to grounded effects and cannot be grounded.",
		rating: 4,
		num: 1093,
	},
	infernalheat: {
	...
	longsnout: {
		...
		onModifyMove(move, pokemon) { if (move.flags?.sweep && move.target === 'normal'|| move.target === 'any') { move.target = 'allAdjacentFoes'; } },
		flags: {},
		name: "Long Snout",
	...
	magnetrise: {
		onStart(source) {
			const slot = source.ability1 === 'magnetrise' ? 1 : 2;
			this.field.addPseudoWeather('magnetrise', source, source.getAbility(slot as 1 | 2));
			this.add('-fieldstart', 'move: Magnet Rise', '[from] ability: Magnet Rise');
		},
		flags: {},
		name: "Magnet Rise",
		shortDesc: "On switch-in, sets Magnet Rise field effect for 4 turns, making Steel types and Magnet Rise users airborne.",
		rating: 2.5,
		num: 1047,
	},
	megablizzard: { // Partially implemented in Pokemon.effectiveWeather() in sim/pokemon.ts
		...
		onModifyDef(def, pokemon) {
			...
			if (pokemon.hasType('Steel') && !pokemon.hasType('Ice')) { def = this.modify(def, 0.5); }
			return def;
		},
		onResidualOrder: 1,
		onResidual(pokemon) {
			if (pokemon.effectiveWeather() !== 'snowscape') return;
			if (pokemon.hasType('Ice')) { this.heal(pokemon.baseMaxhp / 16, pokemon, pokemon); }
		},
		name: "Mega Blizzard",
		shortDesc: "This Pokemon is always treated as if Snowscape is active.",
		rating: 4,
	},
	megaluna: { // Partially implemented in Pokemon.effectiveWeather() in sim/pokemon.ts
		...
		onTryMove(attacker, defender, move) {
			...
			if (move.flags && (move.flags.light || move.flags.shadow)) {
				this.add('-fail', attacker, move, '[from] ability: Mega Luna');
				this.attrLastMove('[still]');
				return null;
			}
		},
		onUpdate(pokemon) {
			if (pokemon.effectiveWeather() !== 'eclipse') return;
			if (pokemon.hasAbility(['illuminate', 'shadowtag', 'shadowwalker'])) { pokemon.trapped = false; }
		},
		onResidualOrder: 1,
		onResidual(pokemon) {
			if (pokemon.effectiveWeather() !== 'eclipse') return;
			if (pokemon.hasAbility(['illuminate', 'astralaspect', 'lunaraspect', 'lunamancy', 'nightbloom'])) { this.heal(pokemon.baseMaxhp / 12, pokemon, pokemon); } 
			else if (pokemon.hasAbility(['chlorophyll', 'solaraspect', 'solarpower'])) { this.damage(pokemon.baseMaxhp / 12, pokemon, pokemon); }
		},
		name: "Mega Luna",
		shortDesc: "This Pokemon is always treated as if Eclipse is active.",
		rating: 4,
		num: 1105,
	},
	...
	memorywipe: {
		onStart(pokemon) {
			...
			for (const target of pokemon.adjacentFoes()) { 
				if (!activated) { 
					this.add('-ability', pokemon, 'Memory Wipe', 'boost');
					activated = true;
				}
				target.clearBoosts;
				this.add('-clearboost', target);
	...
	miracleeye: {
		...
		flags: {},
		name: "Miracle Eye",
		shortDesc: "User's Psychic type moves ignore Immunities",
		rating: 3,
		num: 1038,
	},
	mossarmor: {
		onDamagingHit(damage, target, source, move) { 
			if (!target.mossArmorBroken) { 
				if (move?.effectType === 'Move' && target.getMoveHitData(move).crit || move.type === 'Bug' || move.type === 'Fire' || move.type === 'Poison' || move.flags?.slicing) { // Break armor if hit by critical hit or a Bug/Fire/Poison or Slice move
					target.mossArmorBroken = true; 
					this.add('-activate', target, 'ability: Moss Armor');
					this.add('-message', `${target.name}'s Moss Armor broke!`);
				}
			}
		},
		onSourceModifyDamage(damage, source, target, move) { 
			if (!target.mossArmorBroken) { 
				if (move.type === 'Electric' || move.type === 'Ground' || move.type === 'Water') { 
					this.debug('Moss Armor resist');
		...
		flags: { breakable: 1 },
		name: "Moss Armor",
		shortDesc: "0.5x damage from Electric/Ground/Water moves. Armor breaks when hit by critical hits or Bug/Fire/Poison or Slice moves.",
		rating: 4.5,
		num: 1100,
	},
	mudarmor: {
		onDamagingHit(damage, target, source, move) { 
			if (!target.mudArmorBroken) { 
				if (move?.effectType === 'Move' && target.getMoveHitData(move).crit || move.type === 'Grass' || move.type === 'Ice' || move.type === 'Water') { // Break armor if hit by critical hit or Grass/Ice/Water move
		...
		onSourceModifyDamage(damage, source, target, move) { 
			if (!target.mudArmorBroken) { 
				if (move.type === 'Electric' || move.type === 'Fire' || move.type === 'Rock') { 
					this.debug('Mud Armor resist');
					return this.chainModify(0.5);
				}
			}
		},
	...
	musician: { // implemented in runMove in sim\battle-actions.ts
	    onBasePower(basePower, attacker, defender, move) {if (move.flags && move.flags.sound) {  return this.chainModify(1.3);  } },
		flags: {},
		name: "Musician", 
		shortDesc: "1.3x power on Sound moves; When another pokemon uses a Sound move, immediately copy it.",
		rating: 1.5,
    	num: 1041,
	},
	needleice: {
	...
	nightbloom: {
		...
		onTryHit(target, source, move) {
			if (target !== source && move.flags?.lunar) {
				...
				const boosted = target.boostBy({spe: 1});
				if (boosted) { this.add('-boost', target, 'spe', 1); }
				const oldhp = target.hp;
				target.hp = Math.min(target.maxhp, target.hp + Math.floor(target.baseMaxhp / 4));
				if (target.hp > oldhp) { this.add('-heal', target, target.getHealth); }
				return null;
		...
		onModifySpe(spe, pokemon) { if (this.field.isWeather('eclipse')) { return this.chainModify(2); } },
		onTrapPokemonPriority: -10,
		onTrapPokemon(pokemon) { if (pokemon.adjacentFoes().some(foe => foe.hasAbility('shadowtag'))) { pokemon.trapped = false; } },
		onMaybeTrapPokemonPriority: -10,
		onMaybeTrapPokemon(pokemon) { if (pokemon.adjacentFoes().some(foe => foe.hasAbility('shadowtag'))) { pokemon.maybeTrapped = false; } },
		flags: { breakable: 1 },
		name: "Nightbloom",
		shortDesc: "Immune to Lunar moves and Shadow Tag, When hit by a Lunar move: Heal 1/4HP and +1 Speed; 1.3x power on Lunar moves; Under Eclipse: 2x Speed",
		rating: 4,
		num: 1043,
	},
	nightmares: {
		...
		flags: {},
		name: "Nightmares",
		shortDesc: "At end of turn, deal 1/6HP to Sleeping foes. If a Sleeping foe wakes up: inflict them with Fear.",
		rating: 2.5,
		num: 1091,
	},
	oreeater: {
		onTryHit(target, source, move) { 
			if (target !== source && move.type === 'Steel') { 
				if (!this.heal(target.baseMaxhp / 4)) { this.add('-immune', target, '[from] ability: Ore Eater'); }
				return null;
			}
		},
		onSwitchIn(pokemon) { // Absorb Caltrops on entry (like Poison types absorb Toxic Spikes)
			const side = pokemon.side;
			const caltrops = side.sideConditions['caltrops'];
			if (caltrops) {
				const layers = caltrops.layers || 1;
				this.add('-sideend', side, 'Caltrops', '[from] ability: Ore Eater', '[of] ' + pokemon);
				delete side.sideConditions['caltrops'];
				const healAmount = Math.floor(pokemon.baseMaxhp / 10) * layers;
				if (healAmount > 0) this.heal(healAmount, pokemon, pokemon, this.dex.abilities.get('oreeater'));
			}
		},
		flags: { breakable: 1 },
		name: "Ore Eater",
		shortDesc: "Immune to Steel; When hit by a Steel type move: Heal 1/4HP. Absorb Caltrops on Switch-in",
		rating: 3.5,
		num: 1097,
	},
	packmentality: {
		onAllyAfterMove(source, target, move) {
			...
			if (move.target === 'allAdjacentFoes' || move.target === 'allAdjacent' || move.target === 'all' || move.target === 'foeSide') { // For spread moves, choose a random foe
		...
		onAllyTryHit(target, source, move) {
			if (move.id === 'pursuit' && (target.beingCalledBack || target.switchFlag)) {
				this.add('-message', `${this.effectState.target.name} covered ${target.name}'s retreat!`, '[from] ability: Pack Mentality');
				return null;
			}
		},
		flags: {},
		name: "Pack Mentality",
		shortDesc: "When an ally uses a damaging move, this Pokemon follows up with a 30 BP Dark type Physical attack. When an ally switches, but is hit by Pursuit, block it",
		rating: 2,
		num: 1044,
	},
	phantomfrost: {
		onModifyMove(move, pokemon) { if (move.type === 'Ice') { move.type2 = 'Ghost' } },
		onModifyDamage(damage, source, target, move) {
			if (move.type === 'Ice' && target.hasType('Fire')) {
				this.debug('Phantom Frost neutralizes Fire resistance');
				return this.chainModify(2);
			}
		},
		flags: {},
		name: "Phantom Frost",
		shortDesc: "This pokemon's Ice Type moves ignore Fire resistance, and gain Ghost Type as Type 2.",
		rating: 2,
		num: 1085,
	},
	pheremoninfusion: {
		flags: {},
		name: "Pheremone Infusion",
		shortDesc: "If user Poisons or Toxic Poisons a target, they are also Charmed, and if they know any moves learned via infusion those moves are replaced with Poison Powder for the rest of the battle",
		onSourceAfterSetStatus(status, target, source, effect) {
			if (!source || source === target) return;
			if (status.id !== 'psn' && status.id !== 'tox') return;
			target.addVolatile('attract', source);
			const movePool = this.dex.species.getMovePool(target.species.id);
			const poisonPowder = this.dex.moves.get('poisonpowder');
			let converted = false;
			for (const [i, moveSlot] of target.moveSlots.entries()) {
				if (moveSlot.id === poisonPowder.id) continue;
				const moveData = this.dex.moves.get(moveSlot.id);
				if (!moveData.flags.infusible || movePool.has(moveSlot.id)) continue;
				const newSlot = {
					move: poisonPowder.name,
					id: poisonPowder.id,
					pp: poisonPowder.pp,
					maxpp: poisonPowder.pp,
					target: poisonPowder.target,
					disabled: false,
					used: false,
				};
				target.moveSlots[i] = newSlot;
				target.baseMoveSlots[i] = newSlot;
				converted = true;
			}
			if (converted) this.add('-activate', source, 'ability: Pheremone Infusion');
		},
		rating: 1.5,
		num: 1107,
	},
	piercingdrill: {
		onBasePower(basePower, attacker, defender, move) { if (move.flags?.pierce) { return this.chainModify(1.1); } },
		onModifyMove(move, pokemon) {
			if (move.pierce) {
				const [num, denom] = move.pierce;
				move.pierce = (num * 2 >= denom) ? [1, 1] : [num * 2, denom];
		...
		flags: {},
		name: "Piercing Drill",
		shortDesc: "1.1x power on Pierce moves. Doubles the damage Pierce moves deal thru Protect (capped at full damage). Contact moves without Pierce gain it at 1/4 damage thru Protect.",
		rating: 4,
		num: 1086,
	},
	pressurizedcell: {
	...
	proudtusks: {
		...
		flags: {},
		name: "Proud Tusks",
		shortDesc: "1.3x power with Piercing moves. Doubles the damage Pierce moves deal thru Protect (capped at full damage).",
		rating: 3,
		num: 1047,
	},
	rainbowwings: {
		onStart(source) {
			...
			const slot = source.ability1 === 'rainbowwings' ? 1 : 2;
			this.field.addPseudoWeather('rainbow', source, source.getAbility(slot as 1 | 2));
		},
	...
	resonance: {
		...
		flags: { breakable: 1 },
		name: "Resonance",
		shortDesc: "Immune to Sound/Wind moves. When hit by Wind moves, reflects 1.2x damage as Sound damage to all foes. When hit by Sound moves, +1 Sp. Atk.",
		rating: 4,
		num: 1050,
	},
	rockbody: {
		onWeather(target, source, effect) { if (effect.id === 'sandstorm') { this.heal(target.baseMaxhp / 16); } },
		onImmunity(type) { if (type === 'sandstorm') return false; },
		onTryHit(target, source, move) {
			if (target !== source && move.type === 'Rock') {
				const oldhp = target.hp;
				target.hp = Math.min(target.maxhp, target.hp + Math.floor(target.baseMaxhp / 4));
				if (target.hp > oldhp) { this.add('-heal', target, target.getHealth, '[from] ability: Rock Body'); } 
				else { this.add('-immune', target, '[from] ability: Rock Body'); }
				return null;
			}
		},
		flags: { breakable: 1 },
		name: "Rock Body",
		shortDesc: "Under Sandstorm: heal 1/16HP at the end of every turn. This Pokemon heals 1/4 max HP when hit by Rock type moves; immunity if already at full HP.",
		rating: 3.5,
		num: 1051,
	},
	seaguardianscurse: {
	...
	seedgift: {
		onSwitchInPriority: -2,
		onStart(pokemon) {
			const abilityState = pokemon.ability1 === 'seedgift'? pokemon.abilityState1: pokemon.abilityState2;
			if (!abilityState.seedgiftUses) abilityState.seedgiftUses = 0;
			if (abilityState.seedgiftUses >= 6) {
			...
			if (healed) {
				abilityState.seedgiftUses++;
				const remaining = 6 - abilityState.seedgiftUses;
	...
	shapememory: {
  		name: "Shape Memory",
  		shortDesc: "Remembers height/weight scaling through Transform and switching.",
  		onStart(pokemon) {
  			pokemon.addVolatile('shapememory');
  			const h = pokemon.shapeMemoryHeightScale ?? 1;
  			const w = pokemon.shapeMemoryWeightScale ?? 1;
  			if (h !== 1) pokemon.heightmm = Math.max(10, Math.round(pokemon.heightmm * h));
	...
	sixminded: {
		onPrepareHit(source, target, move) { // Breath moves are split into 6 hits, each does 1/5 dmg
			if (move.category === 'Status' || move.multihit || move.flags?.charge || move.flags?.futuremove || move.spreadHit) return;
			if (move.flags?.breath) {
				move.multihit = 6;
				move.multihitType = 'sixminded';
		...
		flags: {},
		name: "Six Minded",
		shortDesc: "Breath moves are split into 6 hits with Smart Target, each hit deals 1/5 damage. Can use moves while asleep.",
		rating: 4,
		num: 1095,
	},
	solaraspect: {
		...
		onSourceModifyDamage(damage, source, target, move) { 
			if (move.flags && move.flags.lunar) { 
				this.debug('Solar Aspect weakness to lunar');
				return this.chainModify(2);
			}
		},
		onTryHitPriority: 1,
		onTryHit(target, source, move) {
			if (move.flags?.solar && target !== source) {
	...
	soothingvoice: {
		...
		onModifyTypePriority: -1,
		onModifyType(move, pokemon) { if (move.flags?.sound) { move.type = 'Poison'; } },
		onBasePower(basePower, attacker, defender, move) { if (move.flags?.sound) { return this.chainModify(1.2); } },
		flags: {},
		name: "Soothing Voice",
		shortDesc: "This Pokemon's Sound moves become Poison type and have 1.2x power, and gain a 20% chance to Poison and lower the target's Special Attack -1 stage.",
		rating: 1.5,
		num: 1099,
	},
	souleater: {
	...
	soothingfeelers: {
		onDamagingHit(damage, target, source, move) { 
			if (this.checkMoveMakesContact(move, source, target)) { 
				if (this.randomChance(3, 10)) { 
					source.trySetStatus('aura', target, {
		...
		onSourceHit(target, source, move) { 
			if (move && this.checkMoveMakesContact(move, source, target)) { 
				if (this.randomChance(3, 10)) { 
					target.trySetStatus('aura', source, {
						Ability: 'tranquilspirit',
						auraName: 'Tranquil Spirit',
						auraDuration: 3,
					});
				}
	...
	spellhorizon: {
		onStart(source) { this.field.setRoom('magicroom'); },
		flags: {},
		name: "Spell Horizon",
	...
	spicyspray: {
		onBasePower(basePower, attacker, defender, move) { if (move.flags?.breath) { return this.chainModify(1.2); } },
	...
	starterdough: {
		onStart(pokemon) {
			const abilityState = pokemon.ability1 === 'starterdough'? pokemon.abilityState1: pokemon.abilityState2;
			if (abilityState.bakedState === 'burnt') {
		...
		onSourceModifyDamage(damage, source, target, move) {
			if (move.flags?.breath && source.hasAbility('starterdough')) {
				this.heal(damage, target, source);
				return 0;
			}
		},
		...
		onModifyDef(def, pokemon) {
			const abilityState = pokemon.ability1 === 'starterdough'? pokemon.abilityState1: pokemon.abilityState2;
			if (abilityState.bakedState === 'baked') return this.chainModify(1.5);
			if (abilityState.bakedState === 'burnt') return this.chainModify(2);
		},
		onTryHit(target, source, move) {
			if (target !== source && move.type === 'Fire') {
				const abilityState = target.ability1 === 'starterdough'? target.abilityState1: target.abilityState2;
				if (!abilityState.bakedState) abilityState.bakedState = 'none';
				if (abilityState.bakedState === 'none') {
					abilityState.bakedState = 'baked';
					this.add('-activate', target, 'ability: Starter Dough');
					this.add('-message', `${target.name} has been baked!`);
				} else if (abilityState.bakedState === 'baked') {
					abilityState.bakedState = 'burnt';
	...
	steelwings: {
		onBasePower(basePower, attacker, defender, move) { if (move.flags?.slicing) { return this.chainModify(1.5); } },
		onModifyMove(move, pokemon) {  if (move.flags?.wing) { move.flags.slicing = 1; } },
		flags: {},
		name: "Steel Wings",
	...
	stormseye: {
		onTryHit(target, source, move) { 
			if (target !== source && move.flags?.wind) { 
				if (!this.boost({ def: 1, spe: 1 })) { this.add('-immune', target, '[from] ability: Storms Eye'); }
				return null;
			}
		},
		onAnyRedirectTarget(target, source, source2, move) {
			...
			if (this.validTarget(this.effectState.target, source, redirectTarget)) {
				...
				if (this.effectState.target !== target) { this.add('-activate', this.effectState.target, 'ability: Storms Eye'); }
				return this.effectState.target;
			}
		},
		flags: { breakable: 1 },
		name: "Storm's Eye",
		shortDesc: "Immune to Wind moves. Redirects all Wind moves on the field to the user. When hit by Wind move: Boost Defense and Speed 1 stage.",
		rating: 3,
		num: 1101,
	},
	superconductor: {
	...
	surgingmigraine: {
		onStart(source) { this.field.setRoom('wonderroom'); },
		onResidualOrder: 5,
		onResidual(pokemon) {
			if (!this.field.pseudoWeather['wonderroom']) {
				this.field.setRoom('wonderroom');
		...
		shortDesc: "On switch-in sets Wonder Room until user leaves the field. If removed, reestablishes itself at end of turn",
		rating: 3.5,
		num: 1088,
	},
	swordtail: {
		onBasePower(basePower, attacker, defender, move) { if (move.flags?.slicing) { return this.chainModify(1.3); } },
		onModifyMove(move, pokemon) {  if (move.flags?.sweep) { move.flags.slicing = 1; } },
		flags: {},
		name: "Sword Tail",
	...
	teraformblade: {
		onModifyMove(move) { if (move.flags?.slicing) { move.infiltrates = true; } },
		...
		flags: {},
		name: "Teraform Blade",
		shortDesc: "1.5x power on Slice moves. User's Slice moves ignore screens and Tera.",
		rating: 3.5,
		num: 1103,
	},
	thorns: {
	...
	timebreak: {
		onStart(source) {
			const slot = source.ability1 === 'timebreak' ? 1 : 2;
			this.field.addPseudoWeather('timebreak', source, source.getAbility(slot as 1 | 2));
		},
		onEnd(pokemon) {
	...
	volvation: {
		...
		onSourceModifyAtk(atk, attacker, defender, move) { 
			if (move.flags && (move.flags.bullet || move.flags.bite || move.flags.bomb || move.flags.claw || move.flags.explosive || move.flags.kick || move.flags.pierce || move.flags.punch || move.flags.slicing)) { 
				this.debug('Volvation weaken');
				return this.chainModify(0.5);
			}
		},
	...
	waterlogged: {
		onStart(source) {
			const slot = source.ability1 === 'waterlogged' ? 1 : 2;
			this.field.addPseudoWeather('swamp', source, source.getAbility(slot as 1 | 2));
		},
		flags: {},
		name: "Waterlogged",
	...
	webarmor: {
		...
		onDamagingHit(damage, target, source, move) { 
			if (!target.webArmorBroken && (move.type === 'Fire' || move.flags?.slicing)) {
	...
	craftergod: {
		...
		onModifyAtk(atk, pokemon) { 
			this.debug('Crafter God ATK boost');
			return this.chainModify(1.2);
		},
		onModifySpe(spe, pokemon) { 
			this.debug('Crafter God Speed boost');
			return this.chainModify(1.2);
		},
		onTryAddVolatile(status, pokemon) { 
			if (status.id === 'flinch') { 
				this.add('-immune', pokemon, '[from] ability: Crafter God');
				return null;
			}
		},
	...
	fellstinger: {
		onBasePower(basePower, attacker, defender, move) {
			if (move.flags?.pierce) { 
				this.debug('Fell Stinger pierce boost');
				return this.chainModify(2);
			}
		},
		onModifyMove(move, attacker, defender) {  if (move.pierce && (move.pierce[0] / move.pierce[1]) < 0.5) { move.pierce = [1, 2]; } },
		flags: {},
		name: "Fell Stinger",
		shortDesc: "2x power on Pierce moves. Pierce moves deal at least 1/2 damage thru Protect.",
		rating: 4,
		num: -4,
	},
	giantslayer: {
		onModifyAtkPriority: 5,
		onModifyAtk(atk, pokemon) { 
			this.debug('Giant Slayer ATK boost');
			return this.chainModify(1.5);
		},
		onTryBoost(boost, target, source, effect) { 
			if (effect.name === 'Intimidate' && boost.atk) {
	...
	imperialbastion: {
		...
		onModifyDef(def, pokemon) { 
			this.debug('Imperial Bastion DEF boost');
			return this.chainModify(1.5);
		},
		onTryBoost(boost, target, source, effect) { 
			if (effect.name === 'Intimidate' && boost.atk) {
	...
	indomitablespirit: {
		onBasePower(basePower, attacker, defender, move) { 
			if (move.flags?.aura || move.flags?.dance || move.flags?.heal || move.flags?.kick || move.flags?.punch || move.flags?.slicing || move.flags?.sweep || move.flags?.throw) { 
				this.debug('Indomitable Spirit boost');
				return this.chainModify(1.3);
			}
		},
		onTryAddVolatile(status, pokemon) { 
			if (status.id === 'flinch' || status.id === 'confusion') { 
				this.add('-immune', pokemon, '[from] ability: Indomitable Spirit');
				return null;
			}
		},
		flags: {},
		name: "Indomitable Spirit",
		shortDesc: "1.3x damage on Aura/Dance/Heal/Kick/Punch/Slice/Sweep/Throw moves. Immune to flinch and confusion.",
		rating: 4.5,
		num: -7,
	},
	kinglyauthority: {
		onFoeTryMove(target, source, move) {
			...
			if ((source.isAlly(dazzlingHolder) || move.target === 'all') && move.priority > 0.1) {
				this.attrLastMove('[still]');
				this.add('cant', dazzlingHolder, 'ability: Flame Emperor', move, `[of] ${target}`);
				return false;
			}
		},
		onDamage(damage, target, source, effect) { 
		...
		onBasePowerPriority: 23,
		onBasePower(basePower, attacker, defender, move) { 
			if (move.recoil || move.hasCrashDamage || move.flags?.crash || move.flags?.pierce || move.flags?.sweep) { 
				this.debug('flameemp boost');
				return this.chainModify(1.3);
			}
		},
		...
		shortDesc: "User's Contact moves lose Contact, but gain Weapon. 1.3x damage on recoil, Crash, Pierce, and Sweep moves, halves recoil and crash damage. Protects user and ally from priority effects.",
		rating: 5,
		num: -13,
	},
	mentalsurge: {
	...
	ragingbull: {
		onModifyMove(move, attacker, defender) {
			if (move.flags?.contact) {
				if (move.pierce) {
					const [num, denom] = move.pierce;
					const newNum = num * 8 + denom;
					const newDenom = denom * 8;
					move.pierce = (newNum >= newDenom) ? [1, 1] : [newNum, newDenom];
				} else {
					move.pierce = [1, 8];
				}
			}
	...
	tranquilspirit: {
		...
		onBeforeMove(pokemon) { 
			if (this.randomChance(2, 10)) {
				...
				this.add('-message', `${pokemon.name} is lulled about.`);
				return false;
			}
		},
		flags: {},
		name: "Tranquil Spirit",
		shortDesc: "0.7x Attack and Special Attack. 20% chance to be unable to move.",
		rating: 1,
		num: -11,
	},
	weaponconjuration: {
	...
	battlearmor: {
		onCriticalHit: false,
		onModifyGuardAction(guardActionId, pokemon) { if (guardActionId === 'guard') return 'protect'; },
		flags: { breakable: 1 },
		name: "Battle Armor",
		shortDesc: "Immune to critical hits. If user's Guard Action is 'Guard' upgrade it to 'Protect'",
		rating: 1,
		num: 4,
	},
	...
	clearbody: {
		onTryBoost(boost, target, source, effect) {
			...
			let showMsg = false;
			for (const [i, value] of Object.entries(boost) as [BoostID, number][]) {
				if (value < 0) {
					delete boost[i];
					showMsg = true;
				}
	...
	cloudnine: {
		...
		onStart(pokemon) {
			const abilityState = pokemon.ability1 === 'cloudnine'? pokemon.abilityState1: pokemon.abilityState2;
			abilityState.ending = false;
			this.eachEvent('WeatherChange', this.effect);
		},
		onEnd(pokemon) {
			const abilityState = pokemon.ability1 === 'cloudnine'? pokemon.abilityState1: pokemon.abilityState2;
			abilityState.ending = true;
			this.eachEvent('WeatherChange', this.effect);
		},
	...
	hustle: { // This should be applied directly to the stat as opposed to chaining with the others
		...
		onModifySpAPriority: 5,
		onModifySpA(spa) { return this.modify(spa, 1.5); },
		onSourceModifyAccuracyPriority: -1,
		onSourceModifyAccuracy(accuracy, target, source, move) { if (!!(move.category === 'Status') && typeof accuracy === 'number') { return this.chainModify([1, 2]); } },
		flags: {},
		name: "Hustle",
		shortDesc: "1.5x power, 0.5x accuracy on damaging moves.",
		rating: 3.5,
		num: 55,
	},
	hypercutter: {
	...
	lightningrod: {
		onTryHit(target, source, move) {
			if (target !== source && move.type === 'Electric') { 
				if (!this.boost({ spa: 1 })) { this.add('-immune', target, '[from] ability: Lightning Rod'); }
				return null; }
		},
		onAnyRedirectTarget(target, source, source2, move) {
	...
	limber: {
		...
		onTryBoost(boost, target, source, effect) {
			if (boost.spe && boost.spe < 0) {
				...
				if (!(effect as ActiveMove).secondaries) { this.add("-fail", target, "unboost", "Speed", "[from] ability: Limber", `[of] ${target}`); }
			}
		},
		flags: { breakable: 1 },
		name: "Limber",
	...
	magmaarmor: {
		...
		onSetStatus(status, target, source, effect) {
			...
			if ((effect as Move)?.status) { this.add('-immune', target, '[from] ability: Magma Armor'); }
			return false;
		},
		onImmunity(type, pokemon) { if (type === 'Water') return false; },
		onSourceModifyDamage(damage, source, target, move) { 
			if (move.type === 'Fire' || move.type === 'Ice') { 
				this.debug('Magma Armor resistance to Fire/Ice');
				return this.chainModify(0.5);
			}
			if (move.type === 'Water') { 
				this.debug('Magma Armor immunity to Water');
	...
	magneticpulse: {
		onDamagingHit(damage, target, source, move) {
			if (move.flags?.contact && source.hasType('Electric')) {
				const newTypes = source.getTypes(true).filter(type => type !== 'Electric');
				if (!newTypes.length) newTypes.push('Banal');
		...
		onSourceModifyDamage(damage, source, target, move) { 
			if (move.type === 'Electric') { 
				this.debug('Magnetic Pulse resistance to Electric');
				return this.chainModify(0.5);
			}
		},
		onTrapPokemonPriority: -10,
		...
		flags: {},
		name: "Magnetic Pulse",
		shortDesc: "Electric types that make Contact with the user lose their Electric type. User resists Electric type moves. Immune to Magnet Pull.",
		rating: 3.5,
		num: 13006,
	},
	marvelscale: {
		onSourceModifyDamage(damage, source, target, move) { 
			if (move.flags && move.flags.light) { 
				this.debug('Marvel Scale resistance to light');
				return this.chainModify(0.5);
			}
		},
		onModifyDefPriority: 6,
		...
		onModifySecondaries(secondaries, target, source, move) { if (target.status) { 
				this.debug('Marvel Scale (Shield Dust logic) prevent secondary');
	...
	minus: {
		onModifySpAPriority: 5,
		onModifySpA(spa, pokemon) { // Don't apply SpAtk boost if Plus is on the same Pokemon
			if (pokemon.hasAbility('plus')) return;
			const hasPlusMinus = pokemon.hasAbility(['minus', 'plus']) || pokemon.allies().some(ally => ally.hasAbility(['minus', 'plus']));
			if (hasPlusMinus) { return this.chainModify(1.5); }
		},
		onBasePower(basePower, attacker, defender, move) {
			const hasPlusMinus = attacker.hasAbility(['minus', 'plus']) || attacker.allies().some(ally => ally.hasAbility(['minus', 'plus']));
			if (hasPlusMinus && move.flags && move.flags.beam) { return this.chainModify(1.3); }
		},
		onRedirectTarget(target, source, source2, move) {
			const hasPlusMinus = source.hasAbility(['minus', 'plus']) ||  source.allies().some(ally => ally.hasAbility(['minus', 'plus']));
			if (hasPlusMinus) { return target; }
		},
		flags: {},
		name: "Minus",
		shortDesc: "If this Pokemon or ally has Plus/Minus: 1.5x SpA, 1.3x Beam power, ignore redirection. Works with dual ability system.",
		rating: 2,
		num: 58,
	},
	naturalcure: {
	...
	overgrow: {
		...
		onBasePower(basePower, attacker, defender, move) {
			if (move.name && move.name.includes('Solar')) { 
				this.debug('Overgrow Solar boost');
				return this.chainModify([1200, 1000]);
			}
			if (move.id === 'vinewhip') { 
				this.debug('Overgrow Vine Whip boost');
				return this.chainModify(2);
			}
		},
		onResidualOrder: 5,
		onResidualSubOrder: 2,
		onResidual(pokemon) { if (this.field.isTerrain('grassyterrain') && pokemon.isGrounded() && !pokemon.isSemiInvulnerable()) { this.heal(pokemon.baseMaxhp / 8, pokemon, pokemon); } },
		flags: {},
		name: "Overgrow",
	...
	owntempo: {
		...
		onTryBoost(boost, target, source, effect) {
			if (boost) { // Block all stat stage changes (positive and negative)
				for (const [i] of Object.entries(boost) as [BoostID, number][]) { delete boost[i]; }
	...
	plus: {
		...
		onBasePower(basePower, attacker, defender, move) {
			const hasPlusMinus = attacker.hasAbility(['minus', 'plus']) ||  attacker.allies().some(ally => ally.hasAbility(['minus', 'plus']));
			if (hasPlusMinus && move.name && move.name.includes('Pulse')) {
				this.debug('Plus Pulse boost');
				return this.chainModify([1300, 1000]);
			}
		},
		onTryHit(target, source, move) {
			const hasPlusMinus = source.hasAbility(['minus', 'plus']) ||  source.allies().some(ally => ally.hasAbility(['minus', 'plus']));
			if (hasPlusMinus && target !== source && move.type === 'Electric') {
				if (!this.heal(target.baseMaxhp / 4)) { this.add('-immune', target, '[from] ability: Plus'); }
				return null;
			}
		},
		onAnyRedirectTarget(target, source, source2, move) {
			const hasPlusMinus = source.hasAbility(['minus', 'plus']) ||  source.allies().some(ally => ally.hasAbility(['minus', 'plus']));
			if (!hasPlusMinus) return;
			if (move.type !== 'Electric' || move.flags['pledgecombo']) return;
			const redirectTarget = ['randomNormal', 'adjacentFoe'].includes(move.target) ? 'normal' : move.target;
			if (this.validTarget(this.effectState.target, source, redirectTarget)) {
				if (move.smartTarget) move.smartTarget = false;
		...
		flags: { breakable: 1 },
		name: "Plus",
		shortDesc: "If this Pokemon or ally has Plus/Minus: 1.5x SpA, 1.3x Pulse power, redirect Electric moves, heal 1/4HP from Electric damage. Works with dual ability system.",
		rating: 4,
		num: 57,
	},
	poisonpoint: {
	...
	rockhead: {
		onTryAddVolatile(status, pokemon) { 
			if (status.id === 'flinch') { 
				this.add('-immune', pokemon, '[from] ability: Rock Head');
				return null;
			}
		},
		onBasePowerPriority: 8,
		onBasePower(basePower, attacker, defender, move) { 
			if (move.flags && move.flags.crush) { 
				this.debug('Rock Head crush resist');
				return this.chainModify(0.5);
			}
		},
		onDamage(damage, target, source, effect) { 
	...
	runaway: {
		...
		onMaybeTrapPokemonPriority: -10,
		onMaybeTrapPokemon(pokemon) { pokemon.maybeTrapped = false; },
		forcedGuardAction: 'runaway',
		flags: {},
		name: "Run Away",
		desc: "Immune to trapping effects. Replaces user's Guard action with Run Away",
		shortDesc: "Immune to trapping effects. Replaces user's Guard action with Run Away",
		rating: 0,
		num: 50,
	},
	sandstream: {
	...
	sandveil: {
		...
		onModifyDef(def, pokemon) {
			if (this.field.isWeather('sandstorm')) { 
				this.debug('Sand Veil - boosting DEF');
				return this.chainModify([1200, 1000]);
			}
		},
		onModifySpDPriority: 6,
		onModifySpD(spd, pokemon) { 
			if (this.field.isWeather('sandstorm')) { 
				this.debug('Sand Veil - boosting SpDEF');
				return this.chainModify([1200, 1000]);
		}
		},
		flags: { breakable: 1 },
	...
	shadowtag: {
		onFoeDisableMove(pokemon) {
			if (pokemon.trapped) { for (const moveSlot of pokemon.moveSlots) { 
	...
	shedskin: {
		...
		flags: {},
		name: "Shed Skin",
		shortDesc: "1/2 chance to cure user's Status [except Aura] at end of turn. Immune to trapping effects.",
		rating: 3.5,
		num: 61,
	},
	shellarmor: {
	...
	sturdy: {
		...
		flags: { breakable: 1 },
		name: "Sturdy",
		shortDesc: "If user is at 100%HP, and would be killed by an attack: survive with 1HP. If user's Guard Action is 'Guard' upgrade it to 'Guardlv2'",
		rating: 3,
		num: 5,
	},
	suctioncups: {
	...
	swarm: {
		onAllyAfterMove(source, target, move) {
			...
			if (!target || target.fainted || !target.isActive) return;
			if (move.category === 'Status') return;
			if (target === this.effectState.target) return; // don't attack yourself if all targets you
	...
	thickfat: {
		onSourceModifyAtkPriority: 6,
		onSourceModifyAtk(atk, attacker, defender, move) { 
			if (move.type === 'Ice' || move.type === 'Fire' || (move.flags && move.flags.punch)) { 
	...
	trace: {
		...
		name: "Trace",
		shortDesc: "Copies a random traceable active ability of the foe across from the user.",
		rating: 2.5,
		num: 36,
	},
	truant: {
		...
		flags: {},
		name: "Truant",
		shortDesc: "User is unable to act every other turn. User's moves deal 1/2 damage thru Protect.",
		rating: -1,
		num: 54,
	},
	vitalspirit: {
	...
	wonderguard: {
		...
		flags: { failroleplay: 1, noentrain: 1, failskillswap: 1, breakable: 1 },
		name: "Wonder Guard",
	...
	airlock: {
		...
		onStart(pokemon) {
			const abilityState = pokemon.ability1 === 'airlock'? pokemon.abilityState1: pokemon.abilityState2;
			abilityState.ending = false;
			this.eachEvent('WeatherChange', this.effect);
		},
		onEnd(pokemon) {
	...
	anticipation: {
		onStart(pokemon) { 
			for (const target of pokemon.foes()) {
				 for (const moveSlot of target.moveSlots) {
					...
					if (this.dex.getEffectiveness(move.type, pokemon) > 0) { 
		...
		onModifyAccuracy(accuracy, target, source, move) {
			...
			if (move.priority > 0 || this.dex.getEffectiveness(move.type, target) > 0) { return this.chainModify(2 / 3); }
		},
		forcedGuardAction: 'detect',
		flags: {},
		name: "Anticipation",
	...
	gluttony: {
		onStart(pokemon) {
			const abilityState = pokemon.ability1 === 'gluttony'? pokemon.abilityState1: pokemon.abilityState2;
	...
	heatproof: {
		...
		flags: { breakable: 1 },
		name: "Heatproof",
		shortDesc: "Immune to Burn. Halves incoming damage from Fire and Ice type moves.",
		rating: 2.5,
		num: 85,
	},
	honeygather: {
	...
	hydration: { // 2x aqua ring healing implemented in moves.ts
		onResidualOrder: 5,
		onResidualSubOrder: 3,
		onResidual(pokemon) {
		...
		flags: {},
		name: "Hydration",
		shortDesc: "Under Rain or Aqua Ring: Cure user's Status at end of turn. Doubles Aqua Ring healing",
		rating: 1.5,
		num: 93,
	},
	icebody: {
		...
		onTryHit(target, source, move) {
			if (target !== source && move.type === 'Ice') {
				const oldhp = target.hp;
				target.hp = Math.min(target.maxhp, target.hp + Math.floor(target.baseMaxhp / 4));
				if (target.hp > oldhp) { this.add('-heal', target, target.getHealth, '[from] ability: Ice Body'); } 
				else { this.add('-immune', target, '[from] ability: Ice Body'); }
				return null;
			}
		},
		flags: {},
		name: "Ice Body",
		shortDesc: "Under Hail or Snow: heal 1/16HP at the end of every turn. This Pokemon heals 1/4 max HP when hit by Ice type moves; immunity if already at full HP.",
		rating: 1,
		num: 115,
	},
	ironfist: {
	...
	magicguard: {
		...
		onSourceModifyDamage(damage, source, target, move) { 
			if (move.flags && move.flags.magic) { 
				this.debug('Magic Guard resistance to magic');
				return this.chainModify(0.5);
			}
		},
		forcedGuardAction: 'magiccoat',
		flags: {},
		name: "Magic Guard",
	...
	motordrive: {
		...
		onStart(pokemon) { 
			if (pokemon.volatiles['charge'] && !pokemon.volatiles['motordrivechargeboosted']) {
				this.boost({ spe: 1 }, pokemon, pokemon, null, true);
				pokemon.addVolatile('motordrivechargeboosted');
			}
		},
	...
	noguard: {
		...
		flags: {},
		name: "No Guard",
		shortDesc: "All moves used by or that target the user bypass accuracy checks. User loses access to their Guard Action",
		rating: 4,
		num: 99,
	},
	normalize: {
	...
	simple: {
		onChangeBoost(boost, target, source, effect) { for (const [i, value] of Object.entries(boost) as [BoostID, number][]) { boost[i] = value * 2; } },
		flags: { breakable: 1 },
		name: "Simple",
		rating: 4,
		num: 86,
	...
	sniper: {
		onModifyDamage(damage, source, target, move) {
			if (target.getMoveHitData(move).crit) { 
				this.debug('Sniper crit boost');
				return this.chainModify(1.5);
			}
			if ((move.flags && (move.flags.bullet || move.flags.pierce))) { 
				this.debug('Sniper bullet/piercing boost');
				return this.chainModify(1.5);
			}
		},
		flags: {},
	...
	solarpower: {
        ...
        onBasePower(basePower, attacker, defender, move) { 
			if (move.flags && move.flags.solar) { 
				this.debug('Solar Power solar move boost');
				return this.chainModify([1200, 1000]);
            }
        },
		onWeather(target, source, effect) {
	...
	solidrock: {
		...
		onFlinch(target) { 
			this.add('-immune', target, '[from] ability: Solid Rock');
			return false;
		},
		onTryHit(target, source, move) { 
			if (move && move.flags && move.flags.slicing) { 
				this.add('-immune', target, '[from] ability: Solid Rock');
				return null;
			}
		},
	...
	superluck: {
		...
		onModifyAccuracy(accuracy, target) {
			if (typeof accuracy !== 'number') return;
			if (accuracy < 100) { 
				this.debug('Super Luck - reducing opponent accuracy');
		...
		onModifySecondaries(secondaries) {
			...
			return secondaries.map(effect => { 
				if (effect.chance) { return {...effect, chance: effect.chance * 1.5}; }
				return effect;
			});
		},
		flags: {},
	...
	technician: {
		...
		onBasePower(basePower, attacker, defender, move) {
            ...
            if (move.flags && move.flags.weapon) { 
				this.debug('Technician weapon boost');
                return this.chainModify(1.3);
            }
		},
		flags: {},
	...
	tintedlens: {
		onModifyDamage(damage, source, target, move) {
            ...
            if (move.flags && move.flags.light) { 
				this.debug('Tinted Lens light resist');
                return this.chainModify(0.5);
            }
		},
		flags: {},
	...
	contrary: {
		onChangeBoost(boost, target, source, effect) { for (const [i, value] of Object.entries(boost) as [BoostID, number][]) { boost[i] = value * -1; } },
		flags: { breakable: 1 },
		name: "Contrary",
	...
	cursedbody: {
		onDamagingHit(damage, target, source, move) {
			...
			if (!move.flags?.futuremove && move.id !== 'struggle') { if (this.randomChance(3, 10)) {
					source.addVolatile('disable', this.effectState.target);
					if (!source.volatiles['curse'] && source.hp && source.side && source.side.name !== target.side.name) { 
	...
	defiant: {
		onAfterEachBoost(boost, target, source, effect) {
			...
			for (const [, value] of Object.entries(boost) as [BoostID, number][]) {
				if (value < 0) {
					statsLowered = true;
					break;
				}
	...
	friendguard: {
		...
		flags: { breakable: 1 },
		name: "Friend Guard",
		shortDesc: "Reduces damage to Ally by 25%. When an ally switches, but is hit by Pursuit, block it",
		rating: 0,
		num: 132,
	},
	harvest: {
	...
	heavymetal: {
		...
		onSourceModifyDamage(damage, source, target, move) { 
			if (move.flags && move.flags.sweep) { 
				this.debug('Heavy Metal weakness to sweeping');
				return this.chainModify(2);
			}
		},
		flags: { breakable: 1 },
	...
	illusion: { // yes, you can Illusion an active pokemon but only if it's to your right
		...
		onDamagingHit(damage, target, source, move) {
			if (target.illusion) {
				const illusionState = target.ability1 === 'illusion'? target.abilityState1: target.abilityState2;
				this.singleEvent('End', this.dex.abilities.get('Illusion'), illusionState, target, source, move);
			}
		},
	...
	justified: {
		...
		flags: {},
		name: "Justified",
		shortDesc: "Immune to Dark type. Resist Binding, Sweep, Shadow moves. When hit by a Dark type, Binding, Sweep, or Shadow move, or when cursed, +1 Attack.",
		rating: 3.5,
		num: 154,
	},
	lightmetal: {
		...
		onSourceModifyDamage(damage, source, target, move) { 
			if (move.flags && move.flags.wind) { 
				this.debug('Light Metal weakness to wind');
				return this.chainModify(2);
			}
		},
		flags: { breakable: 1 },
	...
	telepathy: {
		...
		onModifyAccuracy(accuracy, target, source, move) {
			...
			if (!target.isAlly(source)) return;
			const telepathyUser = source.side.active.find((ally: Pokemon | null) => ally?.hasAbility('telepathy'));
			if (!telepathyUser) return;
			if (telepathyUser.moveThisTurnResult === false) {
				this.add('-activate', target, 'ability: Telepathy');
				return 0;
			}
		},
	...
	teravolt: {
		...
		onBasePower(basePower, attacker, defender, move) { if (move.type === 'Electric') {
				const multiplier = attacker.volatiles['overdrive'] ? 1.5 : 1.3;
	...
	wonderskin: {
		...
		onModifyAccuracy(accuracy, target, source, move) {
			 if (move.category === 'Status' && typeof accuracy === 'number') { 
				this.debug('Wonder Skin - halving accuracy'); 
				return accuracy * 0.5; 
			} 
		},
		onResidualOrder: 29,
		onResidual(pokemon) { 
			if (pokemon.status && this.randomChance(1, 3)) { 
				this.add('-activate', pokemon, 'ability: Wonder Skin', '[status cure]');
				pokemon.cureStatus();
			}
		},
		flags: { breakable: 1 },
	...
	aromaveil: {
		...
		onStart(pokemon) {
			...
			for (const ally of pokemon.alliesAndSelf()) { 
				for (const status of cureStatuses) { 
					if (ally.volatiles[status]) {
	...
	aurabreak: {
		onStart(pokemon) { 
			...
			for (const target of this.getAllActive()) {
				...
				if (target.status === 'aura') {
					...
					target.cureStatus();
					target.setAbility(target.baseAbility2);
		...
		onBeforeMove(attacker, defender, move) { 
			if (move.flags?.aura) { 
				this.add('-message', `${attacker.name} cannot use aura moves while Aura Break is active!`);
				return false;
			}
		},
		...
		onAnyModifyDamage(damage, source, target, move) { 
			if (move.flags?.aura) { 
				if (source.hasAbility('auramaster') || source.hasAbility('spritiguide')) { 
					this.debug('Aura Break inverting Aura Master/Spirit Guide');
		...
		onSuppressAbility(pokemon) {
			...
			return activeSlots.some((slot: any) => suppressedAbilities.includes(slot.id));
		},
		flags: { breakable: 1 },
		name: "Aura Break",
	...
	deltastream: {
		...
		flags: {},
		name: "Delta Stream",
		shortDesc: "Sets Delta Stream while user is on the field. Delta stream overpowers all weathers and primal weathers, replaces Eclipse",
		rating: 4,
		num: 191,
	},
	desolateland: {
	...
	flowerveil: { // Protects user from status, redirection, and stat lowering effects. Under Sun, Grassy, or Misty Terrain, also protects ally.
		onSetStatus(status, target, source, effect) { 
			if (target.hasAbility('flowerveil')) { 
		...
		onAllySetStatus(status, target, source, effect) {
			...
			if (holder.hasAbility('flowerveil')) {
				...
				if ((terrain === 'grassyterrain' || terrain === 'mistyterrain' || weather === 'sunnyday' || weather === 'desolateland') && source && target !== source) { 
					this.add('-block', target, 'ability: Flower Veil', `[of] ${holder}`);
					return null;
				}
			}
		},
		...
		onAllyRedirectTarget(target, source, source2, move) {
			...
			if (holder.hasAbility('flowerveil')) {
				const terrain = this.field.terrain;
				const weather = this.field.weather;
				if ((terrain === 'grassyterrain' || terrain === 'mistyterrain' || weather === 'sunnyday' || weather === 'desolateland')) { 
					this.add('-block', target, 'ability: Flower Veil', `[of] ${holder}`);
					return target;
				}
			}
	...
	parentalbond: { // Damage modifier implemented in BattleActions#modifyDamage()
		onPrepareHit(source, target, move) {
			if (move.category === 'Status' || move.multihit || move.flags['noparentalbond'] || move.flags['charge'] || move.flags['futuremove'] || move.spreadHit) return;
			move.multihit = 2;
	...
	toughclaws: {
		onBasePowerPriority: 21,
		onBasePower(basePower, attacker, defender, move) { 
			if (move.flags && move.flags.claw) { return this.chainModify(1.5); } 
	...
	berserk: {
		onDamage(damage, target, source, effect) { 
			if ( effect.effectType === "Move" && !effect.multihit && !(effect.hasSheerForce && source.hasAbility('sheerforce')) ) { this.effectState.checkedBerserk = false; } 
	...
	comatose: {
		...
		onSetStatus(status, target, source, effect) { 
			if ((effect as Move)?.status) { this.add('-immune', target, '[from] ability: Comatose'); }
			return false;
	...
	dancer: { // implemented in runMove in sim\battle-actions.ts
	...
	fullmetalbody: {
		...
		onSourceModifyDamage(damage, source, target, move) { 
			if ( (move.flags?.bullet || move.flags?.kick || move.flags?.pierce || move.flags?.punch || move.flags?.slicing) ) { 
				this.debug('Full Metal Body resistance');
				return this.chainModify(0.5);
			}
		},
		flags: {},
	...
	prismarmor: {
		...
		onTryHit(target, source, move) { 
			if (move.flags && (move.flags.beam || move.flags.light)) { 
				this.add('-immune', target, '[from] ability: Prism Armor');
				return null;
			}
		},
		flags: {},
	...
	receiver: {
		onBasePowerPriority: 8,
		onBasePower(basePower, attacker, defender, move) {
			if (move.flags && move.flags.throw) { 
				this.debug('Receiver throwing boost');
				return this.chainModify([1300, 1000]);
			}
		},
		onTryHit(target, source, move) {
			if (move.flags && move.flags.throw && !(move.id === 'fling')) { 
				const newMove = this.dex.getActiveMove(move.id);
			...
			if (move.id === 'fling') { // Handle Fling: catch the item if user has no item
				...
				if (sourceItem && !target.item) { 
					this.add('-activate', target, 'ability: Receiver');
	...
	shadowshield: {
		...
		onTryHit(target, source, move) { 
			if (move.flags && move.flags.shadow) { 
				this.add('-immune', target, '[from] ability: Shadow Shield');
				return null;
			}
		},
		onTrapPokemonPriority: -10,
		...
		onResidual(pokemon) {
			...
			if (pokemon.hp > pokemon.maxhp / 2) { if (pokemon.species.forme !== 'Meteor') { pokemon.formeChange('Minior-Meteor'); } }
				...
				if (pokemon.species.forme === 'Meteor') { 
					this.field.setWeather('turbulentwinds');
	...
	surgesurfer: {
		onModifySpe(spe, pokemon) {
			 ...
			 if (this.field.isTerrain('toxicterrain')) { return this.chainModify(2); } 
			},
		onModifyMove(move, pokemon) { 
			if (move.id === 'surf')  {
				this.debug('Surge Surfer boost');
				move.basePower = 105;
				if (this.field.isTerrain('electricterrain')) { move.type2 = 'Electric'; }
				if (this.field.isTerrain('grassyterrain')) { move.type2 = 'Grassy'; }
				if (this.field.isTerrain('mistyterrain')) { move.type2 = 'Misty'; }
				if (this.field.isTerrain('psychicterrain')) { move.type2 = 'Psychic'; }
				if (this.field.isTerrain('toxicterrain')) { move.type2 = 'Poison'; }
			}
		},
		flags: {},
		name: "Surge Surfer",
	...
	tanglinghair: {
		onDamagingHit(damage, target, source, move) { 
			if (this.checkMoveMakesContact(move, source, target, true)) { 
				...
				this.boost({ spe: -1 }, source, target, null, true);
                source.addVolatile('partiallytrapped', target, this.dex.conditions.get('bind'));
			}
		},
		flags: {},
		name: "Tangling Hair",
	...
	watercompaction: {
		onTryHit(target, source, move) { 
			if (move.type === 'Water' && target !== source) { 
	...
	dragonsmaw: {
		onModifyAtkPriority: 5,
		onModifyAtk(atk, attacker, defender, move) { 
			if (move.type === 'Dragon' || move.flags['bite']) { 
	...
	mirrorarmor: {
		onTryBoost(boost, target, source, effect) { // Don't bounce self stat changes, or boosts that have already bounced
			...
			for (const [b, value] of Object.entries(boost) as [BoostID, number][]) {
				if (value < 0) {
	...
	neutralizinggas: { // Ability suppression implemented in sim/pokemon.ts:Pokemon#ignoringAbility
		...
		onSwitchIn(pokemon) {
			...
			const ngState = pokemon.ability1 === 'neutralizinggas'? pokemon.abilityState1: pokemon.abilityState2;
			ngState.ending = false;
			...
			for (const target of this.getAllActive()) {
				...
				for (const slot of activeSlots) {
					...
					if (auraAbilities.includes(slot.id)) continue;
					if (slot.effect.flags['cantsuppress']) continue;
					if (slot.id === 'illusion' && target.illusion) { this.singleEvent('End', slot.effect, slot.state, target, pokemon, 'neutralizinggas'); }
					if (strongWeathers.includes(slot.id)) { this.singleEvent('End', slot.effect, slot.state, target, pokemon, 'neutralizinggas'); }
				}
		...
		onEnd(source) {
			...
			const ngState = source.ability1 === 'neutralizinggas'? source.abilityState1: source.abilityState2;
			if (ngState.ending) return;
			ngState.ending = true;
			...
			for (const pokemon of sortedActive) {
				...
				for (const slot of activeSlots) {
					...
					if (slot.id === 'gluttony') slot.state.gluttony = false;
				}
			}
		},
		flags: { failroleplay: 1, noreceiver: 1, noentrain: 1, notrace: 1, notransform: 1 },
	...
	quickdraw: {
		...
		onBasePower(basePower, attacker, defender, move) { 
			if (move.flags && move.flags.weapon) { 
				this.debug('Quick Draw weapon boost');
				return this.chainModify([1300, 1000]);
			}
		},
	...
	ripen: {
		...
		onChangeBoost(boost, target, source, effect) { if (effect && (effect as Item).isBerry) { for (const [i, value] of Object.entries(boost) as [BoostID, number][]) { boost[i] = value * 2; } } },
		onSourceModifyDamagePriority: -1,
		onSourceModifyDamage(damage, source, target, move) { 
			if (target.abilityState1.berryWeaken || target.abilityState2.berryWeaken) { 
				target.abilityState1.berryWeaken = false;
				target.abilityState2.berryWeaken = false;
				return this.chainModify(0.5);
			}
		},
		...
		onEatItem(item, pokemon) {
			...
			pokemon.abilityState1.berryWeaken = weakenBerries.includes(item.name);
			pokemon.abilityState2.berryWeaken = weakenBerries.includes(item.name); // Record if the pokemon ate a berry to resist the attack
		},
		flags: {},
		name: "Ripen",
	...
	steelyspirit: {
		...
		onResidual(pokemon) { // User and ally auras deplete at half the usual rate (0.5 per turn instead of 1)
			...
			for (const ally of team) { if (ally.status === 'aura' && ally.statusState.time !== undefined) { ally.statusState.time += 0.5; }  }
		},
		flags: {},
		name: "Steely Spirit",
	...
	unseenfist: {
		onModifyMove(move) {
			if (move.flags['punch']) {
				if (move.pierce) {
					const [num, denom] = move.pierce;
					const newNum = num * 4 + denom;
					const newDenom = denom * 4;
					move.pierce = (newNum >= newDenom) ? [1, 1] : [newNum, newDenom];
				} else {
					move.pierce = [1, 4];
				}
		...
		flags: {},
		name: "Unseen Fist",
		shortDesc: "User's Punch moves ignore protection effects, dealing 1/4 damage through them.",
		rating: 2,
		num: 260,
	},
	wanderingspirit: {
	...
	costar: {
		...
		onStart(pokemon) {
			const ally = pokemon.allies()[0];
			if (!ally) return;
			for (const i in ally.boosts) { pokemon.boosts[i as BoostID] = ally.boosts[i as BoostID]; }
			const volatilesToCopy = ['focusenergy', 'laserfocus'];
			for (const volatile of volatilesToCopy) pokemon.removeVolatile(volatile);
			for (const volatile of volatilesToCopy) { if (ally.volatiles[volatile]) { pokemon.addVolatile(volatile); } }
	...
	dragonskin: {
		onUpdate(pokemon) { 
			if (pokemon.status === 'brn' || pokemon.status === 'dragonblight') { 
				this.add('-activate', pokemon, 'ability: Dragon Skin');
				pokemon.cureStatus();
			}
		},
		onSetStatus(status, target, source, effect) {
			if (status.id !== 'brn' || status.id !== 'dragonblight') return;
			if ((effect as Move)?.status) { this.add('-immune', target, '[from] ability: Dragon Skin'); }
			return false;
		},
		onSourceModifyAtkPriority: 6,
		onSourceModifyAtk(atk, attacker, defender, move) { 
			if (move.type === 'Dragon' || move.type === 'Electric' || move.type === 'Fire' || move.type === 'Grass' || move.type === 'Water') { 
				this.debug('Dragon Skin weaken'); 
				return this.chainModify(0.5); 
			} 
		},
		onSourceModifySpAPriority: 5,
		...
		onBasePowerPriority: 23,
		onBasePower(basePower, pokemon, target, move) { if (move.typeChangerBoosted === this.effect) return this.chainModify(1.2); },
		flags: {},
		name: "Dragon Skin",
		shortDesc: "User's Normal type moves become Dragon type, and are boosted in power 1.2x. User resists Dragon, Electric, Fire, Grass, and Water type moves. Immune to Burn and Dragonblight.",
		rating: 4,
		num: 311,
	},
	eartheater: { //spikes immunity implemented in spikes move def
		...
		flags: { breakable: 1 },
		name: "Earth Eater",
		shortDesc: "Immune to Ground; When hit by a Ground type move: Heal 1/4HP. Absorb Spikes on Switch-in",
		rating: 3.5,
		num: 297,
	},
	electromorphosis: {
	...
	goodasgold: {
		...
		onSourceModifyDamage(damage, source, target, move) { 
			if (move.flags?.slicing) { 
				this.debug('Good as Gold weakness to slicing');
				return this.chainModify(2);
			}
		},
		flags: { breakable: 1 },
	...
	guarddog: {
		...
		flags: { breakable: 1 },
		name: "Guard Dog",
		shortDesc: "Immune to phasing effects; When intimidated: +1 Attack [instead of -1 Attack]. If user's Guard Action is 'Guard' upgrade it to 'Guardlv2'.  When an ally switches, but is hit by Pursuit, block it",
		rating: 2,
		num: 275,
	},
	hadronengine: {
	...
	lingeringaroma: {
		onDamagingHit(damage, target, source, move) {
			if (!this.checkMoveMakesContact(move, source, target, !source.isAlly(target))) return;
			const sourceSlots = (source as any).getActiveAbilitySlots?.() || [];
			const replaceableSlots = sourceSlots.filter((slot: any) => !slot.effect.flags['cantsuppress'] && slot.id !== 'lingeringaroma');
			if (!replaceableSlots.length) return;
			const slotToReplace = replaceableSlots[0];
			const oldAbility = source.setAbility('lingeringaroma', target, null, false, false, slotToReplace.slot);
	...
	opportunist: {
		onFoeAfterBoost(boost, target, source, effect) {
			...
			for (const [i, value] of Object.entries(boost) as [BoostID, number][]) { if (value > 0) { boostPlus[i] = (boostPlus[i] || 0) + value; } }
		},
	...
	poisonpuppeteer: {
		...
		flags: { failroleplay: 1, noentrain: 1 },
	...
	purifyingsalt: {
		onSetStatus(status, target, source, effect) { 
			if ((effect as Move)?.status) { this.add('-immune', target, '[from] ability: Purifying Salt'); }
			return false;
		},
		onTryAddVolatile(status, target) { 
			if (status.id === 'yawn') { 
	...
	rockypayload: {
		onModifyAtkPriority: 5,
		onModifyAtk(atk, attacker, defender, move) { 
			if (move.type === 'Rock' || (move.flags && move.flags.bomb)) { 
	...
	supremeoverlord: {
		...
		onBasePower(basePower, attacker, defender, move) { 
			if (this.effectState.fallen) {
				const powMod = [4096, 4301, 4505, 4710, 4915, 5120]; 
	...
	terashell: {
		...
		onAnyBasePower(basePower, attacker, defender, move) {
			const abilityHolder = this.effectState.target;
			if (defender !== abilityHolder) return;
			if (defender.hp < defender.maxhp) return;
			if (move.category === 'Status') return;
			if (!defender.teraShellUsedTypes) defender.teraShellUsedTypes = [];
			const usedTypes = defender.teraShellUsedTypes;
			const allTypes = ['Normal', 'Fire', 'Water', 'Electric', 'Grass', 'Ice', 'Fighting', 'Poison', 'Ground', 'Flying', 'Psychic', 'Bug', 'Rock', 'Ghost', 'Dragon', 'Dark', 'Steel', 'Fairy'];
			const availableTypes = allTypes.filter(type => !usedTypes.includes(type) && !defender.hasType(type));
