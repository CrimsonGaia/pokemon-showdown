...
export class PokemonSources {
	// A set of specific possible PokemonSources; implemented as an Array rather than a Set for perf reasons.
	sources: PokemonSource[];
	// if nonzero: the set also contains all possible sources from this gen and earlier.
	sourcesBefore: number;
	/**
	 * the set requires sources from this gen or later this should be unchanged from the format's minimum past gen
	 ...
	 * potential fathers need to be checked to see if they can actually learn the move combination in question.
	...
	// Moves that should be in limitedEggMoves that would otherwise be skipped because they can be learned universally in a past generation
	possiblyLimitedEggMoves?: ID[] | null;
	// Moves that should be in limitedEggMoves that would otherwise be skipped because they can be learned via Gen 1-2 tradeback
	tradebackLimitedEggMoves?: ID[] | null;
	// Tracks level up egg moves for female-only Pokemon
	levelUpEggMoves?: ID[] | null;
	// Moves that can be learned via Pomeg glitch and does not require a  particular parent to learn
	...
	add(source: PokemonSource, limitedEggMove?: ID | null) {
		...
		if (limitedEggMove) { if (source.substr(0, 3) === '1ET') { this.tradebackLimitedEggMoves = [limitedEggMove]; } }
		if (limitedEggMove && this.limitedEggMoves !== null) { this.limitedEggMoves = [limitedEggMove]; } 
		else if (limitedEggMove === null) { this.limitedEggMoves = null; }
	}
	...
	intersectWith(other: PokemonSources) {
		if (this.pomegEventEgg && other.pomegEggMoves) {
			const newSources = [];
			for (const source of other.sources) { newSources.push(source.substr(0, 2) === '3E' ? this.pomegEventEgg : source); }
			other.sources = newSources;
		} else if (other.pomegEventEgg && this.pomegEventEgg !== null) {
			const newSources = [];
			for (const source of this.sources) { newSources.push(source.substr(0, 2) === '3E' ? other.pomegEventEgg : source); }
			this.sources = newSources;
			this.pomegEventEgg = other.pomegEventEgg;
		} else if (!other.pomegEggMoves && !other.sourcesBefore) { this.pomegEventEgg = null; }
		if (other.sourcesBefore || this.sourcesBefore) {
			...
			if (other.sourcesBefore > this.sourcesBefore) {
				for (const source of this.sources) {
					const sourceGen = parseInt(source.charAt(0));
					if (sourceGen <= other.sourcesBefore) { other.sources.push(source); }
				}
			} else if (this.sourcesBefore > other.sourcesBefore) {
				for (const source of other.sources) {
					const sourceGen = parseInt(source.charAt(0));
					if (sourceGen <= this.sourcesBefore) { this.sources.push(source); }
				}
			}
		...
		if (other.restrictedMove && other.restrictedMove !== this.restrictedMove) {
			if (this.restrictedMove) { // incompatible
				this.sources = [];
				this.sourcesBefore = 0;
			} else { this.restrictedMove = other.restrictedMove; }
		}
		if (other.limitedEggMoves) {
			if (!this.limitedEggMoves) { this.limitedEggMoves = other.limitedEggMoves; } 
			else { this.limitedEggMoves.push(...other.limitedEggMoves); }
		}
		if (other.possiblyLimitedEggMoves) {
			if (!this.possiblyLimitedEggMoves) { this.possiblyLimitedEggMoves = other.possiblyLimitedEggMoves; } 
			else { this.possiblyLimitedEggMoves.push(...other.possiblyLimitedEggMoves); }
		}
		if (other.tradebackLimitedEggMoves) {
			if (!this.tradebackLimitedEggMoves) { this.tradebackLimitedEggMoves = other.tradebackLimitedEggMoves; } 
			else { this.tradebackLimitedEggMoves.push(...other.tradebackLimitedEggMoves); }
		}
		if (other.levelUpEggMoves) {
			if (!this.levelUpEggMoves) { this.levelUpEggMoves = other.levelUpEggMoves; } 
			else { this.levelUpEggMoves.push(...other.levelUpEggMoves); }
		}
		if (other.pomegEggMoves) {
			if (!this.pomegEggMoves) { this.pomegEggMoves = other.pomegEggMoves; } 
			else { this.pomegEggMoves.push(...other.pomegEggMoves); }
		}
		if (other.learnsetDomain) {
			if (!this.learnsetDomain) { this.learnsetDomain = other.learnsetDomain; } 
			else { this.learnsetDomain.filter(source => other.learnsetDomain?.includes(source)); }
		}
		if (this.possiblyLimitedEggMoves && !this.sourcesBefore) {
			...
			for (const source of eggSources) { minEggGen = Math.min(minEggGen, parseInt(source.charAt(0))); }
			if (minEggGen) {
...
export class TeamValidator {
	...
	baseValidateTeam(
		...
		for (const set of team) {
			...
			if (set.species === 'Pikachu-Starter' || set.species === 'Eevee-Starter') {
				lgpeStarterCount++;
				if (lgpeStarterCount === 2 && ruleTable.isBanned('nonexistent')) { problems.push(`You can only have one of Pikachu-Starter or Eevee-Starter on a team.`); }
			}
			if (dex.gen === 3 && set.species.startsWith('Deoxys')) {
				if (!deoxysType) { deoxysType = set.species; } 
			...
			if (options.removeNicknames) {
				...
				if (useCrossSpeciesNicknames && (crossSpecies = dex.species.get(set.name)).exists) { set.name = crossSpecies.name; } 
				else {
	...
	getEventOnlyData(species: Species, noRecurse?: boolean): { species: Species, eventData: EventInfo[] } | null {
		...
		if (!learnset.eventData && species.forme) { return this.getEventOnlyData(dex.species.get(species.baseSpecies), true); }
		if (!learnset.eventData) { throw new Error(`Event-only species ${species.name} has no eventData table`); }
	...
	getValidationSpecies(set: PokemonSet): { outOfBattleSpecies: Species, tierSpecies: Species } {
		...
		if (ability.id === 'battlebond' && toID(species.baseSpecies) === 'greninja' &&
			...
			if (ruleTable.has('obtainableformes')) { tierSpecies = outOfBattleSpecies; }
		}
	...
	validateSet(set: PokemonSet, teamHas: AnyObject): string[] | null {
		...
		if (this.gen <= 5 || ruleTable.has('obtainablemisc')) { set.gender = species.gender || set.gender; }
		...
		if (ability.id && !ability.exists) {
			if (dex.gen < 3) {
				...
				set.ability = '';
			} else { return [`"${set.ability}" is an invalid ability.`]; }
		}
		if (ability2.id && !ability2.exists) {
			...
			} else { return [`"${set.ability2}" is an invalid ability.`]; }
		}
		if (nature.id && !nature.exists) {
			if (dex.gen < 3) {
				...
				set.nature = '';
			} else { problems.push(`"${set.nature}" is an invalid nature.`); }
		}
		...
		if (set.hpType) {
			...
			if (!type.exists || ['normal', 'fairy', 'stellar'].includes(type.id)) { problems.push(`${name}'s Hidden Power type (${set.hpType}) is invalid.`); }
			else { set.hpType = type.name; }
		}
		...
		if (problem) problems.push(problem);
		if (ruleTable.has('obtainablemisc')) {
			if (dex.gen === 4 && item.id === 'griseousorb' && species.num !== 487) { problems.push(`${set.name} cannot hold the Griseous Orb.`, `(In Gen 4, only Giratina could hold the Griseous Orb).`); }
			if (dex.gen <= 1 || dex.currentMod === 'gen7letsgo') { if (item.id) { set.item = ''; }  } // no items allowed
		}
		if (!set.ability) set.ability = 'No Ability';
		if (ruleTable.has('obtainableabilities')) {
			if (dex.gen <= 2 || dex.currentMod === 'gen7letsgo') { set.ability = 'No Ability'; } 
			else {
				...
				if (ability.name === species.abilities['H']) {
					...
					if (unreleasedHidden && ruleTable.has('-unreleased')) { problems.push(`${name}'s Hidden Ability is unreleased.`); } 
					else if (dex.gen === 7 && ['entei', 'suicune', 'raikou'].includes(species.id) && this.minSourceGen > 1) { problems.push(`${name}'s Hidden Ability is only available from Virtual Console, which is not allowed in this format.`); } 
					...
					} else if (dex.gen === 5 && set.level < 10 && (species.maleOnlyHidden || species.gender === 'N')) { problems.push(`${name} must be at least level 10 to have a Hidden Ability.`); }
					if (species.maleOnlyHidden) {
						if (set.gender && set.gender !== 'M') { problems.push(`${name} must be male to have a Hidden Ability.`); }
						set.gender = 'M';
						setSources.sources = ['5D'];
					}
				} else { setSources.isHidden = false; }
			}
		}
		ability = dex.abilities.get(set.ability);
		...
		if (ruleTable.isBanned('nonexistent')) { problems.push(...this.validateStats(set, species, setSources, pokemonGoProblems)); }
		...
		for (const moveName of set.moves) {
			...
			if (problem) {
				...
				if (!allowedByOM) { problems.push(problem); } 
				else { moveLegalityWhitelist[move.id] = true; }
			}
		}
		...
		if ((!setSources.sourcesBefore && setSources.sources.length) || isUnderleveled) {
				...
				while (evoSpecies.prevo) {
					...
					if (eventData) {
						for (let eventIndex = 0; eventIndex < eventData.length; eventIndex++) {
							...
							if (eventLevel && set.level >= eventLevel) { setSources.sources.push(`${eventData[eventIndex].generation}S${eventIndex} ${evoSpecies.id}`); }
						}
					}
					if (evoSpecies.name === isUnderleveled) break;
					evoSpecies = dex.species.get(evoSpecies.prevo);
				}
			} else { checkGoLegality = true; }
			...
			if (legalSources.length) { setSources.sources = legalSources; } 
				...
				if (pokemonGoProblems?.length) {
					...
					for (const pokemonGoProblem of pokemonGoProblems) { problems.push(pokemonGoProblem); }
				}
			} else {
				...
				if (!nonEggSource) {
					// all egg moves
					if (skippedEggSource) { problems.push(`${name} is from a Gen 2 or 3 egg, which cannot be obtained at levels below 5.`); } 
					else {
					...
					if (species.id === 'mew' && pokemonGoProblems && !pokemonGoProblems.length) {
						...
						if (setSources.sources.length > 1) { problems.push(`${name} has an event-exclusive move that it doesn't qualify for (only one of several ways to get the move will be listed):`); }
			...
			if (!legal && species.gen <= 2 && dex.gen >= 7 && !this.validateSource(set, '7V', setSources, species)) { legal = true; }
			if (!legal) {
				if (!pokemonGoProblems || (pokemonGoProblems?.length)) {
					if (eventData.length === 1) { problems.push(`${species.name} is only obtainable from an event - it needs to match its event:`); } 
					else { problems.push(`${species.name} is only obtainable from events - it needs to match one of its events:`); }
				...
				} else { setSources.isFromPokemonGo = true; }
			}
		}
		...
		if (ruleTable.has('obtainablemoves') && setSources.isFromPokemonGo) {
			...
			setSources.sourcesBefore = 0;
			if (moveProblems && !moveProblems.length) { problems.push(...this.validateMoves(outOfBattleSpecies, set.moves, setSources, set, name, moveLegalityWhitelist)); } }

		if (ruleTable.has('obtainablemoves')) {
			if (species.id === 'keldeo' && set.moves.includes('secretsword') && this.minSourceGen > 5 && dex.gen <= 7) { problems.push(`${name} has Secret Sword, which is only compatible with Keldeo-Ordinary obtained from Gen 5.`); }
			const requiresGen3Source = setSources.maxSourceGen() <= 3;
			if (requiresGen3Source && dex.abilities.get(set.ability).gen === 4 && !species.prevo && dex.gen <= 5) { problems.push(`${name} has a Gen 4 ability and isn't evolved - it can't use moves from Gen 3.`); } // Ability Capsule allows this in Gen 6+
			const canUseAbilityPatch = dex.gen >= 8 && format.mod !== 'gen8dlc1';
			if (setSources.isHidden && !canUseAbilityPatch && setSources.maxSourceGen() < 5) { problems.push(`${name} has a Hidden Ability - it can't use moves from before Gen 5.`); }
			if (
		...
		if (!problems.length) {
			if (this.gen > 5 && !ruleTable.has('obtainablemisc')) { set.gender ||= 'N'; }
	...
	validateStats(set: PokemonSet, species: Species, setSources: PokemonSources, pokemonGoProblems: string[] | null) {
		...
		for (const moveName of set.moves) {
			...
			if (move.id === 'hiddenpower' && move.type !== 'Normal') {
				...
				else if (set.hpType !== move.type && ruleTable.has('obtainablemisc')) { problems.push(`${name}'s Hidden Power type ${set.hpType} is incompatible with Hidden Power ${move.type}`); }
			}
		}
		if (set.hpType && maxedIVs && ruleTable.has('obtainablemisc')) {
			if (dex.gen <= 2) {
				...
				let statName: StatID;
				for (statName in HPdvs) { set.ivs[statName] = HPdvs[statName]! * 2; }
				set.ivs.hp = -1;
			} else if (!canBottleCap) { set.ivs = TeamValidator.fillStats(dex.types.get(set.hpType).HPivs, 31); }
		}
		if (!set.hpType && set.moves.some(m => dex.moves.get(m).id === 'hiddenpower')) { set.hpType = dex.getHiddenPower(set.ivs).type; }
		...
		if (set.hpType === 'Fighting' && ruleTable.has('obtainablemisc')) { if (has3PerfectIVs) { problems.push(`${name} must not have Hidden Power Fighting because it starts with 3 perfect IVs because it's a Gen 6+ legendary.`); } }
		if (has3PerfectIVs && ruleTable.has('obtainablemisc')) {
			let perfectIVs = 0;
			for (const stat in set.ivs) { if (set.ivs[stat as 'hp'] >= 31) perfectIVs++; }
			if (perfectIVs < 3) {
				if (!pokemonGoProblems || pokemonGoProblems?.length) {
		...
		if (set.hpType && !canBottleCap) {
			...
			if (set.hpType !== ivHpType) { problems.push(`${name} has Hidden Power ${set.hpType}, but its IVs are for Hidden Power ${ivHpType}.`); }
		} else if (set.hpType) { if (!this.possibleBottleCapHpType(set.hpType, set.ivs)) { problems.push(`${name} has Hidden Power ${set.hpType}, but its IVs don't allow this even with (Bottle Cap) Hyper Training.`); } }
		if (dex.gen <= 2) {
			// validate DVs
			const ivs = set.ivs;
			...
			if (expectedHpDV !== hpDV) { problems.push(`${name} has an HP DV of ${hpDV}, but its Atk, Def, Spe, and Spc DVs give it an HP DV of ${expectedHpDV}.`); }
			if (ivs.spa !== ivs.spd) {
				if (dex.gen === 2) { problems.push(`${name} has different SpA and SpD DVs, which is not possible in Gen 2.`); } 
				else { ivs.spd = ivs.spa; }
			}
			if (dex.gen > 1 && !species.gender) {
				...
				if (set.gender && set.gender !== expectedGender) { problems.push(`${name} is ${set.gender}, but it has an Atk DV of ${atkDV}, which makes its gender ${expectedGender}.`); } 
				else { set.gender = expectedGender; }
			}
			if (
				...
				while (set.evs.atk > 0 && 2 * 80 + set.ivs.atk + Math.floor(set.evs.atk / 4) + 5 > 255) { set.evs.atk -= 4; }
			}
			if (dex.gen > 1) {
				const expectedShiny = !!(defDV === 10 && speDV === 10 && spcDV === 10 && atkDV % 4 >= 2);
				if (expectedShiny && !set.shiny) { problems.push(`${name} is not shiny, which does not match its DVs.`); } 
				else if (!expectedShiny && set.shiny) { problems.push(`${name} is shiny, which does not match its DVs (its DVs must all be 10, except Atk which must be 2, 3, 6, 7, 10, 11, 14, or 15).`); }
			}
			set.nature = 'Serious';
		}
		for (const stat in set.evs) { if (set.evs[stat as 'hp'] < 0) { problems.push(`${name} has less than 0 ${allowAVs ? 'Awakening Values' : 'EVs'} in ${Dex.stats.names[stat as 'hp']}.`); } }
		if (dex.currentMod === 'gen7letsgo') { // AVs
			for (const stat in set.evs) {
				if (set.evs[stat as 'hp'] > 0 && !allowAVs) {
					...
					break;
				} else if (set.evs[stat as 'hp'] > 200) { problems.push(`${name} has more than 200 Awakening Values in ${Dex.stats.names[stat as 'hp']}.`); }
			}
		} else { // EVs
			for (const stat in set.evs) { if (set.evs[stat as StatID] > 255) { problems.push(`${name} has more than 255 EVs in ${Dex.stats.names[stat as 'hp']}.`); } }
			if (dex.gen <= 2) {
				if (set.evs.spa !== set.evs.spd) {
					if (dex.gen === 2) { problems.push(`${name} has different SpA and SpD EVs, which is not possible in Gen 2.`); } 
					else { set.evs.spd = set.evs.spa; }
				}
			}
		}
		let totalEV = 0;
		...
		if (!this.format.debug) {
			if (set.level > 1 && evLimit !== 0 && totalEV === 0) { problems.push(`${name} has exactly 0 EVs - did you forget to EV it? (If this was intentional, add exactly 1 to one of your EVs, which won't change its stats but will tell us that it wasn't a mistake).`); } 
			else if (![508, 510].includes(evLimit!) && [508, 510].includes(totalEV)) { problems.push(`${name} has exactly ${totalEV} EVs, but this format does not restrict you to 510 EVs (If this was intentional, add exactly 1 to one of your EVs, which won't change its stats but will tell us that it wasn't a mistake).`); }
			// Check for level import errors from user in VGC -> DOU, etc. Note that in VGC etc (Adjust Level Down = 50), `set.level` will be 100 here for validation purposes
			if (set.level === 50 && ruleTable.maxLevel !== 50 && !ruleTable.maxTotalLevel && evLimit !== 0 && totalEV % 4 === 0) { problems.push(`${name} is level 50, but this format allows level ${ruleTable.maxLevel} Pokémon. (If this was intentional, add exactly 1 to one of your EVs, which won't change its stats but will tell us that it wasn't a mistake).`); }
		}
		if (evLimit !== null && totalEV > evLimit) {
			if (!evLimit) { problems.push(`${name} has EVs, which is not allowed by this format.`); } 
			else { problems.push(`${name} has ${totalEV} total EVs, which is more than this format's limit of ${evLimit}.`); }
		}
		return problems;
	}
	// Not exhaustive, just checks Atk and Spe, which are the only competitively relevant IVs outside of extremely obscure situations.
	possibleBottleCapHpType(type: string, ivs: StatsTable) {
		if (!type) return true;
		if (['Dark', 'Dragon', 'Grass', 'Ghost', 'Poison'].includes(type)) { if (ivs.spe % 2 === 0) return false; } // Spe must be odd
		if (['Psychic', 'Fire', 'Rock', 'Fighting'].includes(type)) { if (ivs.spe !== 31 && ivs.spe % 2 === 1) return false; } // Spe must be even
		if (type === 'Dark') { if (ivs.atk % 2 === 0) return false; } // Atk must be odd
		if (['Ice', 'Water'].includes(type)) { if (ivs.spe % 2 === 0 && ivs.atk % 2 === 0) return false; } // Spe or Atk must be odd
		return true;
	}
	 ...
	 */
	validateSource(set: PokemonSet, source: PokemonSource, setSources: PokemonSources, species: Species, because?: string) {
		...
		if (source.charAt(1) === 'S') {
			...
			if (this.findEggMoveFathers(source, species, setSources)) { return undefined; }
	...
	findEggMoveFathers(source: PokemonSource, species: Species, setSources: PokemonSources,
		...
		if (!eggMoves) { return getAll ? ['*'] : true; } // happens often in gen 1-6 LC if your only egg moves are level-up moves, which aren't limited and so aren't in `limitedEggMoves`
		...
		if (species.id === 'nidoqueen' || species.id === 'nidorina') { eggGroups = dex.species.get('nidoranf').eggGroups; } 
		else if (species.id === 'shedinja') { eggGroups = dex.species.get('nincada').eggGroups; } // Shedinja and Nincada are different Egg groups; Shedinja itself is genderless
		else if (dex !== this.dex) { eggGroups = dex.species.get(species.id).eggGroups; } // Gen 1 tradeback; grab the egg groups from Gen 2
		if (eggGroups[0] === 'Undiscovered') eggGroups = dex.species.get(species.evos[0]).eggGroups;
		if (eggGroups[0] === 'Undiscovered' || !eggGroups.length) { throw new Error(`${species.name} has no egg groups for source ${source}`); }
	...
	/**
	 * We could, if we wanted, do a complete move validation of the father's moveset to see if it's valid. This would recurse and be NP-Hard so instead we won't. 
	 * We'll instead use a simplified algorithm: The father is allowed to have multiple egg moves and a maximum of one move from any other restrictive source; recursion is done only if there are less
	 * egg moves to validate or if the father has an egg group it doesn't share with the egg Pokemon. Recursion is also limited to two iterations of calling findEggMoveFathers.
	 */
	fatherCanLearn(baseSpecies: Species, species: Species, moves: ID[], eggGen: number, pokemonBlacklist: ID[],
		...
		for (const move of moves) {
				...
				if (learnset[move]) {
					for (const moveSource of learnset[move]) {
						...
						if (!'ESDV'.includes(moveSource.charAt(1)) || canLearnFromSmeargle) {
							...
							} else { eggSources.add(moveSource + eggPokemon); }
						}
					}
				}
			...
			if (eggGen >= 3 && onlyEggSources.length && eggSources.limitedEggMoves === null && eggSources.sourcesBefore) { eggSources.possiblyLimitedEggMoves = [toID(`${eggSources.sourcesBefore}${move}`)]; }
	...
	motherCanLearn(species: ID, move: ID) {
		...
		for (const { learnset } of fullLearnset) { if (learnset[move]) return true; }
		return false;
	}
	validateForme(set: PokemonSet) {
		...
		if (species.name === 'Keldeo' && set.moves.map(toID).includes('secretsword' as ID) && dex.gen >= 8) { set.species = 'Keldeo-Resolute'; }
		...
		if (species.baseSpecies === "Hoopa" && dex.gen >= 9) {
			...
			if (species.name === "Hoopa" && hyperspaceFury >= 0) { problems.push(`In Generation 9, Hoopa cannot run Hyperspace Fury because it gets replaced with Hyperspace Hole upon changing forme.`); } 
			else if (species.name === "Hoopa-Unbound" && hyperspaceHole >= 0) { problems.push(`In Generation 9, Hoopa-Unbound cannot run Hyperspace Hole because it gets replaced with Hyperspace Fury upon changing forme.`); }
		}
		if (species.baseSpecies === "Greninja" && toID(set.ability) === 'battlebond') { set.species = "Greninja-Bond"; }
		if (species.baseSpecies === "Rockruff" && toID(set.ability) === 'owntempo') { set.species = "Rockruff-Dusk"; }
		if (species.baseSpecies === "Unown" && dex.gen === 2) {
			let resultBinary = '';
			for (const iv of ['atk', 'def', 'spe', 'spa'] as const) { resultBinary += set.ivs[iv].toString(2).padStart(5, '0').slice(1, 3); }
			...
			if (unownLetter !== expectedLetter) { problems.push(`Unown has forme ${unownLetter}, but its DVs give it the forme ${expectedLetter}.`); }
		}
		return problems;
	}
	checkSpecies(set: PokemonSet, species: Species, tierSpecies: Species, setHas: { [k: string]: true }) {
		...
		if (tierSpecies.baseSpecies === 'Greninja' && toID(set.ability) === 'battlebond') { setHas['pokemon:greninjabond'] = true; }
		if (tierSpecies.baseSpecies === 'Rockruff' && toID(set.ability) === 'owntempo') { setHas['pokemon:rockruffdusk'] = true; }
		...
		if (!tierSpecies.canGigantamax && set.gigantamax) { return `${tierSpecies.name} cannot Gigantamax but is flagged as being able to.`; }
		...
		if (isGmax) {
			...
			if (banReason) { return `Gigantamaxing ${species.name} is ${banReason}.`; }
		}
		...
		if (nonexistentCheck) {
			...
			if (tierSpecies.isNonstandard === 'LGPE') { return `${tierSpecies.name} does not exist in this game, only in Let's Go Pikachu/Eevee.`; }
			if (tierSpecies.isNonstandard === 'CAP') { return `${tierSpecies.name} is a CAP and does not exist in this game.`; }
			if (tierSpecies.isNonstandard === 'Unobtainable') { return `${tierSpecies.name} is not possible to obtain in this game.`; }
			if (tierSpecies.isNonstandard === 'Gigantamax') { return `${tierSpecies.name} is a placeholder for a Gigantamax sprite, not a real Pokémon. (This message is likely to be a validator bug.)`; }
		...
		if (tierSpecies.gmaxUnreleased && set.gigantamax) {
			...
			if (banReason) { return `${tierSpecies.name} is flagged as gigantamax, but it cannot gigantamax without hacking or glitches.`; }
			if (banReason === '') return null;
		}
	...
	checkMove(set: PokemonSet, move: Move, setHas: { [k: string]: true }) {
		...
		if (move.isNonstandard) {
			...
			if (banReason) {
				if (move.isNonstandard === 'Unobtainable') { return `${move.name} is not obtainable without hacking or glitches${dex.gen >= 9 && move.gen < dex.gen ? ` in Gen ${dex.gen}` : ``}.`; }
				if (move.isNonstandard === 'Gigantamax') { return `${move.name} is not usable without Gigantamaxing its user, ${move.isMax}.`; }
	...
	checkAbility(set: PokemonSet, ability: Ability, setHas: { [k: string]: true }) {
		...
		if (this.format.id.startsWith('gen9pokebilities')) {
			...
			if (ability.id !== this.toID(species.abilities['S'])) { for (const abilityName of unSeenAbilities) { setHas['ability:' + toID(abilityName)] = true; } }
		}
	...
	validateEvent(
		...
		} else { requiredIVs = eventData.perfectIVs || 0; }
		if (requiredIVs && set.ivs) {
			// Legendary Pokemon must have at least 3 perfect IVs in gen 6
			// Events can also have a certain amount of guaranteed perfect IVs
			let perfectIVs = 0;
			let statName: StatID;
			for (statName in set.ivs) { if (set.ivs[statName] >= 31) perfectIVs++; }
			if (perfectIVs < requiredIVs) {
				if (fastReturn) return true;
				if (eventData.perfectIVs) { problems.push(`${name} must have at least ${requiredIVs} perfect IVs${etc}.`); }
			}
		...
		if (ruleTable.has('obtainablemoves')) {
			...
			if (eventData.from === "Gen 5 Dream World" && setSources.dreamWorldMoveCount > 1) { problems.push(`${name} can only have one Dream World move.`); }
		}
		if (ruleTable.has('obtainableabilities')) {
	...
	validateMoves(
		...
		if (setSources.size() && setSources.moveEvoCarryCount > 3) {
			...
			if (!setSources.size()) { problems.push(`${name} needs to know ${species.evoMove || 'a Fairy-type move'} to evolve, so it can only know 3 other moves from ${species.prevo}.`); }
		}
		if (problems.length) return problems;
		...
		if (setSources.babyOnly && setSources.sources.length) {
			...
			setSources.sources = setSources.sources.filter(source => {
				...
				if (source.charAt(1) === 'E') { if (babyEvo && source.slice(2) === babyEvo) return false; }
				if (source.charAt(1) === 'D') { if (babyEvo && source.slice(2) === babyEvo) return false; }
				return true;
			});
			if (!setSources.size()) { problems.push(`${name}'s event/egg moves are from an evolution, and are incompatible with its moves from ${baby.name}.`); }
		}
		if (setSources.babyOnly && setSources.size() && this.gen > 2) {
			// there do theoretically exist evo/tradeback incompatibilities in gen 2, but those are very complicated to validate and should be
			// handled separately anyway, so for now we just treat them all as legal (competitively relevant ones can be manually banned)
			const baby = dex.species.get(setSources.babyOnly);
			setSources.sources = setSources.sources.filter(source => {
			...
			if (!setSources.size()) { problems.push(`${name} has moves from before Gen ${baby.gen}, which are incompatible with its moves from ${baby.name}.`); }
		}
		return problems;
	}
	/**
	 ...
	 */
	validatePokemonGo(species: Species, set: PokemonSet, setSources: PokemonSources, name: string = species.name,): string[] | null {
		let problems = [];
		...
		if (!pokemonGoData) {
			...
			if (otherSpecies && !species.evoLevel) {
				...
				if (otherProblems) { problems = otherProblems; } 
				else { return null; }
			} else { return null; }
		} else {
			...
			if (set.shiny && pokemonGoSources.includes('noshiny')) { problems.push(`${name} is not obtainable from Pokemon GO.`); } 
			else {
				...
				if (species.id === 'mewtwo' && set.level >= 20) { minIVs = Math.min(minIVs, 0); } // A bug allowed Mewtwo to be encountered with an IV floor of 0 from GO Battle League
				if (pokemonGoSources.includes('research')) {
					if (species.id === 'cresselia') { minLevel = Math.min(minLevel, 10); } // For some reason, Cresselia from research has a possible level 10 source
					else { minLevel = Math.min(minLevel, 15); }
					minIVs = Math.min(minIVs, 10);
				}
				if (pokemonGoSources.includes('giovanni') && !set.shiny) {
					// Purified Pokemon can be leveled down to level 8 after trading; they are forced to special trades, but currently all Giovanni Shadow Pokemon are already forced special trades
					minLevel = Math.min(minLevel, 8);
				...
				if (set.level < minLevel) { problems.push(`${name} must be at least level ${minLevel} to be from Pokemon GO.`); }
				...
				if (IVsTooLow) { problems.push(`${name} must have at least ${postTransferMinIVs} ` + (postTransferMinIVs === 1 ? `IV` : `IVs`) + ` in non-Speed stats to be from Pokemon GO.`); }
				if (hasEvenIVs) { problems.push(`${name} must have odd non-Speed IVs to be from Pokemon GO.`); }
				const canBottleCap = dex.gen >= 7 && set.level >= (dex.gen < 9 ? 100 : 50);
				if (ivs.atk !== ivs.spa && !(canBottleCap && (ivs.atk === 31 || ivs.spa === 31))) { problems.push(`${name}'s Atk and Sp. Atk IVs must match to be from Pokemon GO.`); }
				if (ivs.def !== ivs.spd && !(canBottleCap && (ivs.def === 31 || ivs.spd === 31))) { problems.push(`${name}'s Def and Sp. Def IVs must match to be from Pokemon GO.`); }
			}
		}
		return problems;
	}
	...
	checkCanLearn(
		...
		// Abstractly, what it does is construct the union of sets of all possible ways this pokemon could be obtained, and then intersect
		// it with a the pokemon's existing set of all possible ways it could be obtained. If this intersection is non-empty, the move is legal.
		...
		for (const { species, learnset } of fullLearnset) {
			...
			if (checkingPrevo && !moveSources.size()) { if (!setSources.babyOnly || !species.prevo) { babyOnly = species.id; } }
			...
			if (moveid === 'sketch') { sketch = true; } 
				...
				if (move.flags['nosketch'] || move.isZ || move.isMax) { cantLearnReason = `can't be Sketched.`; } 
			...
			for (let learned of sources) {
				...
				if (checkingPrevo && learned.charAt(1) === 'X') continue; // IS: Prevo-exclusive moves cannot be inherited by evolutions
				if (setSources.learnsetDomain && !setSources.learnsetDomain.includes(`${learnedGen}${toID(species.baseSpecies)}`) && (learned.charAt(1) !== 'E' || learnedGen < 8)) 
				{
					if (!cantLearnReason) { cantLearnReason = `is incompatible with ${(setSources.restrictiveMoves || []).join(', ')}.`; }
					continue;
				}
				if (learnedGen < this.minSourceGen && !canUseHomeRelearner) {
					if (!cantLearnReason) { cantLearnReason = `can't be transferred from Gen ${learnedGen} to ${this.minSourceGen}.`; }
					continue;
				}
				if (noFutureGen && learnedGen > dex.gen) {
					if (!cantLearnReason) { cantLearnReason = `can't be transferred from Gen ${learnedGen} to ${dex.gen}.`; }
					continue;
				}
				...
				const ability = dex.abilities.get(set.ability);
				if (dex.gen < 6 && ability.gen > learnedGen && !checkingPrevo) { // You can evolve a transferred mon to reroll for its new Ability.
					cantLearnReason = `is learned in gen ${learnedGen}, but the Ability ${ability.name} did not exist then.`;
					continue;
				}
				if (species.isNonstandard !== 'CAP') { // HMs can't be transferred
					if (dex.gen >= 4 && learnedGen <= 3 && ['cut', 'fly', 'surf', 'strength', 'flash', 'rocksmash', 'waterfall', 'dive',].includes(moveid)) {
						cantLearnReason = `can't be transferred from Gen 3 to 4 because it's an HM move.`;
						continue;
					}
					if (dex.gen >= 5 && learnedGen <= 4 && ['cut', 'fly', 'surf', 'strength', 'rocksmash', 'waterfall', 'rockclimb',].includes(moveid)) {
				...
				if (learned.charAt(1) === 'L') {
					...
					} else if (level >= 5 && learnedGen === 3 && species.canHatch) { learned = `${learnedGen}Epomeg` as MoveSource; } // Pomeg Glitch
					...
					} else { // this move is unavailable, skip it
						cantLearnReason = `is learned at level ${parseInt(learned.substr(2))}.`;
						continue;
					}
				}
				...
				if (learnedGen >= 8 && learned.charAt(1) === 'E' && learned.slice(1) !== 'Eany' && learned.slice(1) !== 'Epomeg' || 'LMTR'.includes(learned.charAt(1))) {
					if (learnedGen === dex.gen && learned.charAt(1) !== 'R') {
						// current-gen level-up, TM or tutor moves: always available
					...
					// egg moves: only if hatched from an egg
					...
					if (learned.slice(1) === 'Eany') {
						...
						} else { limitedEggMove = null; }
					} else if (learned.slice(1) === 'Epomeg') {
						// Pomeg glitched moves have to be from an egg but since they aren't true egg moves, there should be no breeding restrictions
						moveSources.pomegEggMoves = [move.id];
					} else if (learnedGen < 6 || (species.mother && !this.motherCanLearn(toID(species.mother), moveid))) { limitedEggMove = move.id; }
					learned = `${learnedGen}E${species.prevo ? species.id : ''}` as MoveSource;
					if (tradebackEligible && learnedGen === 2 && move.gen <= 1) { moveSources.add(`1ET${learned.slice(2)}`, limitedEggMove); } // can tradeback
					moveSources.add(learned, limitedEggMove);
				} else if (learned.charAt(1) === 'S') {
					// event moves: only if that was the source
					// Event Pokémon: Available as long as the past gen can get the Pokémon and then trade it back.
					if (tradebackEligible && learnedGen === 2 && move.gen <= 1) { moveSources.add(`1ST${learned.slice(2)} ${species.id}`); } // can tradeback
					moveSources.add(`${learned} ${species.id}`);
					const eventLearnset = dex.species.getLearnsetData(species.id);
					if (eventLearnset.eventData?.[parseInt(learned.charAt(2))].emeraldEventEgg && learnedGen === 3) { moveSources.pomegEventEgg = `${learned} ${species.id}`; }
				} else if (learned.charAt(1) === 'D') {
					// DW moves: only if that was the source
					moveSources.add(`${learned}${species.id}`);
				...
				if (learned.charAt(1) === 'E' && learnedGen >= 8 && !canLearnSpecies.includes(toID(baseSpecies.baseSpecies))) { canLearnSpecies.push(toID(baseSpecies.baseSpecies)); }
			...
			if (canUseHomeRelearner) {
				...
				if (!['nincada', 'spinda'].includes(species.id) && learnsetData?.learnset?.[move.id]) { fullSources.push(...learnsetData.learnset[move.id]); }
				learnsetData = this.getExternalLearnsetData(species.id, 'gen8legends');
				if (learnsetData?.learnset?.[move.id]) { fullSources.push(...learnsetData.learnset[move.id]); }
			...
			if (ruleTable.has('mimicglitch') && species.gen < 5) {
				...
				if (getGlitch) {
					...
					if (move.gen < 5) { limit1 = false; }
				}
			}
			if (!moveSources.size()) { if ((species.evoType === 'levelMove' && species.evoMove !== move.name) || (species.id === 'sylveon' && move.type !== 'Fairy')) { moveSources.moveEvoCarryCount = 1; } }
		}
		if (limit1 && sketch) { // limit 1 sketch move
			if (setSources.sketchMove) { return ` can't Sketch ${move.name} and ${setSources.sketchMove} because it can only Sketch 1 move.`; }
			setSources.sketchMove = move.name;
		}
		if (blockedHM) { // Limit one of Defog/Whirlpool to be transferred
			if (setSources.hm) return ` can't simultaneously transfer Defog and Whirlpool from Gen 4 to 5.`;
			setSources.hm = moveid;
		}
		if (!setSources.restrictiveMoves) { setSources.restrictiveMoves = []; }
		if (!setSources.restrictiveMoves.includes(move.name)) { setSources.restrictiveMoves.push(move.name); }
		...
		if (checkedSpecies && setSources.isFromPokemonGo &&
			...
			if (pokemonGoData?.LGPERestrictiveMoves) {
				...
				for (const moveName of setSources.restrictiveMoves) { restrictiveMovesToID.push(toID(moveName)); }
				for (const restrictiveMove in pokemonGoData.LGPERestrictiveMoves) {
					...
					if (toID(move) === restrictiveMove) {
						if (!moveLevel) { return `'s move ${move.name} is incompatible with its Pokemon GO origin.`; } 
						else if (set.level && set.level < moveLevel) { return ` must be at least level ${moveLevel} to learn ${move.name} due to its Pokemon GO origin.`; }
					}
					if (levelUpMoveCount) levelUpMoveCount++;
					if (restrictiveMovesToID.includes(restrictiveMove)) {
						if (!levelUpMoveCount) { levelUpMoveCount++; } 
						else if (levelUpMoveCount > 4) { return `'s moves ${(setSources.restrictiveMoves || []).join(', ')} are incompatible with its Pokemon GO origin.`; }
					}
				}
			}
		}
		...
		if (!tradebackEligible) {
			if (!dex.species.getLearnsetData(nextSpecies.id).learnset) { nextSpecies = dex.species.get(nextSpecies.changesFrom || nextSpecies.baseSpecies); }
			while (nextSpecies) {
		...
		if (dex.gen >= 3 && eggSources.length && moveSources.limitedEggMoves === null && moveSources.sourcesBefore) { moveSources.possiblyLimitedEggMoves = [toID(`${moveSources.sourcesBefore}${move.id}`)]; }
