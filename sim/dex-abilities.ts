...
export class Ability extends BasicEffect implements Readonly<BasicEffect> {
	...
	constructor(data: AnyObject) {
		...
		if (!this.gen) {
			if (this.num >= 268) { this.gen = 9; } 
			else if (this.num >= 234) { this.gen = 8; } 
...
export class DexAbilities {
	...
	getByID(id: ID): Ability {
		...
		if (this.dex.getAlias(id)) { ability = this.get(this.dex.getAlias(id)); } 
	...
	all(): readonly Ability[] {
		...
		const abilities = [];
		for (const id in this.dex.data.Abilities) { abilities.push(this.getByID(id as ID)); }
