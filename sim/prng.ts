...
export class PRNG {
	...
	constructor(seed: PRNGSeed | null = null, initialSeed?: PRNGSeed) {
		...
		if (Array.isArray(seed)) { seed = seed.join(',') as PRNGSeed; } // compat for old inputlogs
		if (typeof seed !== 'string') { throw new Error(`PRNG: Seed ${seed} must be a string`); }
		this.startingSeed = initialSeed ?? seed;
		this.setSeed(seed);
	}
	setSeed(seed: PRNGSeed) {
		if (seed.startsWith('sodium,')) { this.rng = new SodiumRNG(seed.split(',') as SodiumRNGSeed); } 
		...
		} else if (/[0-9]/.test(seed.charAt(0))) { this.rng = new Gen5RNG(seed.split(',').map(Number) as Gen5RNGSeed); } 
		else { throw new Error(`Unrecognized RNG seed ${seed}`); }
	}
	getSeed(): PRNGSeed { return this.rng.getSeed(); }
	/**
	 * Creates a clone of the current PRNG.
	 * The new PRNG will have its initial seed set to the seed of the current instance.
	 */
	clone(): PRNG { return new PRNG(this.rng.getSeed(), this.startingSeed); }
	/**
	...
	random(from?: number, to?: number): number {
		...
		else if (!to) { return Math.floor(result * from / 2 ** 32); } 
		else { return Math.floor(result * (to - from) / 2 ** 32) + from; }
	}
	/**
	 ...
	 */
	randomChance(numerator: number, denominator: number): boolean { return this.random(denominator) < numerator; }
	/**
	...
	sample<T>(items: readonly T[]): T {
		...
		if (item === undefined && !Object.prototype.hasOwnProperty.call(items, index)) { throw new RangeError(`Cannot sample a sparse array`); }
		return item;
	}
	/**
	...
	static generateSeed(): PRNGSeed { return PRNG.convertSeed(SodiumRNG.generateSeed()); }
	static convertSeed(seed: SodiumRNGSeed | Gen5RNGSeed): PRNGSeed { return seed.join(',') as PRNGSeed; }
	static get(prng?: PRNG | PRNGSeed | null) { return prng && typeof prng !== 'string' && !Array.isArray(prng) ? prng : new PRNG(prng as PRNGSeed); }
}
/**
...
export class SodiumRNG implements RNG {
	...
	seed!: Uint8Array;
	/** Creates a new source of randomness for the given seed. */
	constructor(seed: SodiumRNGSeed) { this.setSeed(seed); }
	...
	getSeed(): PRNGSeed { return `sodium,${Utils.bufReadHex(this.seed)}`; }
	next() {
...
export class Gen5RNG implements RNG {
	seed: Gen5RNGSeed;
	/** Creates a new source of randomness for the given seed. */
	constructor(seed: Gen5RNGSeed | null = null) { this.seed = [...seed || Gen5RNG.generateSeed()]; }
	getSeed(): PRNGSeed { return this.seed.join(',') as PRNGSeed; }
	next(): number {
	...
	nextFrame(seed: Gen5RNGSeed, framesToAdvance = 1): Gen5RNGSeed {
		...
		for (let i = 0; i < framesToAdvance; i++) { seed = this.multiplyAdd(seed, a, c); } // seed = seed * a + c
		return seed;
	}
...
export class Gen3RNG implements RNG {
	seed: number;
	constructor(seed: Gen3RNGSeed | null = null) { this.seed = seed ? seed[1] : Math.trunc(Math.random() * 2 ** 32); }
	getSeed() { return ['gen3', this.seed]; }
	next(): number {
