...
export class Battle {
	...
	// The number of active pokemon per half-field. See header comment in side.ts for details.
	...
	queue: BattleQueue;
	readonly faintQueue: { target: Pokemon, source: Pokemon | null, effect: Effect | null, }[];
	readonly log: string[];
	...
	constructor(options: BattleOptions) {
		...
		this.speedOrder = [];
		for (let i = 0; i < this.activePerHalf * 2; i++) { this.speedOrder.push(i); }
		...
		const inputOptions: { formatid: ID, seed: PRNGSeed, rated?: string | true } = { formatid: options.formatid, seed: this.prngSeed, };
		if (this.rated) inputOptions.rated = this.rated;
		if (typeof __version !== 'undefined') {
			if (__version.head) { this.inputLog.push(`>version ${__version.head}`); }
			if (__version.origin) { this.inputLog.push(`>version-origin ${__version.origin}`); }
		}
		...
		for (const rule of this.ruleTable.keys()) {
			...
			if (subFormat.exists) {
				const hasEventHandler = Object.keys(subFormat).some( val => val.startsWith('on') && !['onBegin', 'onTeamPreview', 'onBattleStart', 'onValidateRule', 'onValidateTeam', 'onChangeSet', 'onValidateSet',].includes(val) );
				if (hasEventHandler) this.field.addPseudoWeather(rule);
			}
		}
		const sides: SideID[] = ['p1', 'p2', 'p3', 'p4'];
		for (const side of sides) { if (options[side]) { this.setPlayer(side, options[side]); } }
	}
	toJSON(): AnyObject { return State.serializeBattle(this); }
	static fromJSON(serialized: string | AnyObject): Battle { return State.deserializeBattle(serialized); }
	get p1() { return this.sides[0]; }
	get p2() { return this.sides[1]; }
	get p3() { return this.sides[2]; }
	get p4() { return this.sides[3]; }
	toString() { return `Battle: ${this.format}`;}
	random(m?: number, n?: number) { return this.prng.random(m, n); }
	randomChance(numerator: number, denominator: number) {
		if (this.forceRandomChance !== null) return this.forceRandomChance;
		return this.prng.randomChance(numerator, denominator);
	}
	sample<T>(items: readonly T[]): T { return this.prng.sample(items); }
	...
	suppressingAbility(target?: Pokemon) { return this.activePokemon && this.activePokemon.isActive && (this.activePokemon !== target || this.gen < 8) && this.activeMove && this.activeMove.ignoreAbility && !target?.hasItem('Ability Shield'); }
	...
	updateSpeed() { for (const pokemon of this.getAllActive()) { pokemon.updateSpeed(); } }
	/**
	...
	speedSort<T extends AnyObject>(list: T[], comparator: (a: T, b: T) => number = this.comparePriority) {
		...
		let sorted = 0;
		// This is a Selection Sort - not the fastest sort in general, but actually faster than QuickSort for small arrays like the ones `speedSort` is used for.
		...
		while (sorted + 1 < list.length) {
			...
			for (let i = 0; i < nextIndexes.length; i++) {
				...
				if (index !== sorted + i) {[list[sorted + i], list[index]] = [list[index], list[sorted + i]];}
			}
	 ...
	 * Unlike `eachEvent`, this contains a lot of other handling and is only intended for the 'Residual' and 'SwitchIn' events.
	 */
	fieldEvent(eventid: string, targets?: Pokemon[]) {
		...
		while (handlers.length) {
			...
			if ((handler.effectHolder as Pokemon).fainted) { if (!(handler.state?.isSlotCondition)) continue; }
			// Time Break: pause anything with its own per-turn Residual effect, or a genuine multi-turn duration counter, while it's active
			// except Timebreak's own field handler. Single-tick bookkeeping volatiles 
			// (duration: 1, no onResidual of their own — flinch, tripped, slip, magicdust) are exempt
			// those always resolve at the end of the turn they were granted regardless of "narrative" time passing, 
			// so pausing them would strand a Pokemon mid-flinch for the rest of the battle.
			const isSingleTickExpiry = handler.state?.duration === 1 && !handler.callback;
			if (eventid === 'Residual' && effect.id !== 'timebreak' && !isSingleTickExpiry && this.field.getPseudoWeather('timebreak')) { continue; }
			...
			if (handler.state?.target instanceof Pokemon) {
				if (effect.effectType === 'Ability' && !handler.state.id.startsWith('ability:')) {
					...
					if (target.abilityState1 !== handler.state && target.abilityState2 !== handler.state) { continue; }
				} else {
					let expectedStateLocation;
					if (effect.effectType === 'Item' && !handler.state.id.startsWith('item:')) { expectedStateLocation = handler.state.target.itemState; } 
				...
				let expectedStateLocation;
				if (effect.effectType === 'Weather') { expectedStateLocation = handler.state.target.weatherState; } 
				else if (effect.effectType === 'Terrain') { expectedStateLocation = handler.state.target.terrainState; } 
				else { expectedStateLocation = handler.state.target.pseudoWeather[effect.id]; }
				if (expectedStateLocation !== handler.state) { continue; }
			}
			...
			if (handler.callback) { this.singleEvent(handlerEventid, effect, handler.state, handler.effectHolder, null, null, undefined, handler.callback); }
			this.faintMessages();
			if (this.ended) return;
		}
	}
	...
	singleEvent(
		...
		if (effect.effectType === 'Status' && (target instanceof Pokemon) && target.status !== effect.id) { return relayVar; } // it's changed; call it off
		if (eventid === 'SwitchIn' && effect.effectType === 'Ability' && effect.flags['breakable'] && this.suppressingAbility(target as Pokemon)) {
			this.debug(eventid + ' handler suppressed by Mold Breaker');
			return relayVar;
		}
		if (eventid !== 'Start' && eventid !== 'TakeItem' && effect.effectType === 'Item' && (target instanceof Pokemon) && target.ignoringItem()) {
		...
		let returnVal;
		if (typeof callback === 'function') { returnVal = callback.apply(this, args); } 
		else { returnVal = callback; }
		this.eventDepth--;
	...
	runEvent(
		...
		if (onEffect) {
			...
			if (callback !== undefined) {
				...
				handlers.unshift(this.resolvePriority({ effect: sourceEffect, callback, state: this.initEffectState({}), end: null, effectHolder: target, }, `on${eventid}`));
			}
		}
		if (['Invulnerability', 'TryHit', 'DamagingHit', 'EntryHazard'].includes(eventid)) { handlers.sort(Battle.compareLeftToRightOrder); } 
		else if (fastExit) { handlers.sort(Battle.compareRedirectOrder); } 
		else { this.speedSort(handlers); }
		let hasRelayVar = 1;
		const args = [target, source, sourceEffect];
		...
		if (Array.isArray(target)) {
			if (Array.isArray(relayVar)) { targetRelayVars = relayVar; } 
			else { for (let i = 0; i < target.length; i++) targetRelayVars[i] = true; }
		}
		for (const handler of handlers) {
			if (handler.index !== undefined) { // TODO: find a better way to do this
			...
			if (effect.effectType === 'Status' && (effectHolder as Pokemon).status !== effect.id) { continue; }
			if (effect.effectType === 'Ability' && effect.flags['breakable'] && this.suppressingAbility(effectHolder as Pokemon)) {
			...
			effect.effectType === 'Item' && (effectHolder instanceof Pokemon) && effectHolder.ignoringItem()) {
				if (eventid !== 'Update') { this.debug(eventid + ' handler suppressed by Embargo, Klutz or Magic Room'); }
				continue;
			} else if (
				eventid !== 'End' && effect.effectType === 'Ability' &&
				(effectHolder instanceof Pokemon) && effectHolder.ignoringAbility()
			) {
				if (eventid !== 'Update') { this.debug(eventid + ' handler suppressed by Gastro Acid or Neutralizing Gas'); }
				continue;
			}
			...
			if (typeof handler.callback === 'function') {
				...
				this.effect = parentEffect;
				this.effectState = parentEffectState;
			} else { returnVal = handler.callback; }
		...
		this.eventDepth--;
		if (typeof relayVar === 'number' && relayVar === Math.abs(Math.floor(relayVar))) { relayVar = this.modify(relayVar, this.event.modifier); } // this.debug(eventid + ' modifier: 0x' + ('0000' + (this.event.modifier * 4096).toString(16)).slice(-4).toUpperCase());
		this.event = parentEvent;
		return Array.isArray(target) ? targetRelayVars : relayVar;
	}
	// priorityEvent works just like runEvent, except it exits and returns on the first non-undefined value instead of only on null/false.
	priorityEvent(
		eventid: string, target: Pokemon | Side | Battle, source?: Pokemon | null,
		effect?: Effect, relayVar?: any, onEffect?: boolean
	): any { return this.runEvent(eventid, target, source, effect, relayVar, onEffect, true); }
	resolvePriority(h: EventListenerWithoutPriority, callbackName: string) {
		...
		if (!handler.subOrder) {
			...
			if (handler.effect.effectType === 'Condition') {
				if (handler.state?.target instanceof Side) {
					if (handler.state.isSlotCondition) { handler.subOrder = 3; } 
					else { handler.subOrder = 4; } // side condition
				} else if (handler.state?.target instanceof Field) { handler.subOrder = 5; } // field condition
			} else if (handler.effect.effectType === 'Ability') {
				if (handler.effect.name === 'Poison Touch' || handler.effect.name === 'Perish Body') { handler.subOrder = 6; } 
				else if (handler.effect.name === 'Stall') { handler.subOrder = 9; }
			}
		}
		...
		if (handler.effectHolder && (handler.effectHolder as Pokemon).getStat) {
			...
			callbackName === 'onAllyTryHitSide') { handler.speed = pokemon.getStat('spe', true, true); }
	...
	getCallback(target: Pokemon | Side | Field | Battle, effect: Effect, callbackName: string) {
		...
		if (callback === undefined && target instanceof Pokemon && this.gen >= 5 && callbackName === 'onSwitchIn' && !(effect as any).onAnySwitchIn && (['Ability', 'Item'].includes(effect.effectType) || ( effect.effectType === 'Status' && ['ability', 'item'].includes(effect.id.split(':')[0]) ))) { callback = (effect as any).onStart; }
		return callback;
	}
	findEventHandlers(target: Pokemon | Pokemon[] | Side | Battle, eventName: string, source?: Pokemon | null) {
		...
		if (source && prefixedHandlers) { handlers.push(...this.findPokemonEventHandlers(source, `onSource${eventName}`)); }
		if (target instanceof Side) {
			for (const side of this.sides) {
				if (shouldBubbleDown) {
					for (const active of side.active) {
						if (side === target || side === target.allySide) { handlers = handlers.concat(this.findPokemonEventHandlers(active, `on${eventName}`)); } 
						else if (prefixedHandlers) { handlers = handlers.concat(this.findPokemonEventHandlers(active, `onFoe${eventName}`)); }
						if (prefixedHandlers) handlers = handlers.concat(this.findPokemonEventHandlers(active, `onAny${eventName}`));
					}
				}
				if (side.n < 2 || !side.allySide) {
					if (side === target || side === target.allySide) { handlers.push(...this.findSideEventHandlers(side, `on${eventName}`)); } 
					else if (prefixedHandlers) { handlers.push(...this.findSideEventHandlers(side, `onFoe${eventName}`)); }
	...
	findPokemonEventHandlers(pokemon: Pokemon, callbackName: string, getKey?: 'duration') {
		...
		if (callback !== undefined || (getKey && pokemon.statusState[getKey])) { handlers.push(this.resolvePriority({ effect: status, callback, state: pokemon.statusState, end: pokemon.clearStatus, effectHolder: pokemon, }, callbackName)); }
		for (const id in pokemon.volatiles) {
			...
			if (callback !== undefined || (getKey && volatileState[getKey])) { handlers.push(this.resolvePriority({ effect: volatile, callback, state: volatileState, end: pokemon.removeVolatile, effectHolder: pokemon, }, callbackName)); }
		}
		// Handle both abilities
		const ability1 = pokemon.getAbility(1);
		callback = this.getCallback(pokemon, ability1, callbackName);
		if (callback !== undefined || (getKey && pokemon.abilityState1[getKey])) { handlers.push(this.resolvePriority({ effect: ability1, callback, state: pokemon.abilityState1, end: () => pokemon.clearAbility(1), effectHolder: pokemon, }, callbackName)); }
		const ability2 = pokemon.getAbility(2);
		callback = this.getCallback(pokemon, ability2, callbackName);
		if (callback !== undefined || (getKey && pokemon.abilityState2[getKey])) { handlers.push(this.resolvePriority({ effect: ability2, callback, state: pokemon.abilityState2, end: () => pokemon.clearAbility(2), effectHolder: pokemon, }, callbackName)); }
		const item = pokemon.getItem();
		callback = this.getCallback(pokemon, item, callbackName);
		if (callback !== undefined || (getKey && pokemon.itemState[getKey])) { handlers.push(this.resolvePriority({ effect: item, callback, state: pokemon.itemState, end: pokemon.clearItem, effectHolder: pokemon, }, callbackName)); }
		const species = pokemon.baseSpecies;
		callback = this.getCallback(pokemon, species, callbackName);
		if (callback !== undefined) { handlers.push(this.resolvePriority({ effect: species, callback, state: pokemon.speciesState, end() {}, effectHolder: pokemon, }, callbackName)); }
	...
	findBattleEventHandlers(callbackName: string, getKey?: 'duration', customHolder?: Pokemon) {
		...
		if (callback !== undefined || (getKey && this.formatData[getKey])) { handlers.push(this.resolvePriority({ effect: format, callback, state: this.formatData, end: null, effectHolder: customHolder || this, }, callbackName)); }
	...
	onEvent(eventid: string, target: Format, ...rest: AnyObject[]) { // rest = [priority, callback]
		...
		if (target.effectType !== 'Format') { throw new TypeError(`${target.name} is a ${target.effectType} but only Format targets are supported right now`); }
		...
		if (eventHandlers === undefined) { this.events[callbackName] = [eventHandler]; } 
		else { eventHandlers.push(eventHandler); }
	}
	...
	getPokemon(fullname: string | Pokemon) {
		...
		for (const side of this.sides) { for (const pokemon of side.pokemon) { if (pokemon.fullname === fullname) return pokemon; } }
		return null;
	}
	getAllPokemon() {
		const pokemonList: Pokemon[] = [];
		for (const side of this.sides) { pokemonList.push(...side.pokemon); }
		return pokemonList;
	}
	...
	makeRequest(type?: RequestState) {
		...
		} else { type = this.requestState; }
		for (const side of this.sides) side.activeRequest = null;
		if (type === 'move') {
			for (const side of this.sides) {
	...
	getRequests(type: RequestState) { // default to no request
		...
		default:
			for (let i = 0; i < this.sides.length; i++) {
				...
				const activeData = side.active.map(pokemon => {
					...
					// send Guard Action data
					const guardMove = pokemon.getGuardActionMove();
					const guardActionMax = guardMove ? (guardMove.guardActionCD ?? 2) : 0;
					const guardActionCooldown = (pokemon as any).guardActionCooldown || 0;
					(data as any).guardAction = guardMove ? {
						id: guardMove.id,
						name: guardMove.name,
						cur: Math.max(0, guardActionMax - guardActionCooldown),
						max: guardActionMax,
					} : null;
					(data as any).canGuardAction = this.actions.canGuardAction(pokemon) !== null;
					// Mega Bar: which lettered Mega this Pokemon's held item makes available.
					const p = pokemon as any;
					p.canMegaEvoX = !!this.actions.canMegaEvoX?.(pokemon);
					p.canMegaEvoY = !!this.actions.canMegaEvoY?.(pokemon);
					p.canMegaEvoZ = !!this.actions.canMegaEvoZ?.(pokemon);
					p.canMegaEvoA = !!this.actions.canMegaEvoA?.(pokemon);
					p.canMegaEvoQ = !!this.actions.canMegaEvoQ?.(pokemon);
					(data as any).canMegaEvoX = p.canMegaEvoX;
					(data as any).canMegaEvoY = p.canMegaEvoY;
					(data as any).canMegaEvoZ = p.canMegaEvoZ;
					(data as any).canMegaEvoA = p.canMegaEvoA;
					(data as any).canMegaEvoQ = p.canMegaEvoQ;
					return data;
				});
		...
		for (let i = 0; i < this.sides.length; i++) {
			if (requests[i]) { if (!this.supportCancel || !multipleRequestsExist) requests[i].noCancel = true; } 
			else { requests[i] = { wait: true, side: this.sides[i].getRequestData() }; }
		}
		return requests;
	}
	tiebreak() {
		...
		if (tiedSides.length <= 1) { return this.win(tiedSides[0]); }
	...
	win(side?: SideID | '' | Side | null) {
		if (this.ended) return false;
		if (side && typeof side === 'string') { side = this.getSide(side); } 
		else if (!side || !this.sides.includes(side)) { side = null; }
		this.winner = side ? side.name : '';
		this.add('');
		if (side?.allySide) { this.add('win', side.name + ' & ' + side.allySide.name); } 
		else if (side) { this.add('win', side.name); } 
		else { this.add('tie'); }
		this.ended = true;
		this.requestState = '';
		for (const s of this.sides) { if (s) s.activeRequest = null; }
		return true;
	}
	lose(side: SideID | Side) {
		if (typeof side === 'string') { side = this.getSide(side); }
		if (!side) return; // can happen if a battle crashes
		if (this.gameType !== 'freeforall') { return this.win(side.foe); }
	...
	canSwitch(side: Side) { return this.possibleSwitches(side).length; }
	...
	private possibleSwitches(side: Side) {
		...
		for (let i = side.active.length; i < side.pokemon.length; i++) {
			...
			if (!pokemon.fainted) { canSwitchIn.push(pokemon); }
		}
		return canSwitchIn;
	}
	swapPosition(pokemon: Pokemon, newPosition: number, attributes?: string) {
		if (newPosition >= pokemon.side.active.length) { throw new Error("Invalid swap position"); }
	...
	faint(pokemon: Pokemon, source?: Pokemon, effect?: Effect) { pokemon.faint(source, effect); }
	endTurn() {
		this.turn++;
		this.lastSuccessfulMoveThisTurn = null;
		...
		for (const side of this.sides) {
			...
			for (const pokemon of side.active) {
				...
				for (const moveSlot of pokemon.moveSlots) {
					...
					if (activeMove.flags['cantusetwice'] && pokemon.lastMove?.id === moveSlot.id) { pokemon.disableMove(pokemon.lastMove.id); }
				}
				// If it was an illusion, it's not any more
				if (pokemon.getLastAttackedBy() && this.gen >= 7) pokemon.knownType = true;
				for (let i = pokemon.attackedBy.length - 1; i >= 0; i--) {
					const attack = pokemon.attackedBy[i];
					if (attack.source.isActive) { attack.thisTurn = false; } 
					else { pokemon.attackedBy.splice(pokemon.attackedBy.indexOf(attack), 1); }
				}
				...
				if (this.gen > 2) {
					for (const source of pokemon.foes()) {
						...
						for (const abilitySlot in species.abilities) {
							...
							if (abilityName === source.ability1 || abilityName === source.ability2) { continue; } // pokemon event was already run above so we don't need to run it again.
							const ruleTable = this.ruleTable;
							if ((ruleTable.has('+hackmons') || !ruleTable.has('obtainableabilities')) && !this.format.team) { continue; } // hackmons format
							else if (abilitySlot === 'H' && species.unreleasedHidden) { continue; } // unreleased hidden ability
		...
		if (!this.ended && !this.requestState) {
			const timeStopped = this.field.getPseudoWeather('timebreak');
			for (const side of this.sides) {
				const s = side as any;
				...
				if (!timeStopped) {
					...
					if (teraOnFieldMon) {
						...
						const isGlimmmegastellar =
							teraOnFieldMon.species.id === 'glimmoramegastellar' ||
							teraOnFieldMon.species.name === 'Glimmora-Mega-Stellar';
						const isBaxmegastellarQ =
							teraOnFieldMon.species.id === 'baxcaliburmegastellarq' ||
							teraOnFieldMon.species.name === 'Baxcalibur-Mega-Q-Stellar';
				...
				if (!timeStopped) {
					const megaMonOnField = side.active.find(p => p && !!(p as any).megaEvoOriginalSpecies);
					const megaMonAnywhere = side.pokemon.find(p => !!(p as any).megaEvoOriginalSpecies);
					if (megaMonOnField) {
						const turnsByLetter: { [k: string]: number } = { X: 5, Y: 5, A: 5, Q: 4, Z: 3 };
						const letter = ((megaMonOnField as any).megaEvoLetter || 'X') as string;
						const turns = turnsByLetter[letter] || 5;
						const drainRate = Math.ceil(megaMax / turns);
						let nextMega = Number(s.megaCharge) - drainRate;
						if (nextMega < 0) nextMega = 0;
						s.megaCharge = nextMega;
						if (nextMega === 0) { this.actions.revertMegaLetter?.(megaMonOnField); }
					} else if (megaMonAnywhere) {} //do nothing if mega is in the back 
					else {
						let nextMega = Number(s.megaCharge) + 20;
						if (nextMega > megaMax) nextMega = megaMax;
						s.megaCharge = nextMega;
					}
				}
			}
	...
	maybeTriggerEndlessBattleClause(trappedBySide: boolean[], stalenessBySide: ('internal' | 'external' | undefined)[]) {
		...
		if (losers.length === this.sides.length) { this.add('-message', `Each side's team started with the rudimentary means to perform restorative berry-cycling.`); }
		return this.tie();
	}
	start() {
		...
		if (this.sides.some(side => !side.pokemon[0])) { throw new Error('Battle not started: A player has an empty team.'); }
	...
	runPickTeam() {
		...
		if (this.requestState === 'teampreview') { return; }
	...
	checkEVBalance() {
		...
		for (const side of this.sides) {
			...
			if (limitedJVs === null) { limitedJVs = sideLimitedJVs; } 
			else if (limitedJVs !== sideLimitedJVs) { this.add('bigerror', "Warning: One player isn't adhering to a 130 JV limit, and the other player is."); }
		}
	}
	boost(boost: SparseBoostsTable, target: Pokemon | null = null, source: Pokemon | null = null, effect: Effect | null = null, isSecondary = false, isSelf = false) {
		if (this.event) {
		...
		for (boostName in boost) {
			...
			if (boostBy) {
				...
				default:
					...
					else if (effect.effectType === 'Item') { this.add(msg, target, boostName, boostBy, '[from] item: ' + effect.name); } 
					else {
			...
			} else if (effect?.effectType === 'Ability') { if (isSecondary || isSelf) this.add(msg, target, boostName, boostBy); } 
	...
	spreadDamage(
		...
		for (const [i, curDamage] of damage.entries()) {
			...
			case 'confused':
				...
				break;
			default:
				if (effect.effectType === 'Move' || !name) { this.add('-damage', target, target.getHealth); } 
				else if (source && (source !== target || effect.effectType === 'Ability')) { this.add('-damage', target, target.getHealth, `[from] ${name}`, `[of] ${source}`); } 
				else { this.add('-damage', target, target.getHealth, `[from] ${name}`); }
				break;
			}
	...
	heal(damage: number, target?: Pokemon, source: Pokemon | null = null, effect: 'drain' | Effect | null = null) {
		...
		default:
			if (!effect) break;
			if (effect.effectType === 'Move') { this.add('-heal', target, target.getHealth); } 
			else if (source && source !== target) { this.add('-heal', target, target.getHealth, `[from] ${effect.fullname}`, `[of] ${source}`); } 
			else { this.add('-heal', target, target.getHealth, `[from] ${effect.fullname}`); }
			break;
		}
	...
	chain(previousMod: number | number[], nextMod: number | number[]) {
		...
		if (Array.isArray(previousMod)) { previousMod = this.trunc(previousMod[0] * 4096 / previousMod[1]); } 
		else { previousMod = this.trunc(previousMod * 4096); }
		if (Array.isArray(nextMod)) { nextMod = this.trunc(nextMod[0] * 4096 / nextMod[1]); } 
		else { nextMod = this.trunc(nextMod * 4096); }
	...
	normalizeJVs(set: PokemonSet) {
		...
		if (Object.keys(jvs).length === 0 && anySet.evs) {
			...
			for (const s of ['hp', 'atk', 'def', 'spa', 'spd', 'spe'] as const) {
				const ev = (evs[s] ?? 0) | 0;
				jvs[s] = evLooksLikeJV ? ev : Math.floor(ev / 4);
			}
		}
		for (const s of ['hp', 'atk', 'def', 'spa', 'spd', 'spe'] as const) { if (jvs[s] === undefined) jvs[s] = 0; }
		for (const s of ['hp', 'atk', 'def', 'spa', 'spd', 'spe'] as const) {
			let v = (jvs[s] ?? 0) | 0;
			if (v < 0) v = 0;
			if (v > 64) v = 64;
		...
		let total = 0;
		for (const s of ['hp', 'atk', 'def', 'spa', 'spd', 'spe'] as const) { total += jvs[s]!; }
		while (total > 130) {
			let reduced = false;
			for (const s of ['hp', 'atk', 'def', 'spa', 'spd', 'spe'] as const) {
				if (total <= 130) break;
	...
	normalizeAbilitySetAndSize(set: PokemonSet) {
		...
		} else { if (anySet.ability2 === undefined) anySet.ability2 = ''; } // Even without abilitySet, ensure ability2 exists so dual-ability code never sees undefined
		const sz = anySet.size;
		if (sz === undefined || sz === null || sz === '') { anySet.size = 'M'; } 
		else if (!['XS', 'S', 'M', 'L', 'XL'].includes(sz)) { anySet.size = 'M'; }
	}
	spreadModify(baseStats: StatsTable, set: PokemonSet): StatsTable {
		const modStats: StatsTable = { hp: 0, atk: 0, def: 0, spa: 0, spd: 0, spe: 0 };
		for (const statName in baseStats) { modStats[statName as StatID] = this.statModify(baseStats, set, statName as StatID); }
		return modStats;
	}
	...
	getCategory(move: string | Move) { return this.dex.moves.get(move).category || 'Physical'; }
	...
	validTarget(target: Pokemon, source: Pokemon, targetType: string) { return this.validTargetLoc(source.getLocOf(target), source, targetType); }
	getTarget(pokemon: Pokemon, move: string | Move, targetLoc: number, originalTarget?: Pokemon) {
		...
		if (['adjacentAlly', 'any', 'normal'].includes(move.target) && targetLoc === selfLoc && !pokemon.volatiles['twoturnmove'] && !pokemon.volatiles['iceball'] && !pokemon.volatiles['rollout']) { return move.flags['futuremove'] ? pokemon : null; }
		if (move.target !== 'randomNormal' && this.validTargetLoc(targetLoc, pokemon, move.target)) {
			const target = pokemon.getAtLoc(targetLoc);
			if (target?.fainted) {
				if (this.gameType === 'freeforall') { return target; } // Target is a fainted opponent in a free-for-all battle; attack shouldn't retarget
			...
			if (target && !target.fainted) { return target; } // Target is unfainted: use selected target location
	...
	getRandomTarget(pokemon: Pokemon, move: string | Move) {
		...
		// For instance: Metronome chooses Ice Beam. Since the user didn't choose a target when choosing Metronome, Ice Beam's target must be chosen randomly.
		// The target is chosen randomly from possible targets, EXCEPT that moves that can target either allies or foes will only target foes when used without an explicit target.
		move = this.dex.moves.get(move);
	...
	faintMessages(lastFirst = false, forceCheck = false, checkWin = true) {
		...
		while (this.faintQueue.length) {
			...
			if (!pokemon.fainted && this.runEvent('BeforeFaint', pokemon, faintData.source, faintData.effect)) {
				...
				const p = pokemon as any;
				if (p.preTeraState) {
					const s = p.preTeraState;
					if (s.types) p.types = s.types;
					p.addedType = s.addedType;
					p.knownType = s.knownType;
					p.apparentType = s.apparentType;
					delete p.preTeraState;
				}
				delete p.terastallized;
				if (pokemon.formeRegression) { // after clearing volatiles
	...
	getActionSpeed(action: AnyObject) {
		if (action.choice === 'guard') {
			const guardMove = action.pokemon.getGuardActionMove();
			action.priority = guardMove ? guardMove.priority : 0;
			if (guardMove) {
				action.moveid = guardMove.id;
				action.move = guardMove;
			}
		}
		if (action.choice === 'move') {
			let move = action.move;
			// take priority from the base move, so abilities like Prankster only apply once (instead of compounding every time `getActionSpeed` is called)
	...
	runAction(action: Action) {
		...
		case 'start': {
			...
			for (const pokemon of this.getAllPokemon()) {
				...
				const terasever: {[k: string]: string} = { 'Baxcalibur-Mega-Q-Stellar': 'terasever', 'Baxcalibur-Mega-Y-Stellar': 'terasever', };
					const glaiveRush = pokemon.baseMoves.indexOf('glaiverush');
					if (glaiveRush >= 0) {
						const move = this.dex.moves.get(terasever[pokemon.species.name]);
						pokemon.baseMoveSlots[glaiveRush] = {
							move: move.name,
							id: move.id,
							pp: move.pp,
							maxpp: move.pp,
					...
					const teracannon: {[k: string]: string} = { 'Glimmora-Mega-Stellar': 'teracannon', };
					const meteorbeam = pokemon.baseMoves.indexOf('meteorbeam');
					if (meteorbeam >= 0) {
						const move = this.dex.moves.get(teracannon[pokemon.species.name]);
						pokemon.baseMoveSlots[meteorbeam] = {
							move: move.name,
							id: move.id,
							pp: move.pp,
							maxpp: move.pp,
			...
			for (const side of this.sides) {
				for (let i = 0; i < side.active.length; i++) {
					...
					} else if (side.pokemon[i]) { this.actions.switchIn(side.pokemon[i], i); }
				}
		...
		case 'move':
			...
			this.actions.runMove(action.move, action.pokemon, action.targetLoc, { sourceEffect: action.sourceEffect, originalTarget: action.originalTarget, });
			break;
		case 'megaEvo':
		...
		case 'megaEvoY':
			...
			break;
		case 'megaEvoZ':
			this.actions.runMegaEvoZ?.(action.pokemon);
			break;
		case 'megaEvoA':
			this.actions.runMegaEvoA?.(action.pokemon);
			break;
		case 'megaEvoQ':
			this.actions.runMegaEvoQ?.(action.pokemon);
			break;
		case 'terastallize': {
			const p = action.pokemon as any;
			// Snapshot original typing so we can un-tera later
			if (!p.preTeraState) {
				p.preTeraState = {
					types: Array.isArray(p.types) ? [...p.types] : undefined,
					addedType: p.addedType,
					knownType: p.knownType,
				};
			}
		...
		case 'guard': {
			...
			if (!pokemon.isActive) return false;
			if (pokemon.fainted) return false;
			if (!this.actions.canGuardAction(pokemon)) return false;
			this.actions.useGuardAction(pokemon);
			break;
		}
		case 'beforeTurnMove':
			if (!action.pokemon.isActive) return false;
		...
		case 'event':
			...
			break;
		case 'team':
			if (action.index === 0) { action.pokemon.side.pokemon = []; }
		...
		case 'instaswitch':
		case 'switch':
			if (action.choice === 'switch' && action.pokemon.status) { this.singleEvent('CheckShow', this.dex.abilities.getByID('naturalcure' as ID), null, action.pokemon); }
			if (this.actions.switchIn(action.target, action.pokemon.position, action.sourceEffect) === 'pursuitfaint') { // when a pokemon faints from pursuit, the switch is cancelled
		...
		if (['megaEvo', 'megaEvoX', 'megaEvoY', 'megaEvoZ', 'megaEvoA', 'megaEvoQ'].includes(action.choice)) {
			...
			return false;
		} else if (this.queue.peek()?.choice === 'instaswitch') { return false; }
		if (action.choice !== 'start') {
			this.eachEvent('Update');
			for (const [pokemon, originalHP] of residualPokemon) {
				const maxhp = pokemon.getUndynamaxedHP(pokemon.maxhp);
				if (pokemon.hp && pokemon.getUndynamaxedHP() <= maxhp / 2 && originalHP > maxhp / 2) { this.runEvent('EmergencyExit', pokemon); }
			}
		}
		if (action.choice === 'runSwitch') {
			const pokemon = action.pokemon;
			if (pokemon.hp && pokemon.hp <= pokemon.maxhp / 2 && pokemonOriginalHP! > pokemon.maxhp / 2) { this.runEvent('EmergencyExit', pokemon); }
		}
			...
			if (switches[i] && !this.canSwitch(this.sides[i])) {
				...
				for (const pokemon of this.sides[i].active) {
					if (pokemon.hp && pokemon.switchFlag && pokemon.switchFlag !== 'revivalblessing' && !pokemon.skipBeforeSwitchOutEventFlag) {
						...
						if (this.ended) return true;
						if (pokemon.fainted) { switches[i] = this.sides[i].active.some(sidePokemon => sidePokemon && !!sidePokemon.switchFlag); }
					}
				}
			}
		}
		...
		if (this.queue.peek()?.choice === 'move') { // In gen 8, speed is updated dynamically so update the queue's speed properties and sort it.
			this.updateSpeed();
	...
	/**
	 * Generally called at the beginning of a turn, to go through the turn one action at a time.
	 * If there is a mid-turn decision (like U-Turn), this will return and be called again later to resume the turn.
	 */
	turnLoop() {
		this.add('');
		this.add('t:', Math.floor(Date.now() / 1000));
	...
	// Takes a choice string passed from the client. Starts the next turn if all required choices have been made.
	choose(sideid: SideID, input: string) {
		const side = this.getSide(sideid);
		if (!side.choose(input)) {
			if (!side.choice.error) { side.emitChoiceError(`Unknown error for choice: ${input}. If you're not using a custom client, please report this as a bug.`); }
			return false;
		}
	...
	makeChoices(...inputs: string[]) {
		if (inputs.length) { for (const [i, input] of inputs.entries()) { if (input) this.sides[i].choose(input); } } 
		else  { for (const side of this.sides) { side.autoChoose(); } }
		this.commitChoices();
	}
	commitChoices() {
		this.updateSpeed();
		// Sometimes you need to make switch choices mid-turn (e.g. U-turn, fainting). When this happens, the rest of the turn is saved (and not re-sorted), but the new switch choices are sorted and inserted before the rest of the turn.
		...
		for (const side of this.sides) { this.queue.addChoice(side.choice.actions); }
		this.clearRequest();
		this.queue.sort();
	...
	undoChoice(sideid: SideID) {
		...
		if (side.requestState === 'move') {
			for (const action of side.choice.actions) {
				...
				if (side.updateRequestForPokemon(pokemon, req => side.updateDisabledRequest(pokemon, req))) { updated = true; }
			}
		}
		side.clearChoice();
	...
	debug(activity: string) { if (this.debugMode) { this.add('debug', activity); } }
	getDebugLog() {
		const channelMessages = extractChannelMessages(this.log.join('\n'), [-1]);
		return channelMessages[-1].join('\n');
	}
	debugError(activity: string) { this.add('debug', activity); }
	// players
	getTeam(options: PlayerOptions): PokemonSet[] {
		...
		if (!options.seed) { options.seed = PRNG.generateSeed(); }
		if (!this.teamGenerator) { this.teamGenerator = Teams.getGenerator(this.format, options.seed); } 
		else { this.teamGenerator.setSeed(options.seed); }
		...
		for (const side of this.sides) {
			const team = side.pokemon.map(pokemon => {
				...
				if ((toID(set.species) === 'zacian' && toID(set.item) === 'rustedsword') ||
					...
					if (ironHeadIndex >= 0) { newSet.moves[ironHeadIndex] = crowned[newSet.species]; }
				}
				return newSet;
			});
	...
	sendUpdates() {
		const hasNewLog = this.sentLogPos < this.log.length;
		// Send battle log updates if any
		if (hasNewLog) {
			this.send('update', this.log.slice(this.sentLogPos));
			this.sentLogPos = this.log.length;
		}
		// ALWAYS send requests when needed
		if (!this.sentRequests) {
			for (const side of this.sides) { if (side.activeRequest) side.emitRequest(); } // Only emit if this side still has a pending request
			this.sentRequests = true;
		}
		// ALWAYS send end packet when battle ended
	...
	getSide(sideid: SideID): Side { return this.sides[parseInt(sideid[1]) - 1]; }
	/**
	 ...
	 */
	getOverflowedTurnCount(): number { return this.gen >= 8 ? (this.turn - 1) % 256 : this.turn - 1; }
	initEffectState(obj: Partial<EffectState>, effectOrder?: number): EffectState {
		...
		else if (obj.id && obj.target && (!(obj.target instanceof Pokemon) || obj.target.isActive)) { obj.effectOrder = this.effectOrder++; } 
	...
	clearEffectState(state: EffectState) {
		...
		for (const k in state) {
			if (k === 'id' || k === 'target') { continue; } 
			else if (k === 'effectOrder') { state.effectOrder = 0; } 
			else { delete state[k]; }
		}
	}
	destroy() {
