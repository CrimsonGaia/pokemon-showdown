 ...
 * The action queue is the core of the battle simulation. A rough overview of the core battle loop:
...
export interface MoveAction {
	...
	pokemon: Pokemon; /** the pokemon doing the move */
	targetLoc: number; /** location of the target, relative to pokemon's side */
	originalTarget: Pokemon; /** original target pokemon, for target-tracking moves */
...
export class BattleQueue {
	...
	peek(end?: boolean): Action | undefined { return this.list[end ? this.list.length - 1 : 0]; }
	push(action: Action) { return this.list.push(action); }
	unshift(action: Action) { return this.list.unshift(action); }
	[Symbol.iterator]() { return this.list[Symbol.iterator](); }
	entries() { return this.list.entries(); }
	/**
	 * Takes an ActionChoice, and fills it out into a full Action object.
	 * Returns an array of Actions because some ActionChoices (like mega moves) resolve to two Actions (mega evolution + use move)
	 */
	...
	addChoice(choices: ActionChoice | ActionChoice[]) {
		...
		for (const choice of choices) {
			...
			for (const resolvedChoice of resolvedChoices) { if (resolvedChoice && resolvedChoice.choice === 'move' && resolvedChoice.move.id !== 'recharge') { resolvedChoice.pokemon.side.lastSelectedMove = resolvedChoice.move.id; } }
		}
	}
	willAct() {
		for (const action of this.list) { if (['move', 'switch', 'instaswitch', 'shift'].includes(action.choice)) { return action; } }
		return null;
	}
	willMove(pokemon: Pokemon) {
		if (pokemon.fainted) return null;
		for (const action of this.list) { if (action.choice === 'move' && action.pokemon === pokemon) { return action; } }
		return null;
	}
	...
	willSwitch(pokemon: Pokemon) {
		for (const action of this.list) { if (['switch', 'instaswitch'].includes(action.choice) && action.pokemon === pokemon) { return action; } }
		return null;
	}
	// Inserts the passed action into the action queue when it normally would have happened (sorting by priority/speed), without re-sorting the existing actions.
