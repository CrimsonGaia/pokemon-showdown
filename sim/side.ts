...
export interface ChosenAction {
	choice: 'move' | 'guard' | 'switch' | 'instaswitch' | 'revivalblessing' | 'team' | 'shift' | 'pass';// action type
	...
	megaz?: boolean | null; // true if megaing z
	megaa?: boolean | null; // true if megaing a
	megaq?: boolean | null; // true if megaing q
...
export class Side {
	...
	toJSON(): AnyObject { return State.serializeSide(this); }
	...
	getChoice() {
		if (this.choice.actions.length > 1 && this.choice.actions.every(action => action.choice === 'team')) { return `team ` + this.choice.actions.map(action => action.pokemon!.position + 1).join(', '); }
		return this.choice.actions.map(action => {
			...
			case 'move':
				...
				if (action.megaz) details += ` megaz`;
				if (action.megaa) details += ` megaa`;
				if (action.megaq) details += ` megaq`;
	...
	foePokemonLeft() {
		if (this.battle.gameType === 'freeforall') { return this.battle.sides.filter(side => side !== this).map(side => side.pokemonLeft).reduce((a, b) => a + b); }
	...
	hasAlly(pokemon: Pokemon) { return pokemon.side === this || pokemon.side === this.allySide; }
	addSideCondition(status: string | Condition, source: Pokemon | 'debug' | null = null, sourceEffect: Effect | null = null): boolean {
		...
		if (status.durationCallback) { this.sideConditions[status.id].duration = status.durationCallback.call(this.battle, this.active[0], source, sourceEffect); }
	...
	addSlotCondition(
		...
		if (status.durationCallback) { conditionState.duration = status.durationCallback.call(this.battle, this.active[0], source, sourceEffect); }
	...
	emitChoiceError(message: string, update?: { pokemon: Pokemon, update: (req: PokemonMoveRequestData) => boolean | void }) {
	...
	isChoiceDone() {
		...
		if (this.requestState === 'teampreview') { return this.choice.actions.length >= this.pickedTeamSize(); }
	...
	chooseMove(
		...
		targetLoc = 0,
		event: 'megax' | 'megay' | 'megaz' | 'megaa' | 'megaq' | 'ultra' | 'terastallize' | '' = '',
		teraempower = false
		) {
		if (this.requestState !== 'move') { return this.emitChoiceError(`Can't move: You need a ${this.requestState} response`); }
		const index = this.getChoiceIndex();
		if (index >= this.active.length) { return this.emitChoiceError(`Can't move: You sent more choices than unfainted Pokémon.`); }
		...
		if (typeof moveText === 'number' || (moveText && /^[0-9]+$/.test(moveText))) {
			...
			if (moveIndex < 0 || moveIndex >= request.moves.length || !request.moves[moveIndex]) { return this.emitChoiceError(`Can't move: Your ${pokemon.name} doesn't have a move ${moveIndex + 1}`); }
			...
			if (!targetType) { if (moveid !== 'testfight') { return this.emitChoiceError(`Can't move: Your ${pokemon.name} doesn't have a move matching ${moveid}`); }  }
		}
		...
		if (autoChoose || moveid === 'testfight') { targetLoc = 0; } 
			...
			if (!targetLoc && this.active.length >= 2) { return this.emitChoiceError(`Can't move: ${move.name} needs a target`); }
			if (!this.battle.validTargetLoc(targetLoc, pokemon, targetType)) { return this.emitChoiceError(`Can't move: Invalid target for ${move.name}`); }
		...
		if (lockedMove) {
			...
			if (pokemon.volatiles[lockedMoveID]?.targetLoc) { lockedMoveTargetLoc = pokemon.volatiles[lockedMoveID].targetLoc; }
			...
			// test fight button
			if (!pokemon.maybeLocked) { return this.emitChoiceError(`Can't move: ${pokemon.name}'s Fight button is known to be safe`); }
		...
		const megaz = (event === 'megaz');
		const megaa = (event === 'megaa');
		const megaq = (event === 'megaq');
		if (megax && !pokemon.canMegaEvoX) { return this.emitChoiceError(`Can't move: ${pokemon.name} can't mega evolve X`); }
		if (megay && !pokemon.canMegaEvoY) { return this.emitChoiceError(`Can't move: ${pokemon.name} can't mega evolve Y`); }
		if (megaz && !pokemon.canMegaEvoZ) { return this.emitChoiceError(`Can't move: ${pokemon.name} can't mega evolve Z`); }
		if (megaa && !pokemon.canMegaEvoA) { return this.emitChoiceError(`Can't move: ${pokemon.name} can't mega evolve A`); }
		...
		if (terastallize && this.choice.terastallize) { return this.emitChoiceError(`Can't move: You can only Terastallize once per battle.`); }
		if (terastallize && this.battle.gen !== 9) { return this.emitChoiceError(`Can't move: You can only Terastallize in Gen 9.`); }
		const isTeraEmpower = teraempower;
		if (isTeraEmpower) {
			if (this.battle.gen !== 9) { return this.emitChoiceError(`Can't move: You can only Tera Empower in Gen 9.`); }
			if (moveid !== 'terablast' && moveid !== 'terastarstorm') { return this.emitChoiceError(`Can't move: Only Tera Blast and Tera Starstorm can be Tera Empowered.`); }
			if (pokemon.terastallized) { return this.emitChoiceError(`Can't move: ${pokemon.name} is already Terastallized.`); }
			const sideAny = pokemon.side as any;
			const charge = Number(sideAny.teraCharge ?? 0);     
			const max = Number(sideAny.teraChargeMax ?? 100); 
			const COST = 10;
			if (charge >= max) { return this.emitChoiceError(`Can't move: Your Tera Charge is full; Terastallize instead.`); }
			if (charge < COST) { return this.emitChoiceError(`Can't move: Not enough Tera Charge to Tera Empower.`);  }
		}
		this.choice.actions.push({
			choice: 'move',
			pokemon,
		...
		if (pokemon.maybeDisabled && (this.battle.gameType === 'singles')) { this.choice.cantUndo = true; }
		if (megax || megay|| megaz|| megaa|| megaq) this.choice.mega = true;
	...
	updateDisabledRequest(pokemon: Pokemon, req: PokemonMoveRequestData) {
		...
		if (req.moves.every(m => m.disabled || m.id === 'struggle')) {
			...
			if (req.canMegaEvoZ) {
				req.canMegaEvoZ = false;
				updated = true;
			}
			if (req.canMegaEvoA) {
				req.canMegaEvoA = false;
				updated = true;
			}
			if (req.canMegaEvoQ) {
				req.canMegaEvoQ = false;
				updated = true;
			}
	...
	updateRequestForPokemon(pokemon: Pokemon, update: (req: PokemonMoveRequestData) => boolean | void) {
		if (!(this.activeRequest as MoveRequest)?.active) { throw new Error(`Can't update a request without active Pokemon`); }
	...
	chooseSwitch(slotText?: string) {
		if (this.requestState !== 'move' && this.requestState !== 'switch') { return this.emitChoiceError(`Can't switch: You need a ${this.requestState} response`); }
		const index = this.getChoiceIndex();
		if (index >= this.active.length) {
			if (this.requestState === 'switch') { return this.emitChoiceError(`Can't switch: You sent more switches than Pokémon that need to switch`); }
			return this.emitChoiceError(`Can't switch: You sent more choices than unfainted Pokémon`);
		}
		const pokemon = this.active[index];
		let slot;
		if (!slotText) {
			if (this.requestState !== 'switch') { return this.emitChoiceError(`Can't switch: You need to select a Pokémon to switch in`); }
		...
		if (isNaN(slot) || slot < 0) {
			...
			if (slot < 0) { return this.emitChoiceError(`Can't switch: You do not have a Pokémon named "${slotText}" to switch to`); }
		}
		if (slot >= this.pokemon.length) { return this.emitChoiceError(`Can't switch: You do not have a Pokémon in slot ${slot + 1} to switch to`); } 
		else if (slot < this.active.length && !this.slotConditions[pokemon.position]['revivalblessing']) { return this.emitChoiceError(`Can't switch: You can't switch to an active Pokémon`); } 
		else if (this.choice.switchIns.has(slot)) { return this.emitChoiceError(`Can't switch: The Pokémon in slot ${slot + 1} can only switch in once`); }
		const targetPokemon = this.pokemon[slot];
		if (this.slotConditions[pokemon.position]['revivalblessing']) {
			if (!targetPokemon.fainted) { return this.emitChoiceError(`Can't switch: You have to pass to a fainted Pokémon`); }
			...
			this.choice.actions.push({ choice: 'revivalblessing', pokemon, target: targetPokemon, } as ChosenAction);
			return true;
		}
		if (targetPokemon.fainted) { return this.emitChoiceError(`Can't switch: You can't switch to a fainted Pokémon`); }
		if (this.requestState === 'move') {
			...
			} else if (pokemon.maybeTrapped) { this.choice.cantUndo = true; }
		} else if (this.requestState === 'switch') {
			if (!this.choice.forcedSwitchesLeft) { throw new Error(`Player somehow switched too many Pokemon`);  }
	 ...
	 * supporting it.
	 */
	pickedTeamSize() { return Math.min(this.pokemon.length, this.battle.ruleTable.pickedTeamSize || Infinity); }
	chooseTeam(data?: string) {
		if (this.requestState !== 'teampreview') { return this.emitChoiceError(`Can't choose for Team Preview: You're not in a Team Preview phase`); }
		...
		for (const [index, pos] of positions.entries()) {
			if (isNaN(pos) || pos < 0 || pos >= this.pokemon.length) { return this.emitChoiceError(`Can't choose for Team Preview: You do not have a Pokémon in slot ${pos + 1}`); }
			if (positions.indexOf(pos) !== index) { return this.emitChoiceError(`Can't choose for Team Preview: The Pokémon in slot ${pos + 1} can only switch in once`); }
		}
		const result = ruleTable.onChooseTeam?.[0].call(this.battle, positions, this.pokemon, !data);
		if (result) {
			if (typeof result === 'string') { return this.emitChoiceError(`Can't choose for Team Preview: ${result}`); }
			if (result.length < pickedTeamSize) { throw new Error(`onChooseTeam from ${ruleTable.onChooseTeam![1]} returned a team of size ${result.length}, which is less than the required size of ${pickedTeamSize}`); }
	...
	chooseShift() {
		const index = this.getChoiceIndex();
		if (index >= this.active.length) { return this.emitChoiceError(`Can't shift: You do not have a Pokémon in slot ${index + 1}`); } 
		else if (this.requestState !== 'move') { return this.emitChoiceError(`Can't shift: You can only shift during a move phase`); } 
		else if (this.battle.gameType !== 'triples') { return this.emitChoiceError(`Can't shift: You can only shift to the center in triples`); } 
		else if (index === 1) { return this.emitChoiceError(`Can't shift: You can only shift from the edge to the center`); }
		const pokemon: Pokemon = this.active[index];
		this.choice.actions.push({
			choice: 'shift',
			pokemon,
	...
	chooseGuardAction(): boolean {
	const index = this.getChoiceIndex();
		if (index >= this.active.length) { return this.emitChoiceError(`Can't use Guard Action: You sent more choices than unfainted Pokémon.`); }
		const pokemon: Pokemon = this.active[index];
		const guardMove = this.battle.actions.canGuardAction(pokemon);
		if (!guardMove) { return this.emitChoiceError(`Can't use Guard Action: ${pokemon.name} doesn't have a Guard Action available right now.`); }
		this.choice.actions.push({ choice: 'guard', pokemon, moveid: guardMove.id, });
		return true;
	}
	choose(input: string) {
		if (!this.requestState) { return this.emitChoiceError(this.battle.ended ? `Can't do anything: The game is over` : `Can't do anything: It's not your turn`); }
		if (this.choice.cantUndo) { return this.emitChoiceError(`Can't undo: A trapping/disabling effect would cause undo to leak information`); }
		this.clearChoice();
		const choiceStrings = (input.startsWith('team ') ? [input] : input.split(','));
		if (choiceStrings.length > this.active.length) { return this.emitChoiceError( `Can't make choices: You sent choices for ${choiceStrings.length} Pokémon, but this is a ${this.battle.gameType} game!`); }
		for (const choiceString of choiceStrings) {
			...
			case 'move':
				...
				let event: 'mega' | 'megax' | 'megay' | 'megaz' | 'megaa' | 'megaq' | 'ultra' | 'terastallize' | '' = '';
				let teraempower = false;
				while (true) {
					...
					if (/\s(?:-|\+)?[1-3]$/.test(data) && toID(data) !== 'conversion2') {
						...
						event = 'megay';
						data = data.slice(0, -6);
					} else if (data.endsWith(' megaz')) {
						if (event) return error();
						event = 'megaz';
						data = data.slice(0, -6);
					} else if (data.endsWith(' megaa')) {
						if (event) return error();
						event = 'megaa';
						data = data.slice(0, -6);
					} else if (data.endsWith(' megaq')) {
						if (event) return error();
						event = 'megaq';
						data = data.slice(0, -6);
					...
					} else if (data.endsWith(' teraempower')) {
						teraempower = true;
						data = data.slice(0, -12).trim();
					}
					 else { break; }
				}
				if (!this.chooseMove(data, targetLoc, event, teraempower)) return false;
				break;
			case 'switch':
			...
			case 'shift':
				...
				break;
			case 'guard':
				if (data) return this.emitChoiceError(`Unrecognized data after "guard": ${data}`);
				if (!this.chooseGuardAction()) return false;
				break;
			case 'team':
	...
	choosePass(): boolean | Side {
		...
		case 'switch':
			if (pokemon.switchFlag) { // This condition will always happen if called by Battle#choose()
				if (!this.choice.forcedPassesLeft) { return this.emitChoiceError(`Can't pass: You need to switch in a Pokémon to replace ${pokemon.name}`); }
				this.choice.forcedPassesLeft--;
			}
			break;
		case 'move':
			if (!pokemon.fainted && !pokemon.volatiles['commanding']) { return this.emitChoiceError(`Can't pass: Your ${pokemon.name} must make a move (or switch)`); }
			break;
		default:
			return this.emitChoiceError(`Can't pass: Not a move or switch request`);
		}
		this.choice.actions.push({ choice: 'pass', } as ChosenAction);
		return true;
	}
	/** Automatically finish a choice if not currently complete. */
	autoChoose() {
		if (this.requestState === 'teampreview') { if (!this.isChoiceDone()) this.chooseTeam(); } 
		else if (this.requestState === 'switch') {
			let i = 0;
	...
	destroy() {
		...
		for (const pokemon of this.pokemon) { if (pokemon) pokemon.destroy(); }
