...
export interface FormatData extends Partial<Format>, EventMethods { name: string; }
...
export class RuleTable extends Map<string, string> {
	...
	isBannedSpecies(species: Species) {
		...
		for (const tagid in Tags) {
			...
			if (this.has(`-pokemontag:${tagid}`)) { if ((tag.speciesFilter || tag.genericFilter)!(species)) return true; }
		}
		for (const tagid in Tags) {
			const tag = Tags[tagid as ID];
			if (this.has(`+pokemontag:${tagid}`)) { if ((tag.speciesFilter || tag.genericFilter)!(species)) return false; }
		}
	...
	getTagRules() {
		...
		for (const ruleid of this.keys()) {
			if (/^[+*-]pokemontag:/.test(ruleid)) {
				...
				} else { tagRules.push(ruleid); }
			} else if ('+*-'.includes(ruleid.charAt(0)) && ruleid.slice(1) === 'nonexistent') { tagRules.push(ruleid.charAt(0) + 'pokemontag:nonexistent'); }
		}
	...
	getReason(key: string): string | null {
		...
		if (key === '-nonexistent' || key.startsWith('obtainable')) { return 'not obtainable'; }
	...
	addComplexBan(rule: string, source: string, limit: number, bans: string[]) {
		...
		} else { this.complexBans.push([rule, source, limit, bans]); }
	}
	addComplexTeamBan(rule: string, source: string, limit: number, bans: string[]) {
		...
		} else { this.complexTeamBans.push([rule, source, limit, bans]); }
	}
	...
	resolveNumbers(format: Format, dex: ModdedDex) {
		...
		if (!this.minSourceGen) {
			if (dex.gen >= 9 && this.has('obtainable') && !this.has('natdexmod')) { this.minSourceGen = dex.gen; } 
			else { this.minSourceGen = 1; }
		}
		const timer: Partial<GameTimerSettings> = {};
		if (this.valueRules.has('timerstarting')) { timer.starting = Number(this.valueRules.get('timerstarting')); }
		if (this.has('dctimer')) { timer.dcTimer = true; }
		if (this.has('dctimerbank')) { timer.dcTimer = true; }
		if (this.valueRules.has('timergrace')) { timer.grace = Number(this.valueRules.get('timergrace')); }
		if (this.valueRules.has('timeraddperturn')) { timer.addPerTurn = Number(this.valueRules.get('timeraddperturn')); }
		if (this.valueRules.has('timermaxperturn')) { timer.maxPerTurn = Number(this.valueRules.get('timermaxperturn')); }
		if (this.valueRules.has('timermaxfirstturn')) { timer.maxFirstTurn = Number(this.valueRules.get('timermaxfirstturn')); }
		if (this.has('timeoutautochoose')) { timer.timeoutAutoChoose = true; }
		if (this.has('timeraccelerate')) { timer.accelerate = true; }
		...
		if (!this.defaultLevel) {
			...
			if (this.maxTotalLevel && this.maxLevel > 100 && this.maxLevel * maxTeamSize > this.maxTotalLevel) { this.defaultLevel = 100; } 
			else { this.defaultLevel = this.maxLevel; }
		}
		if (this.minTeamSize && this.minTeamSize < gameTypeMinTeamSize) { throw new Error(`Min team size ${this.minTeamSize}${this.blame('minteamsize')} must be at least ${gameTypeMinTeamSize} for a ${format.gameType} game.`); }
		if (this.pickedTeamSize && this.pickedTeamSize < gameTypeMinTeamSize) { throw new Error(`Chosen team size ${this.pickedTeamSize}${this.blame('pickedteamsize')} must be at least ${gameTypeMinTeamSize} for a ${format.gameType} game.`); }
		if (this.minTeamSize && this.pickedTeamSize && this.minTeamSize < this.pickedTeamSize) { throw new Error(`Min team size ${this.minTeamSize}${this.blame('minteamsize')} is lower than chosen team size ${this.pickedTeamSize}${this.blame('pickedteamsize')}.`); }
		if (!this.minTeamSize) this.minTeamSize = Math.max(gameTypeMinTeamSize, this.pickedTeamSize || 0);
		if (this.maxTeamSize < gameTypeMinTeamSize) { throw new Error(`Max team size ${this.maxTeamSize}${this.blame('maxteamsize')} must be at least ${gameTypeMinTeamSize} for a ${format.gameType} game.`); }
		if (this.maxTeamSize < this.minTeamSize) { throw new Error(`Max team size ${this.maxTeamSize}${this.blame('maxteamsize')} must be at least min team size ${this.minTeamSize}${this.blame('minteamsize')}.`); }
		if (this.minLevel > this.maxLevel) { throw new Error(`Min level ${this.minLevel}${this.blame('minlevel')} should not be above max level ${this.maxLevel}${this.blame('maxlevel')}.`); }
		if (this.defaultLevel > this.maxLevel) { throw new Error(`Default level ${this.defaultLevel}${this.blame('defaultlevel')} should not be above max level ${this.maxLevel}${this.blame('maxlevel')}.`); }
		if (this.defaultLevel < this.minLevel) { throw new Error(`Default level ${this.defaultLevel}${this.blame('defaultlevel')} should not be below min level ${this.minLevel}${this.blame('minlevel')}.`); }
		if (this.adjustLevelDown && this.adjustLevelDown >= this.maxLevel) { throw new Error(`Adjust Level Down ${this.adjustLevelDown}${this.blame('adjustleveldown')} will have no effect because it's not below max level ${this.maxLevel}${this.blame('maxlevel')}.`); }
		if (this.adjustLevel && this.valueRules.has('minlevel')) { throw new Error(`Min Level ${this.minLevel}${this.blame('minlevel')} will have no effect because you're using Adjust Level ${this.adjustLevel}${this.blame('adjustlevel')}.`); }
		if (this.evLimit && this.evLimit >= 1512) { throw new Error(`EV Limit ${this.evLimit}${this.blame('evlimit')} will have no effect because it's not lower than 1512, the maximum possible combination of 252 EVs in every stat (if you currently have an EV limit, use "! EV Limit" to remove the limit).`); }
		if (this.evLimit && this.evLimit < 0) { throw new Error(`EV Limit ${this.evLimit}${this.blame('evlimit')} can't be less than 0 (you might have meant: "! EV Limit" to remove the limit, or "EV Limit = 0" to ban EVs).`); }
		if (timer.starting !== undefined && (timer.starting < 10 || timer.starting > 1200)) { throw new Error(`Timer starting value ${timer.starting}${this.blame('timerstarting')} must be between 10 and 1200 seconds.`); }
		if (timer.grace && timer.grace > 300) { throw new Error(`Timer grace value ${timer.grace}${this.blame('timergrace')} must be at most 300 seconds.`); }
		if (timer.addPerTurn && timer.addPerTurn > 30) { throw new Error(`Timer add per turn value ${timer.addPerTurn}${this.blame('timeraddperturn')} must be at most 30 seconds.`); }
		if (timer.maxPerTurn !== undefined && (timer.maxPerTurn < 10 || timer.maxPerTurn > 1200)) { throw new Error(`Timer max per turn value ${timer.maxPerTurn}${this.blame('timermaxperturn')} must be between 10 and 1200 seconds.`); }
		if (timer.maxFirstTurn !== undefined && (timer.maxFirstTurn < 10 || timer.maxFirstTurn > 1200)) { throw new Error(`Timer max first turn value ${timer.maxFirstTurn}${this.blame('timermaxfirstturn')} must be between 10 and 1200 seconds.`); }
		if ((format as any).cupLevelLimit) { throw new Error(`cupLevelLimit.range[0], cupLevelLimit.range[1], cupLevelLimit.total are now rules, respectively: "Min Level = NUMBER", "Max Level = NUMBER", and "Max Total Level = NUMBER"`);  }
		if ((format as any).teamLength) { throw new Error(`teamLength.validate[0], teamLength.validate[1], teamLength.battle are now rules, respectively: "Min Team Size = NUMBER", "Max Team Size = NUMBER", and "Picked Team Size = NUMBER"`); }
		if ((format as any).minSourceGen) { throw new Error(`minSourceGen is now a rule: "Min Source Gen = NUMBER"`); }
		if ((format as any).maxLevel) { throw new Error(`maxLevel is now a rule: "Max Level = NUMBER"`); }
		if ((format as any).defaultLevel) { throw new Error(`defaultLevel is now a rule: "Default Level = NUMBER"`); }
		if ((format as any).forcedLevel) { throw new Error(`forcedLevel is now a rule: "Adjust Level = NUMBER"`); }
		if ((format as any).maxForcedLevel) { throw new Error(`maxForcedLevel is now a rule: "Adjust Level Down = NUMBER"`); }
	}
	hasComplexBans() { return (this.complexBans?.length > 0) || (this.complexTeamBans?.length > 0); }
}
export class Format extends BasicEffect implements Readonly<BasicEffect> {
	readonly mod: string;
	// Name of the team generator algorithm, if this format uses random/fixed teams. null if players can bring teams.
	declare readonly team?: string;
	override readonly effectType: FormatEffectType;
	readonly debug: boolean;
	/**
	 * Whether or not a format will update ladder points if searched for using the "Battle!" button.
	...
	declare readonly onValidateRule?: (this: { format: Format, ruleTable: RuleTable, dex: ModdedDex }, value: string) => string | void;
	...
	declare readonly onChangeSet?: (this: TeamValidator, set: PokemonSet, format: Format, setHas?: AnyObject, teamHas?: AnyObject) => string[] | void;
	declare readonly onModifySpeciesPriority?: number;
	declare readonly onModifySpecies?: (this: Battle, species: Species, target?: Pokemon, source?: Pokemon, effect?: Effect) => Species | void;
	declare readonly onBattleStart?: (this: Battle) => void;
	declare readonly onTeamPreview?: (this: Battle) => void;
	declare readonly onChooseTeam?: (this: Battle, positions: number[], pokemon: Pokemon[], autoChoose?: boolean) => number[] | string | void;
	declare readonly onValidateSet?: (this: TeamValidator, set: PokemonSet, format: Format, setHas: AnyObject, teamHas: AnyObject) => string[] | void;
	declare readonly onValidateTeam?: (this: TeamValidator, team: PokemonSet[], format: Format, teamHas: AnyObject) => string[] | void;
...
function mergeFormatLists(main: FormatList, custom: FormatList | undefined): FormatList {
	...
	for (const element of main) {
		if (element.section) {
			...
			build.push(current);
		} else if ((element as FormatData).name) { current.formats.push((element as FormatData)); }
	}
	...
	if (custom !== undefined) {
		for (const element of custom) {
			...
			} else if ((element as FormatData).name) { current.formats.push(element as FormatData); } // otherwise, adds the element to its section.
		}
	}
	// builds the final result.
	for (const element of build) { result.push({ section: element.section, column: element.column }, ...element.formats); } // adds the section to the list.
	return result;
}
export class DexFormats {
	...
	load(): this {
		...
		try {
			...
			if (!Array.isArray(customFormats)) { throw new TypeError(`Exported property 'Formats' from "./config/custom-formats.ts" must be an array`); }
		...
		if (!Array.isArray(Formats)) { throw new TypeError(`Exported property 'Formats' from "./config/formats.ts" must be an array`); }
	...
	validate(name: string) {
		...
		const customRules = customRulesString.split(',').map(rule => {
			...
			if (typeof ruleSpec === 'string') {
				if (ruleSpec === '-pokemontag:allpokemon' || ruleSpec === '+pokemontag:allpokemon') { if (hasPokemonRule) throw new Error(`You can't ban/unban pokemon before banning/unbanning all Pokemon.`); }
	...
	// The default mode is `isTrusted = false`, which is a bit of a footgun. PS will never do anything unsafe, but `isTrusted = true` will throw if the format string is invalid, while `isTrusted = false` will silently fall back to the original format.
	get(name?: string | Format, isTrusted = false): Format {
		...
		if (this.dex.data.Rulesets.hasOwnProperty(DEFAULT_MOD + id)) { id = (DEFAULT_MOD + id) as ID; }
		...
		let effect;
		if (this.dex.data.Rulesets.hasOwnProperty(id)) { effect = new Format({ name, ...this.dex.data.Rulesets[id] as any, ...supplementaryAttributes }); } 
		else { effect = new Format({ id, name, exists: false }); }
		return effect;
	}
	all() {
		this.load();
	...
	getRuleTable(format: Format, depth = 1, repeals?: Map<string, number>): RuleTable {
		...
		if (format.name.length > 80) { throw new Error(`Format "${format.name}" has a name longer than 80 characters`); }
		if (depth === 1) {
			const dex = this.dex.mod(format.mod);
			if (dex !== this.dex) { return dex.formats.getRuleTable(format, 2, repeals); }
		}
		const ruleTable = new RuleTable();
		const ruleset = format.ruleset.slice();
		for (const ban of format.banlist) { ruleset.push('-' + ban); }
		for (const ban of format.restricted) { ruleset.push('*' + ban); }
		for (const ban of format.unbanlist) { ruleset.push('+' + ban); }
		if (format.customRules) { ruleset.push(...format.customRules); }
		if (format.checkCanLearn) { ruleTable.checkCanLearn = [format.checkCanLearn, format.name]; }
		if (format.onChooseTeam) { ruleTable.onChooseTeam = [format.onChooseTeam, format.name]; }
		// apply rule repeals before other rules repeals is a ruleid:depth map (positive: unused, negative: used)
		...
		for (let ruleSpec of ruleSpecs) {
			...
			if (typeof ruleSpec !== 'string') {
				...
				} else { throw new Error(`Unrecognized rule spec ${ruleSpec}`); }
				continue;
			}
			...
			if (ruleSpec.startsWith('!') && !ruleSpec.startsWith('!!')) {
				...
				if (repealDepth === depth && !noWarn) { throw new Error(`Rule "${ruleSpec}" did nothing because "${ruleSpec.slice(1)}" is not in effect`); }
				if (repealDepth === -depth) repeals!.delete(ruleSpec.slice(1));
				continue;
			}
			// individual ban/unban
			if ('+*-'.includes(ruleSpec.charAt(0))) {
				if (ruleTable.has(ruleSpec)) { throw new Error(`Rule "${ruleSpec}" in "${format.name}" already exists in "${ruleTable.get(ruleSpec) || format.name}"`); }
				if (skipPokemonBans) {
					if (ruleSpec === '-pokemontag:allpokemon' || ruleSpec === '+pokemontag:allpokemon') { skipPokemonBans--; } 
					else if (this.isPokemonRule(ruleSpec)) {
						if (!format.customRules) { throw new Error(`Rule "${ruleSpec}" must go after any "All Pokemon" rule in ${format.name} ("+All Pokemon" should go in ruleset, not unbanlist)`); }
						continue;
					}
				}
			...
			if (subformat.hasValue) {
				...
				if ((subformat.id === 'pickedteamsize' || subformat.id === 'evlimit') && value === 'Auto') { } // can't be resolved until later
				else if (subformat.hasValue === 'integer' || subformat.hasValue === 'positive-integer') {
					const intValue = parseInt(value);
					if (isNaN(intValue) || value !== `${intValue}`) { throw new Error(`In rule "${ruleSpec}", "${value}" must be an integer number.`);  }
				}
				if (subformat.hasValue === 'positive-integer') {
					if (parseInt(value) === 0) { throw new Error(`In rule "${ruleSpec}", "${value}" must be positive (to remove it, use the rule "! ${subformat.name}").`); }
				...
				if (oldValue === value) { if (!noWarn) { throw new Error(`Rule "${ruleSpec}" is redundant with existing rule "${subformat.id}=${value}"${ruleTable.blame(subformat.id)}.`); } } 
				else if (repealAndReplace) {
					if (oldValue === undefined) {
						if (subformat.mutuallyExclusiveWith && ruleTable.valueRules.has(subformat.mutuallyExclusiveWith)) {
							if (this.dex.formats.get(subformat.mutuallyExclusiveWith).ruleset.length) { throw new Error(`This format does not support "!!"`); }
							ruleTable.valueRules.delete(subformat.mutuallyExclusiveWith);
							ruleTable.delete(subformat.mutuallyExclusiveWith);
						} else { throw new Error(`Rule "${ruleSpec}" is not replacing anything (it should not have "!!")`); }
					}
				} else {
					if (oldValue !== undefined) { throw new Error(`Rule "${ruleSpec}" conflicts with "${subformat.id}=${oldValue}"${ruleTable.blame(subformat.id)} (Use "!! ${ruleSpec}" to override "${subformat.id}=${oldValue}".)`); }
					if (subformat.mutuallyExclusiveWith && ruleTable.valueRules.has(subformat.mutuallyExclusiveWith)) {
				...
				if (ruleTable.has(subformat.id) && !repealAndReplace && !noWarn) { throw new Error(`Rule "${ruleSpec}" in "${format.name}" already exists in "${ruleTable.get(subformat.id) || format.name}"`); }
			}
			ruleTable.set(subformat.id, '');
			if (depth > 16) { throw new Error(`Excessive ruleTable recursion in ${format.name}: ${ruleSpec} of ${format.ruleset}`); }
			...
			for (const [ruleid, sourceFormat] of subRuleTable) {
				...
				if (newValue !== undefined) {
					...
					if (subSubFormat.mutuallyExclusiveWith && ruleTable.valueRules.has(subSubFormat.mutuallyExclusiveWith)) { throw new Error(`Rule "${ruleid}=${newValue}" from ${subformat.name}${subRuleTable.blame(ruleid)} conflicts with "${subSubFormat.mutuallyExclusiveWith}=${ruleTable.valueRules.get(subSubFormat.mutuallyExclusiveWith)}"${ruleTable.blame(subSubFormat.mutuallyExclusiveWith)} (Repeal one with ! before adding another)`); }
					if (newValue !== oldValue) {
						if (oldValue !== undefined) { throw new Error(`Rule "${ruleid}=${newValue}" from ${subformat.name}${subRuleTable.blame(ruleid)} conflicts with "${ruleid}=${oldValue}"${ruleTable.blame(ruleid)} (Repeal one with ! before adding another)`); }
						ruleTable.valueRules.set(ruleid, newValue);
					}
				}
				ruleTable.set(ruleid, sourceFormat || subformat.name);
			}
			for (const [subRule, source, limit, bans] of subRuleTable.complexBans) { ruleTable.addComplexBan(subRule, source || subformat.name, limit, bans); }
			for (const [subRule, source, limit, bans] of subRuleTable.complexTeamBans) { ruleTable.addComplexTeamBan(subRule, source || subformat.name, limit, bans); }
	...
	validateRule(rule: string, format: Format | null = null) {
		...
		case '+':
			if (rule.slice(1).includes('>') || rule.slice(1).includes('+')) {
				...
				if (checkTeam) { return ['complexTeamBan', innerRule, '', limit, bans]; }
				if (bans.length > 1 || limit > 0) { return ['complexBan', innerRule, '', limit, bans]; }
		...
		default:
			...
			if (!ruleset.exists) { throw new Error(`Unrecognized rule "${rule}"`); }
	...
	validateBanRule(rule: string) {
		...
		for (const matchType of matchTypes) {
			...
			case 'pokemontag':
				// valid pokemontags
				const validTags = ['allpokemon', 'allitems', 'allmoves', 'allabilities', 'allnatures',];
				if (validTags.includes(ruleid) || this.validPokemonTag(ruleid)) { matches.push('pokemontag:' + ruleid); }
				continue;
			default:
			...
			if (table.hasOwnProperty(id)) {
				...
				if (table.hasOwnProperty(id)) { matches.push('pokemon:' + id); }
			}
		}
		if (matches.length > 1) { throw new Error(`More than one thing matches "${rule}"; please specify one of: ` + matches.join(', ')); }
		if (matches.length < 1) { throw new Error(`Nothing matches "${rule}"`); }
		return matches[0];
	}
}