 ...
 * Every chat room and battle is a room, and what they do is done in rooms.ts. There's also a global room which every user is in, and handles miscellaneous things like welcoming the user.
 * `Rooms.rooms` is the global table of all rooms, a `Map` of `RoomID:Room`.
 * Rooms should normally be accessed with `Rooms.get(roomid)`.
 * All rooms extend `BasicRoom`, whose important properties like `.users` and `.game` are documented near the the top of its class definition.
 * @license MIT
 */
...
interface UserTable { [userid: string]: User; }
...
export abstract class BasicRoom {
	...
	// Scrollback log. This is the log that's sent to users when joining the room. Should roughly match what's on everyone's screen.
	...
	/**
	 * The room's current battle. Battles are a type of RoomGame, so in battle rooms (which can only be `GameRoom`s), `this.battle === this.game`.
	 * In all other rooms, `this.battle` is `null`.
	 */
	battle: RoomBattle | null;
	/**
	 * The room's current best-of set. Best-of sets are a type of RoomGame, so in best-of set rooms (which can only be `GameRoom`s), `this.bestof === this.game`.
	 * In all other rooms, `this.bestof` is `null`.
	 */
	bestOf: BestOfGame | null;
	/**
	 * The game room's current tournament. If the room is a battle room whose battle is part of a tournament, `this.tour === this.parent.game`.
	...
	constructor(roomid: RoomID, title?: string, options: Partial<RoomSettings> = {}) {
		...
		if (options.parentid) { this.setParent(Rooms.get(options.parentid) || null); }
		this.subRooms = null;
		this.active = false;
		this.muteTimer = null;
		...
		if (Config.logchat) {
			...
			if (Config.loguserstats) { this.logUserStatsInterval = setInterval(() => this.logUserStats(), Config.loguserstats); }
		}
		this.userList = '';
		if (this.batchJoins) { this.userList = this.getUserList(); }
	...
	toString() { return this.roomid; }
	// Send a room message to all users in the room, without recording it in the scrollback log.
	send(message: string) {
		if (this.roomid !== 'lobby') message = '>' + this.roomid + '\n' + message;
		if (this.userCount) Sockets.roomBroadcast(this.roomid, message);
	}
	sendMods(data: string) { this.sendRankedUsers(data, '*'); }
	sendRankedUsers(data: string, minRank: GroupSymbol = '+') {
		...
		for (const i in this.users) {
			...
			if (user.isStaff || this.auth.atLeast(user, minRank)) { user.sendTo(this, data); }
		}
	}
	...
	// Add a room message to the room log, so it shows up in the room for everyone, and appears in the scrollback for new users who join.
	add(message: string) {
	...
	uhtmlchange(name: string, message: string) { this.log.uhtmlchange(name, message); }
	attributedUhtmlchange(user: User, name: string, message: string) { this.log.attributedUhtmlchange(user, name, message); }
	hideText(userids: ID[], lineCount = 0, hideRevealButton?: boolean) {
		const cleared = this.log.clearText(userids, lineCount);
		for (const userid of cleared) { this.send(`|hidelines|${hideRevealButton ? 'delete' : 'hide'}|${userid}|${lineCount}`); }
		this.update();
	}
	// Inserts (sanitized) HTML into the room log.
	addRaw(message: string) { return this.add('|raw|' + message); }
	// Inserts some text into the room log, attributed to user. The attribution will not appear, and is used solely as a hint not to highlight the user.
	addByUser(user: User, text: string): this { return this.add('|c|' + user.getIdentity(this) + '|/log ' + text); }
	// Like addByUser, but without logging
	sendByUser(user: User | null, text: string) { this.send('|c|' + (user ? user.getIdentity(this) : '~') + '|/log ' + text); }
	// Like addByUser, but sends to mods only.
	sendModsByUser(user: User, text: string) { this.sendMods('|c|' + user.getIdentity(this) + '|/log ' + text); }
	update() {
	...
	getUserList() {
		...
		let counter = 0;
		for (const i in this.users) {
			if (!this.users[i].named) { continue; }
			counter++;
	...
	getMuteTime(user: User): number | undefined {
		...
		if (!userid) return;
		for (const entry of this.muteQueue) { if (userid === entry.userid) { return entry.time - Date.now(); } }
	...
	checkModjoin(user: User) {
		...
		if (!Users.Auth.isAuthLevel(modjoinSetting)) { Monitor.error(`Invalid modjoin setting in ${this.roomid}: ${modjoinSetting}`); }
	...
	mute(user: User, setTime?: number) {
		...
		this.runMuteTimer();
		user.updateIdentity();
		if (!(this.settings.isPrivate === true || this.settings.isPersonal)) { void Punishments.monitorRoomPunishments(user); }
		return userid;
	}
		...
		for (const [i, entry] of this.muteQueue.entries()) {
			if (entry.userid === userid ||
				...
				if (i === 0) {
					...
					this.runMuteTimer(true);
				} else { this.muteQueue.splice(i, 1); }
	...
	logUserStats() {
		...
		for (const group of Config.groupsranking) { groups[group] = 0; }
		for (const i in this.users) {
			const user = this.users[i];
			++total;
			if (!user.named) { ++guests; }
			++groups[this.auth.get(user.id)];
		}
		let entry = `|userstats|total:${total}|guests:${guests}`;
		for (const i in groups) { entry += `|${i}:${groups[i]}`; }
		this.roomlog(entry);
	}
	pokeExpireTimer() {
		if (this.expireTimer) clearTimeout(this.expireTimer);
		if (this.settings.isPersonal) { this.expireTimer = setTimeout(() => this.expire(), TIMEOUT_INACTIVE_DEALLOCATE); } 
		else { this.expireTimer = null; }
	}
	expire() {
		this.send('|expire|');
		this.destroy();
	...
	getIntroMessage(user: User) {
		...
		if (this.settings.modchat) { message += ` [${this.settings.modchat} or higher to talk]`; }
		...
		if (this.settings.slowchat) { message += ` [Slowchat ${this.settings.slowchat}s]`; }
		message += `</div>`;
	...
	getStaffIntroMessage(user: User) {
		...
		if (this.pendingApprovals?.size) {
			...
			for (const [userid, entry] of this.pendingApprovals) {
				...
				} else { message += `<strong>Link:</strong><br /> <a href="${entry.link}"">Link</a><br />`; }
		...
		if (!this.settings.isPrivate && !this.settings.isPersonal && this.settings.modchat && this.settings.modchat !== 'autoconfirmed') { messages.push(`|raw|<div class="broadcast-red">Modchat currently set to ${this.settings.modchat}</div>`); }
	...
	validateTitle(newTitle: string, newID?: string, oldID?: string) {
		...
		if (newTitle.includes(',') || newTitle.includes('|')) { throw new Chat.ErrorMessage(`Room title "${newTitle}" can't contain any of: ,|`); }
		if ((!newID.includes('-') || newID.startsWith('groupchat-')) && newTitle.includes('-')) { throw new Chat.ErrorMessage(`Room title "${newTitle}" can't contain -`); }
	...
	setParent(room: Room | null) {
		...
		if (this.parent) {
			...
			if (!this.parent.subRooms!.size) { (this.parent.subRooms as any) = null; }
		}
		(this as any).parent = room;
		if (room) {
			if (!room.subRooms) { (room as any).subRooms = new Map(); }
			(room as any).subRooms.set(this.roomid, this);
			this.settings.parentid = room.roomid;
		} else { delete this.settings.parentid; }
		this.saveSettings();
		for (const userid in this.users) { this.users[userid].updateIdentity(this.roomid); }
	}
	clearSubRooms() {
		if (!this.subRooms) return;
		for (const room of this.subRooms.values()) { (room as any).parent = null; }
		(this as any).subRooms = null;
		// this doesn't update parentid or subroom user symbols because it's intended to be used for cleanup only
	}
	setPrivate(privacy: PrivacySetting) {
		...
		if (this.battle || this.bestOf) {
			if (privacy) {
				...
				// This is the same password generation approach as genPassword in the client replays.lib.php but obviously will not match given mt_rand there uses a different RNG and seed.
				let password = '';
		...
		if (this.game) {
			for (const player of this.game.players) { player.getUser()?.updateSearch(); }
		}
	}
	validateSection(section: string) {
		const target = toID(section);
		if (!RoomSections.sections.includes(target as any)) { throw new Chat.ErrorMessage(`"${target}" is not a valid room section. Valid categories include: ${RoomSections.sections.join(', ')}`); }
		return target as RoomSection;
	}
	setSection(section?: string) {
		if (!this.persist) { throw new Chat.ErrorMessage(`You cannot change the section of temporary rooms.`); }
		if (section) {
			const validatedSection = this.validateSection(section);
			if (this.settings.isPrivate && [true, 'hidden'].includes(this.settings.isPrivate)) { throw new Chat.ErrorMessage(`Only public rooms can change their section.`); }
			const oldSection = this.settings.section;
			if (oldSection === section) { throw new Chat.ErrorMessage(`${this.title}'s room section is already set to "${RoomSections.sectionNames[oldSection]}".`); }
	...
	warnParticipants(message: string) {
		...
		for (const user of warned) { user.popup(`|modal|${message}`); }
		return warned;
	}
	/**
	...
	rename(newTitle: string, newID?: RoomID, noAlias?: boolean) {
		...
		if (this.type === 'chat' && this.game) { throw new Chat.ErrorMessage(`Please finish your game (${this.game.title}) before renaming ${this.roomid}.`); }
		(this as any).roomid = newID;
		this.title = this.settings.title = newTitle;
		this.saveSettings();
		if (newID === oldID) {
			for (const user of Object.values(this.users)) { user.sendTo(this, `|title|${newTitle}`); }
			return;
		}
		...
		if (oldID === 'lobby') { Rooms.lobby = null; } 
		else if (newID === 'lobby') { Rooms.lobby = this as ChatRoom; }
		if (!noAlias) {
			for (const [alias, roomid] of Rooms.aliases.entries()) { if (roomid === oldID) { Rooms.aliases.set(alias, newID); } }
			...
			for (const [alias, roomid] of Rooms.aliases.entries()) { if (roomid === oldID) { Rooms.aliases.delete(alias); } }
	...
	onJoin(user: User, connection: Connection) {
		...
		if (user.named) { this.reportJoin('j', user.getIdentityWithStatus(this), user); }
		const staffIntro = this.getStaffIntroMessage(user);
	...
	onRename(user: User, oldid: ID, joining: boolean) {
		if (user.id === oldid) { return this.onUpdateIdentity(user); }
		if (!this.users[oldid]) { Monitor.crashlog(new Error(`user ${oldid} not in room ${this.roomid}`)); }
		if (this.users[user.id]) { Monitor.crashlog(new Error(`user ${user.id} already in room ${this.roomid}`)); }
		...
		if (joining) {
			...
			const staffIntro = this.getStaffIntroMessage(user);
			if (staffIntro) this.sendUser(user, staffIntro);
		} else if (!user.named) { this.reportJoin('l', ' ' + oldid, user); } 
		else { this.reportJoin('n', user.getIdentityWithStatus(this) + '|' + oldid, user); }
	...
	onUpdateIdentity(user: User) {
		if (user?.connected) {
			if (!this.users[user.id]) return false;
			if (user.named) { this.reportJoin('n', user.getIdentityWithStatus(this) + '|' + user.id, user); }
		}
		return true;
	}
	onLeave(user: User) {
		...
		this.userCount--;
		if (user.named) { this.reportJoin('l', user.getIdentity(this), user); }
	...
	runAutoModchat() {
		...
		if (!staff.length) {
			...
			if (!time || time < 5) { throw new Error(`Invalid time setting for automodchat (${Utils.visualize(this.settings.autoModchat)})`); }
			...
			this.modchatTimer = setTimeout(() => {
				...
				this.modlog({ action: 'AUTOMODCHAT ACTIVATE', });
	...
	checkAutoModchat(user: User) {
		if (user.can('mute', null, this, 'modchat')) {
			...
			if (this.settings.autoModchat?.active) {
				...
				if (typeof oldSetting === 'string') { this.settings.modchat = oldSetting; } 
				else { delete this.settings.modchat; }
	...
	destroy(): void {
		...
		if (this.settings.aliases) { for (const alias of this.settings.aliases) { Rooms.aliases.delete(alias); } }
		this.active = false;
		...
		this.reportJoinsInterval = null;
		if (this.logUserStatsInterval) { clearInterval(this.logUserStatsInterval); }
		this.logUserStatsInterval = null;
		void this.log.destroy();
		Rooms.rooms.delete(this.roomid);
		if (this.roomid === 'lobby') Rooms.lobby = null;
	}
	tr(strings: string | TemplateStringsArray, ...keys: any[]) { return Chat.tr(this.settings.language || 'english' as ID, strings, ...keys); }
}
export class GlobalRoomState {
	...
	constructor() {
		...
		for (const [i, settings] of this.settingsList.entries()) {
			...
			// We're okay with assinging type `ID` to `RoomID` here because the hyphens in chatrooms don't have any special
			// meaning, unlike in helptickets, groupchats, battles etc where they are used for shared modlogs and the like
			...
			if (room.settings.aliases) { for (const alias of room.settings.aliases) { Rooms.aliases.set(alias, id); } }

			this.chatRooms.push(room);
			if (room.settings.autojoin) {
				if (room.settings.modjoin) { this.modjoinedAutojoinList.push(id); } 
				else { this.autojoinList.push(id); }
			}
		}
		Rooms.lobby = Rooms.rooms.get('lobby') as ChatRoom;
		// init battle room logging
		if (Config.logladderip) { this.ladderIpLog = Monitor.logPath('ladderip/ladderip.txt').createAppendStream(); } 
		else { this.ladderIpLog = new Streams.WriteStream({ write() { return undefined; } }); } // Prevent there from being two possible hidden classes an instance of GlobalRoom can have.
		...
		this.formatList = '';
		let lastBattle;
		try { lastBattle = Monitor.logPath('lastbattle.txt').readSync('utf8'); } 
		catch {}
	...
	deserializeBattleRoom(battle: NonNullable<Awaited<ReturnType<GlobalRoomState['serializeBattleRoom']>>>) {
		...
		if (timer) { Object.assign(room.battle.timer.settings, timer); } // json blob of settings
	...
	async loadBattles() {
		...
		*/
		...
		for (const u of Users.users.values()) { u.send(`|pm|~|${u.getIdentity()}|/uhtmlchange restartmsg,`); }
	...
	modlog(entry: PartialModlogEntry, overrideID?: string) { void Rooms.Modlog.write('global', entry, overrideID); }
	writeChatRoomData() {
	...
	get configRankList() {
		...
		for (const rank in Config.punishgroups) { rankList.push({ symbol: Config.punishgroups[rank].symbol, name: Config.punishgroups[rank].name, type: 'punishment' }); }
	...
	sendAll(message: string) { Sockets.roomBroadcast('', message); }
	addChatRoom(title: string) {
		const id = toID(title) as RoomID;
		if (['battles', 'rooms', 'ladder', 'teambuilder', 'home', 'all', 'public'].includes(id)) { return false; }
	...
	prepBattleRoom(format: string) {
		...
		let roomid: RoomID;
		do { roomid = `${roomPrefix}${++battleNum}` as RoomID; } 
	...
	onCreateBattleRoom(players: User[], room: GameRoom, options: AnyObject) {
		for (const player of players) { if (player.statusType === 'idle') { player.setStatusType('online'); } }
		if (Config.reportbattles) {
			if (typeof Config.reportbattles === 'string') { Config.reportbattles = [Config.reportbattles]; } 
			else if (Config.reportbattles === true) { Config.reportbattles = ['lobby']; }
		...
		for (const player of players) { Chat.runHandlers('onBattleStart', player, room); }
	...
	deregisterChatRoom(id: string) {
		...
		// deregister from global settings looping from the end is a pretty trivial optimization, but the assumption is that more recently added rooms are more likely to be deleted
	...
	checkAutojoin(user: User, connection?: Connection) {
		...
		for (let [i, roomid] of this.modjoinedAutojoinList.entries()) {
			...
			if (room.checkModjoin(user)) { user.joinRoom(room.roomid, connection); }
		}
		for (const conn of user.connections) {
			if (conn.autojoins) {
				const autojoins = conn.autojoins.split(',') as RoomID[];
				for (const roomName of autojoins) { void user.tryJoinRoom(roomName, conn); }
				conn.autojoins = '';
			}
		}
	}
	...
	startLockdown(err: Error | null = null, slow = false) {
		...
		for (const [id, curRoom] of Rooms.rooms) {
			if (err) {
				if (id === 'staff' || id === 'development' || (!devRoom && id === 'lobby')) {
					...
					curRoom.update();
				} else { curRoom.addRaw(`<div class="broadcast-red"><b>The server needs to restart because of a crash.</b><br />No new battles can be started until the server is done restarting.</div>`).update(); }
			} else { curRoom.addRaw(`<div class="broadcast-red"><b>The server is restarting soon.</b><br />Please finish your battles quickly. No new battles can be started until the server resets in a few minutes.</div>`).update(); }
		...
		for (const user of Users.users.values()) { user.send(`|pm|~|${user.tempGroup}${user.name}|/raw <div class="broadcast-red"><b>The server is restarting soon.</b><br />Please finish your battles quickly. No new battles can be started until the server resets in a few minutes.</div>`); }
		this.lockdown = true;
		this.writeNumRooms();
	...
	automaticKillRequest() {
		...
		if (Config.autolockdown && Rooms.global.lockdown === true && Rooms.global.battleCount === 0) {
			...
			setTimeout(() => {
				if (Config.autolockdown && Rooms.global.lockdown === true) { process.exit(); } // finally kill the server
				else {
					this.notifyRooms(
						notifyPlaces,
	...
	reportCrash(err: Error | string, crasher = "The server") {
		const time = Date.now();
		if (time - this.lastReportedCrash < CRASH_REPORT_THROTTLE) { return; }
		...
		if (hasPrivateTerm) {
			...
			} else { crashMessage = `|html|<div class="broadcast-red"><b>${crasher} crashed in private code</b></div>`; }
		}
		const devRoom = Rooms.get('development');
		if (devRoom) { devRoom.add(crashMessage).update(); } 
		else {
			Rooms.lobby?.add(crashMessage).update();
			Rooms.get('staff')?.add(crashMessage).update();
		}
		if (privateCrashMessage) { upperStaffRoom!.add(privateCrashMessage).update(); }
	}
	/**
	...
	destroyPersonalRooms(userid: ID) {
		const roomauth = [];
		for (const [id, curRoom] of Rooms.rooms) {
			if (curRoom.settings.isPersonal && curRoom.auth.get(userid) === Users.HOST_SYMBOL) { curRoom.destroy(); } 
			else {
				if (curRoom.settings.isPrivate || curRoom.battle || !curRoom.persist) { continue; }
...
export class ChatRoom extends BasicRoom { // This is not actually used, this is just a fake class to keep TypeScript happy
...
export class GameRoom extends BasicRoom {
	 ...
	 */
	getLog(channel: -1 | 0 | 1 | 2 | 3 | 4 = 0) { return this.log.getScrollback(channel); }
	...
	override update(excludeUser: User | null = null) {
		if (!this.log.broadcastBuffer.length) return;
		if (this.userCount) { Sockets.channelBroadcast(this.roomid, `>${this.roomid}\n${this.log.broadcastBuffer.join('\n')}`); }
		this.log.broadcastBuffer = [];
		this.pokeExpireTimer();
	}
	...
	requestModchat(user: User | null) {
		if (!user) { this.modchatUser = ''; } 
		else if (!this.modchatUser || this.modchatUser === user.id || this.auth.get(user.id) !== Users.PLAYER_SYMBOL) { this.modchatUser = user.id; } 
		else { return "Modchat can only be changed by the user who turned it on, or by staff"; }
	}
	...
	/**
	 * Sends this room's replay to the connection to be uploaded to the replay server. To be clear, the replay goes:
	 * PS server -> user -> loginserver
	 * NOT: PS server -> loginserver
	 * That's why this function requires a connection. For details, see the top comment inside this function.
	 */
	async uploadReplay(user?: User, connection?: Connection, options?: 'forpunishment' | 'silent' | 'auto') {
		// The reason we don't upload directly to the loginserver, unlike every other interaction with the loginserver, is because it takes so much
		// bandwidth that it can get identified as a DoS attack by PHP, Apache, or Cloudflare, and blocked.
		// While I'm sure this is configurable, it's a huge pain, and getting it wrong, especially while migrating infrastructure, leads to everything
		// being unusable and panic while we figure out how to unblock our servers from each other. It's just easier to "spread out" the bandwidth.
		// TODO: My ideal long-term fix would be to just have a database (probably Postgres) shared between client and server, acting as both the server's
		// battle logs as well as the client's replay database, which both client and server have write access to.
		...
		const hidden = options === 'auto' ? 10 :
			...
			isPrivate ? 1 :
			0;
		if (isPrivate && hidden === 10) { password = (battle.password ||= Replays.generatePassword()); }
		if (battle.replaySaved !== true && hidden === 10) { battle.replaySaved = 'auto'; } 
		else { battle.replaySaved = true; }
		// If we have a direct connetion to a Replays database, just upload the replay directly.
		if (Replays.db) {
...
export const Rooms = {
	Modlog: mainModlog,
	// The main roomid:Room table. Please do not hold a reference to a room long-term; just store the roomid and grab it from here (with the Rooms.get(roomid) accessor) when necessary.
	rooms: new Map<RoomID, Room>(),
	aliases: new Map<string, RoomID>(),
	get: getRoom,
	search(name: string): Room | undefined { return getRoom(name) || getRoom(toID(name)) || getRoom(Rooms.aliases.get(toID(name))); },
	...
	createBattle(options: RoomBattleOptions & Partial<RoomSettings>) {
		...
		if (players.length > format.playerCount) { throw new Error(`${players.length} players were provided, but the format is a ${format.playerCount}-player format.`); }
		if (new Set(players).size < players.length) { throw new Error(`Players can't battle themselves`); }
		for (const user of players) { Ladders.cancelSearches(user); }
		...
		if (Rooms.global.lockdown === 'pre' && isBestOf && !options.isBestOfSubBattle) {
			for (const user of players) { user.popup(`The server will be restarting soon. Best-of-${isBestOf} battles cannot be started at this time.`); }
			return null;
		}
		// gotta allow new bo3 child battles to start
		if (Rooms.global.lockdown === true && !options.isBestOfSubBattle) {
			for (const user of players) { user.popup("The server is restarting. Battles will be available again in a few minutes."); }
			return null;
		}
		const p1Special = players.length ? players[0].battleSettings.special : undefined;
		let mismatch = `"${p1Special}"`;
		for (const user of players) {
			if (user.battleSettings.special !== p1Special) { mismatch += ` vs. "${user.battleSettings.special}"`; }
			user.battleSettings.special = undefined;
		}
		if (mismatch !== `"${p1Special}"`) {
			for (const user of players) { user.popup(`Your special battle settings don't match: ${mismatch}`); }
			return null;
		} else if (p1Special) { options.ratedMessage = p1Special; }
		...
		if (format.gameType === 'multi') { roomTitle = `Team ${p1name} vs. Team ${p2name}`; } 
		...
		} else if (options.title) { roomTitle = options.title; } 
		else { roomTitle = `${p1name} vs. ${p2name}`; }
		roomid ||= Rooms.global.prepBattleRoom(options.format);
		options.isPersonal = true;
		if (Rooms.rooms.has(roomid)) { return Rooms.rooms.get(roomid) as GameRoom; } // can happen if restoring a Bo3 game
		const room = Rooms.createGameRoom(roomid, roomTitle, options);
		let game: RoomBattle | BestOfGame;
		if (options.isBestOfSubBattle || !isBestOf) { game = new RoomBattle(room, options); } 
		else { game = new BestOfGame(room, options); }
		room.game = game;
		if (options.isBestOfSubBattle && room.parent) { room.setPrivate(room.parent.settings.isPrivate || false); } 
		else { game.checkPrivacySettings(options); }
