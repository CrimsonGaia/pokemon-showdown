...
export class BestOfPlayer extends RoomGamePlayer<BestOfGame> {
	...
	updateReadyButton() {
		...
		if (this.ready === false) {
			...
			if (battleRoom && user.inRooms.has(battleRoom.roomid)) { battleRoom.send(notification); } 
			else { this.sendRoom(notification); }
		} else {
...
export class BestOfGame extends RoomGame<BestOfPlayer> {
	...
	constructor(room: GameRoom, options: RoomBattleOptions) {
		...
		if (!toID(this.title).includes('bestof')) { this.title += ` (Best-of-${this.bestOf})`; }
		...
		for (const playerOpts of options.players) { this.addPlayer(playerOpts.user, playerOpts); }
	...
	override makePlayer(user: User | null, options: RoomBattlePlayerOptions): BestOfPlayer { return new BestOfPlayer(user, this, ++this.playerNum, options); }
	...
	checkPrivacySettings(options: RoomBattleOptions & Partial<RoomSettings>) {
		...
		for (const p of options.players) {
			if (p.user) {
				...
				} else if (p.hidden) { privacySetter.add(p.user.id); }
	...
	getOptions(): RoomBattleOptions | null {
		...
		if (players.some(p => !p.user)) { return null; }
		return {
			...this.options,
			players,
		};
	...
	updateDisplay() {
		...
		for (const p of this.players) {
			...
			for (let i = 0; i < this.bestOf; i++) {
				if (this.games[i]?.winner === p) { buf += `<i class="fa fa-circle"></i>`; } 
				else { buf += `<i class="fa fa-circle-o"></i>`; }
				if (i !== this.bestOf - 1) { buf += ` `; }
			}
			buf += `</td></tr>`;
		}
		buf += `</table><br /><br />`;
	...
	override onBattleWin(room: Room, winnerid: ID) {
		...
		if (winner) {
			...
			if (loserPlayer && loserPlayer.dcSecondsLeft <= 0) {  return this.forfeit(loserPlayer.name, ` lost the series due to inactivity.`); } // disconnection means opp wins the set
	...
	promptNextGame(room: Room) {
		...
		for (const player of this.players) {
			...
			if (dcAutoloseTime < this.nextBattleTimerEnd) { player.dcAutoloseTime = dcAutoloseTime; }
			player.updateReadyButton();
		}
	...
	pokeNextBattleTimer() {
		...
		if (Date.now() >= this.nextBattleTimerEnd) { return this.nextGame(); }
		for (const p of this.players) {
			if (!p.ready) {
				const now = Date.now() - 100; // fudge to make rounding work better
				if (p.dcAutoloseTime && now > p.dcAutoloseTime) { return this.forfeit(p.name, ` lost the series due to inactivity.`); }
	...
	confirmReady(user: User) {
		const player = this.playerTable[user.id];
		if (!player) { throw new Chat.ErrorMessage("You aren't a player in this best-of set."); }
		if (!this.waitingBattle) { throw new Chat.ErrorMessage("The battle is not currently waiting for ready confirmation."); }
		player.ready = true;
		player.updateReadyButton();
		...
		if (this.players.every(p => p.ready)) { this.nextGame(); }
	}
	override setEnded() {
		this.clearWaiting();
		super.setEnded();
		...
		const winner = winnerid ? this.playerTable[winnerid] : null;
		this.winner = winner;
		if (winner) { this.room.add(`|win|${winner.name}`); } 
