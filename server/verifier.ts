...
export const PM = new QueryProcessManager<{ data: string, signature: string }, boolean>(
	'verifier', module, ({ data, signature }) => {
		...
		let success = false;
		try { success = verifier.verify(Config.loginserverpublickey, signature, 'hex'); } catch {}
		return success;
	}
);
export function verify(data: string, signature: string): Promise<boolean> { return PM.query({ data, signature }); }
if (!PM.isParentProcess) {
	ConfigLoader.ensureLoaded();
	// eslint-disable-next-line no-eval
	PM.startRepl((cmd: string) => eval(cmd));
}
export function start(processCount: ConfigLoader.SubProcessesConfig) { PM.spawn(processCount['verifier'] ?? 1); }
