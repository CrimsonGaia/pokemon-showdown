...
const Items = {
  ...
  apicotberry: {
    ...
    onUpdate(pokemon) {
      if (pokemon.hp <= pokemon.maxhp * 2 / 3 || pokemon.hp <= pokemon.maxhp && (pokemon.ability1 === "gluttony" && pokemon.abilityState1.gluttony || pokemon.ability2 === "gluttony" && pokemon.abilityState2.gluttony)) {
        pokemon.eatItem();
      }
    },
    onEat(pokemon) {
  ...
  chestoberry: {
    ...
    belch: {
      effect: function(target) {
        if (target.status === "slp" || target.status === "drowsy") {
          target.cureStatus();
        }
      }
    },
    ...
    onUpdate(pokemon) {
      if (pokemon.status === "slp" || pokemon.status === "drowsy") {
        pokemon.eatItem();
      }
    },
    onEat(pokemon) {
  ...
  custapberry: {
    ...
    onFractionalPriority(priority, pokemon) {
      if (priority <= 0 && (pokemon.hp <= pokemon.maxhp / 3 || pokemon.hp <= pokemon.maxhp * 2 / 3 && (pokemon.ability1 === "gluttony" && pokemon.abilityState1.gluttony || pokemon.ability2 === "gluttony" && pokemon.abilityState2.gluttony))) {
        if (pokemon.eatItem()) {
  ...
  figyberry: {
    ...
    onUpdate(pokemon) {
      if (pokemon.hp <= pokemon.maxhp / 3 || pokemon.hp <= pokemon.maxhp * 2 / 3 && (pokemon.ability1 === "gluttony" && pokemon.abilityState1.gluttony || pokemon.ability2 === "gluttony" && pokemon.abilityState2.gluttony)) {
        pokemon.eatItem();
      }
    },
    ...
    onEat(pokemon) {
      ...
      if (types.includes("Dark") || types.includes("Fairy") || types.includes("Grass") || types.includes("Psychic")) {
  ...
  ganlonberry: {
    ...
    onUpdate(pokemon) {
      if (pokemon.hp <= pokemon.maxhp / 4 || pokemon.hp <= pokemon.maxhp / 2 && (pokemon.ability1 === "gluttony" && pokemon.abilityState1.gluttony || pokemon.ability2 === "gluttony" && pokemon.abilityState2.gluttony)) {
        pokemon.eatItem();
      }
    },
    onEat(pokemon) {
    ...
    belch: {
      effect: function(target) {
        if (this.randomChance(30, 100)) {
          target.trySetStatus("dragonblight");
        }
      }
    },
    naturalGift: {
  ...
  iapapaberry: {
    ...
    onEat(pokemon) {
      this.heal(pokemon.baseMaxhp / 3);
      const types = pokemon.getTypes();
      if (types.includes("Electric") || types.includes("Fairy") || types.includes("Ghost") || types.includes("Psychic")) {
        pokemon.addVolatile("confusion");
      }
    },
    num: 163,
  ...
  jabocaberry: {
    ...
    onDamagingHit(damage, target, source, move) {
      if (move.category === "Physical" && source.hp && source.isActive && !(source.ability1 === "magicguard" || source.ability2 === "magicguard")) {
        if (target.eatItem()) {
  ...
  kebiaberry: {
    ...
    belch: {
      effect(target) {
        ...
        } else if (this.randomChance(20, 100)) {
  ...
  lansatberry: {
    ...
    belch: { effect(target, source, move) {
    ...
    onUpdate(pokemon) {
      if (!pokemon.volatiles["luckeffect"]) {
        pokemon.addVolatile("luckeffect");
      }
      if (pokemon.hp <= pokemon.maxhp * 2 / 3 || pokemon.hp <= pokemon.maxhp && (pokemon.ability1 === "gluttony" && pokemon.abilityState1.gluttony || pokemon.ability2 === "gluttony" && pokemon.abilityState2.gluttony)) {
        pokemon.eatItem();
      }
  ...
  marangaberry: {
    ...
    onSourceModifyDamage(damage, source, target, move) {
      if (move.category === "Special" && target.hp && target.isActive) {
        if (target.eatItem()) {
  ...
  oranberry: {
    ...
    isFragile: true,
    onFragileBreak(pokemon) {
      this.heal(25, pokemon);
    },
    belch: {
      status: "psn",
      chance: 75,
      effect(target, source, move) {
        if (target && target.hp > 0) target.heal(10);
      }
    },
    naturalGift: {
      basePower: 80,
  ...
  pechaberry: {
    ...
    isFragile: true,
    onFragileBreak(pokemon) {
      if (pokemon.status === "psn" || pokemon.status === "tox") pokemon.cureStatus();
    },
    belch: { effect: function(target) {
      ...
      effect(target, source, move) {
        if (move) {
          ...
          move.onEffectiveness = function(typeMod, type, move_, target_) {
            if (target_ && typeof target_ === "object" && "hasType" in target_ && typeof target_.hasType === "function" && target_.hasType("Dragon")) {
              return 1;
            }
            if (typeof origOnEffectiveness === "function") {
              return origOnEffectiveness.call(this, typeMod, type, move_, target_);
            }
            return typeMod;
          };
        }
    ...
    onFragileBreak(pokemon) {
      ...
      for (const hazard of hazards) {
        if (pokemon.side.removeSideCondition(hazard)) {
          this.add("-message", `Rowap pods broke loose and spun the ${this.dex.conditions.get(hazard).name} away!`);
        }
      }
    },
    belch: { effect(target) {
      if (target && target.status === "aura") {
        target.cureStatus();
      }
    } },
    naturalGift: {
    ...
    belch: {
      effect(target, source, move) {
        ...
        if (move && move.type === "Poison" && source && !source.volatiles["spelonberrybelch"]) {
          ...
          move.onTryImmunity = function(target2, source2, move_) {
            if (move_ && move_.type === "Poison" && target2.hasType && target2.hasType("Steel") && target2.volatiles["spelonberrybelch"]) {
              return false;
            }
            if (typeof origOnTryImmunity === "function") {
              return origOnTryImmunity.call(this, target2, source2, move_);
            }
            return void 0;
          };
        }
