...
const Moves = {
  ...
  avalanche: {
    ...
    basePowerCallback(pokemon, target, move) {
      ...
      if (pokemon.battle.field.isWeather("snow")) {
        bp = 90;
      }
  ...
  beatup: {
    ...
    basePowerCallback(pokemon, target, move) {
      ...
      if (move.hit > 1 && move.flags["contact"]) {
  ...
  drainpunch: {
    ...
    priority: 0,
    critRatio: 2,
    flags: { drain: 1, contact: 1, protect: 1, mirror: 1, metronome: 1 },
    drain: [1, 2],
    secondary: null,
    target: "normal"
  },
  ...
  furycutter: {
    ...
    basePower: 10,
    basePowerCallback(pokemon) {
      return Math.min(100, 10 + 6 * pokemon.timesAttacked);
    },
    type: "Bug",
    category: "Physical",
    name: "Fury Cutter",
  ...
  highjumpkick: {
    ...
    onMoveFail(target, source, move) {
      this.damage(Math.floor(source.baseMaxhp / 3), source, source, this.dex.conditions.get("High Jump Kick"));
    },
    secondary: null,
    target: "normal"
  },
  ...
  jumpkick: {
    ...
    hasCrashDamage: true,
    onMoveFail(target, source, move) {
      this.damage(Math.floor(source.baseMaxhp / 8), source, source, this.dex.conditions.get("Jump Kick"));
    },
    secondary: null,
    target: "normal"
  },
  ...
  noxioustorque: {
    ...
    flags: { crash: 1, protect: 1, failencore: 1, failmefirst: 1, nosleeptalk: 1, noassist: 1, failcopycat: 1, failmimic: 1, failinstruct: 1, nosketch: 1 },
    onHit(target, source, move) {
      source.battle.field.setTerrain("toxicterrain");
    },
    target: "normal"
  },
  nuzzle: {
  ...
  rockthrow: {
    ...
    onBasePower(basePower, source, target, move) {
      ...
      if (foeActive.length && foeActive.every((p) => !p.isGrounded())) {
        ...
        move.airborneSpread = true;
        return this.chainModify(2);
      }
    },
    onTry(source, target, move) {
      if (move.airborneSpread) {
        move.target = "allAdjacentFoes";
      }
    }
  },
  ...
  sacredsword: {
    ...
    ignoreDefensive: true,
    onHit(target, source) {
      if (target.status === "fear") target.cureStatus();
    },
    secondary: null,
    target: "normal"
  },
  ...
  spiritshackle: {
    ...
    pierce3: true,
    secondaries: [
      { chance: 100, onHit(target, source, move) {
  ...
  throatchop: {
    ...
    secondary: { chance: 100, onHit(target) {
  ...
  vcreate: {
    ...
    onAfterMoveSecondarySelf(source, target, move) {
      if (target.volatiles["protect"] || target.volatiles["banefulbunker"] || target.volatiles["kingsshield"] || target.volatiles["spikyshield"] || target.volatiles["burningbulwark"] || target.volatiles["silktrap"] || target.volatiles["obstruct"]) {
        this.boost({ spe: -1, def: -1, spd: -1 }, source, source, move);
      }
    },
    secondary: null,
    target: "normal"
  ...
  vinewhip: {
    ...
    secondary: null,
    onBasePower(basePower, pokemon) {
      if (pokemon.hasAbility && pokemon.hasAbility("overgrow")) {
        return this.chainModify(2);
      }
    },
    target: "normal"
  ...
  aurasphere: {
    ...
    onBasePower(basePower, pokemon, target, move) {
      if (target.status === "aura" || target.volatiles["aura"]) {
        return this.chainModify(1.5);
      }
    },
    target: "any"
  ...
  aurawheel: {
    ...
    onModifyMove(move, pokemon) {
      if (pokemon.species.name === "Morpeko-Hangry") {
        move.secondary = { chance: 10, status: "fear", self: { boosts: { spe: 1 } } };
      } else {
        move.secondary = { chance: 10, status: "par", self: { boosts: { spe: 1 } } };
      }
    },
    target: "normal"
  },
      ...
      if (item && item.belch) {
        ...
        if (belch.status) {
          ...
          if (this.randomChance(chance, 100)) {
            target.trySetStatus(belch.status, source, move);
          }
        }
        if (belch.volatileStatus) {
          const chance = belch.chance ?? 100;
          if (this.randomChance(chance, 100)) {
            target.addVolatile(belch.volatileStatus, source, move);
          }
        }
        if (typeof belch.effect === "function") {
          belch.effect.call(this, target, source, move);
        }
      }
    },
    secondary: null,
  ...
  bleakwindstorm: {
    ...
    onModifyMove(move, pokemon, target) {
      if (target && ["raindance", "primordialsea", "turbulentwinds"].includes(target.effectiveWeather())) {
        move.accuracy = true;
      }
    },
  ...
  burningjealousy: {
    ...
    critRatio: 5,
    flags: { protect: 1, mirror: 1, metronome: 1 },
    secondary: { chance: 100, onHit(target, source, move) {
  ...
  chargebeam: {
    ...
    onAfterMoveSecondarySelf(pokemon, target, move) {
      if (move && move.name === "Charge Beam" && pokemon.boosts.spa > pokemon.boosts.spa - 1) {
  ...
  dracometeor: {
    ...
    basePower: 130,
    onBasePower(basePower, pokemon, target, move) {
      if (pokemon.battle.field.getPseudoWeather("gravity")) {
        return this.chainModify(1.2);
      }
    },
    type: "Dragon",
    ...
    secondary: {
      ...
      onHit(target, source, move) {
        if (target.hasType("Dragon")) {
          target.setStatus("dragonblight", source);
        } else if (target.hasType("Electric")) {
        ...
        } else if (target.hasType("Fire")) {
          target.setStatus("brn", source);
        } else if (target.hasType("Ghost")) {
          target.addVolatile("curse", source);
        } else if (target.hasType("Ice")) {
          target.setStatus("frostbite", source);
        } else if (target.hasType("Normal")) {
          target.setStatus("drowsy", source);
        } else if (target.hasType("Poison")) {
          target.setStatus("psn", source);
        } else if (target.hasType("Psychic")) {
  ...
  firespin: {
    ...
    condition: {
      ...
      onEnd(pokemon) {
        this.add("-end", pokemon, "Fire Spin", "[partiallytrapped]");
      },
  ...
  judgement: {
    ...
    onModifyMove(move, pokemon) {
      if (move.type === "Bug" || move.type === "Dark" || move.type === "Dragon" || move.type === "Rock") {
        move.critRatio = (move.critRatio || 1) + 1;
      } else if (move.type === "Ground" || move.type === "Normal" || move.type === "Psychic" || move.type === "Rock") {
        move.critRatio = (move.critRatio || 1) + 1;
      }
    },
    secondary: null,
  ...
  revelationdance: {
    ...
    onModifyMove(move, pokemon) {
      if (pokemon.hasType(["Bug", "Dark", "Dragon", "Rock"])) {
        move.critRatio = (move.critRatio || 1) + 1;
      } else if (pokemon.hasType(["Ground", "Normal", "Psychic", "Water"])) {
        move.critRatio = Math.max((move.critRatio || 1) - 1, 0);
      }
    },
    secondary: null,
    target: "normal"
  ...
  springtidestorm: {
    ...
    accuracy: 80,
    onTryHit(target, source, move) {
      if (this.field.isTerrain("mistyterrain") || this.field.pseudoWeather["turbulentwinds"]) {
        move.accuracy = true;
      }
    },
    basePower: 100,
  ...
  surf: {
    ...
    onModifyType(move, pokemon, target) {
      if (pokemon.battle.field.isTerrain("toxicterrain")) {
        move.type2 = "Poison";
      }
    }
  },
  ...
  terablast: {
    ...
    onModifyMove(move, pokemon) {
      ...
      if (pokemon.hasType(["Bug", "Dark", "Dragon", "Rock", "Stellar"])) {
        move.critRatio = 5;
      } else if (pokemon.hasType(["Ground", "Normal", "Psychic", "Water", "banal"])) {
        move.critRatio = 3;
      } else {
        move.critRatio = 4;
      }
    ...
    condition: {
      ...
      onBeforeMove(pokemon) {
        if (pokemon.volatiles["partiallytrapped"]) {
          ...
          pokemon.removeVolatile("whirlpoolsuction");
        }
      }
    },
    target: "allAdjacentFoes"
  ...
  courtchange: {
    ...
    onHitField(target, source) {
      const sideConditions = ["mist", "lightscreen", "reflect", "spikes", "safeguard", "tailwind", "toxicspikes", "stealthrock", "waterpledge", "firepledge", "grasspledge", "stickyweb", "auroraveil", "luckychant", "steelspikes", "gmaxcannonade", "gmaxvinelash", "gmaxwildfire", "gmaxvolcalith"];
      let success = false;
    ...
    onHit(target, source, move) {
      if (target.status === "drowsy" && target.statusState.time) {
        ...
        if (wasAtMinimum || target.statusState.time <= 0) {
          ...
          this.add("-message", `${target.name}'s drowsiness is worsening!`);
        }
      }
    },
    secondary: null,
    ...
    onHit(target, source, move) {
      ...
      if (!target.getAbility().flags["failroleplay"]) {
        for (const pokemon of source.alliesAndSelf()) {
          ...
          if (oldAbility) {
            success = true;
            if (target.status && !pokemon.status) {
              pokemon.setStatus(target.status, source, move);
            }
            let i;
            ...
            for (const volatile in target.volatiles) {
              ...
              if (pokemon.volatiles[volatile]) {
                Object.assign(pokemon.volatiles[volatile], target.volatiles[volatile]);
              }
            }
  ...
  doubleteam: {
    ...
    condition: {
      duration: 2,
      onStart(pokemon) {
        this.add("-start", pokemon, "Double Team");
      },
  ...
  entrainment: {
    ...
    onTryHit(target, source) {
      ...
      if (target.ability1 === source.ability1 && target.ability2 === source.ability2 || target.getAbility().flags["cantsuppress"] || target.hasAbility("truant") || source.getAbility().flags["noentrain"]) {
        return false;
      }
    },
    onHit(target, source) {
      const oldAbility = target.setAbility(source.ability1, source, void 0, false, false, 1);
      if (source.ability2) target.setAbility(source.ability2, source, void 0, false, false, 2);
  ...
  fairylock: {
    ...
    condition: {
      ...
      onEffectiveness(typeMod, target, type, move) {
        if (target && target.side && target.side.getSideCondition("fairylock")) {
          if (typeMod < 0 && target.hasType(["Dark", "Dragon", "Ghost"])) {
            return 0;
          }
        }
      },
      ...
      onResidual(pokemon) {
        if (!pokemon.volatiles["fairylockfree"]) {
          if (pokemon.hasType(["Dark", "Dragon", "Ghost"])) {
            this.damage(pokemon.baseMaxhp / 12, pokemon);
          }
        }
      },
      onSideResidualOrder: 26,
      onSideResidual(side) {
        const freePokemon = side.active.find((p) => p && p.volatiles["fairylockfree"]);
        if (freePokemon) {
          const allies = side.active.filter((p) => p && p !== freePokemon && !p.fainted);
          if (allies.length > 0) {
            this.add("-message", `${freePokemon.name} unshackled their ally ${allies[0].name}.`);
          }
      ...
      onSideEnd(side) {
        ...
        for (const pokemon of side.pokemon) {
          pokemon.removeVolatile("fairylockfree");
        }
      }
    },
    secondary: null,
  ...
  focusenergy: {
    ...
    condition: {
      ...
      onHit(target, source, move) {
        if (target.volatiles["focuspunch"] || target.volatiles["beakblast"] || target.volatiles["shelltrap"] || target.volatiles["twoturnmove"]) {
          return null;
        }
      },
      onEnd(pokemon) {
  ...
  harden: {
    ...
    condition: {
      onStart(target) {
        this.add("-start", target, "move: Harden");
      },
      onSourceModifyDamage(damage, source, target, move) {
        if (move.flags["sweep"]) {
          return this.chainModify(2);
      ...
      onEnd(target) {
        this.add("-end", target, "move: Harden");
      }
    },
    secondary: null,
    target: "self"
  ...
  magiccoat: {
    ...
    condition: {
      ...
      onTryHitPriority: 2,
      onTryHit(target, source, move) {
        if (target === source || move.hasBounced || !move.flags["reflectable"] && !move.flags["light"] || target.isSemiInvulnerable()) {
          return;
        }
      ...
      onAllyTryHitSide(target, source, move) {
        if (target.isAlly(source) || move.hasBounced || !move.flags["reflectable"] && !move.flags["light"] || target.isSemiInvulnerable()) {
          return;
        }
        const newMove = this.dex.getActiveMove(move.id);
  ...
  metronome: {
    ...
    onHit(pokemon) {
      const moves = this.dex.moves.all().filter((move2) => (!move2.isNonstandard || move2.isNonstandard === "Unobtainable") && move2.flags["metronome"]);
      let randomMove = "";
      if (moves.length) {
  ...
  mirrorshield: {
    ...
    condition: {
      duration: 1,
      onStart(target) {
        this.add("-singleturn", target, "Mirror Shield");
      },
      onTryHitPriority: 3,
      onTryHit(target, source, move) {
        if (!move.flags["protect"]) {
        ...
        if (move.flags["beam"] || move.flags["light"]) {
          ...
          const lockedmove2 = source.getVolatile("lockedmove");
          if (lockedmove2) {
            if (source.volatiles["lockedmove"].duration === 2) {
              delete source.volatiles["lockedmove"];
            }
  ...
  nightmare: {
    ...
    condition: {
      ...
      onEnd(pokemon) {
        if (pokemon.status !== "slp" && !pokemon.hasAbility("comatose")) {
          pokemon.trySetStatus("fear", pokemon);
        }
      }
    },
    secondary: null,
  ...
  reaperseal: {
    ...
    condition: {
      duration: 3,
      onStart(pokemon, source) {
        this.add("-activate", pokemon, "move: Reaper Seal", "[of] " + source);
      },
      onResidualOrder: 13,
      onEnd(pokemon) {
        this.add("-end", pokemon, "Reaper Seal", "[partiallytrapped]");
      },
      onTrapPokemon(pokemon) {
        pokemon.tryTrap();
      }
  ...
  roleplay: {
    ...
    onHit(target, source) {
      const oldAbility = source.setAbility(target.ability1, target, void 0, false, false, 1);
      if (target.ability2) source.setAbility(target.ability2, target, void 0, false, false, 2);
      if (!oldAbility) return oldAbility;
    },
    secondary: null,
    target: "normal"
  ...
  simplebeam: {
    ...
    onTryHit(target) {
      if (target.getAbility().flags["cantsuppress"] || target.hasAbility("simple") || target.hasAbility("truant")) {
        return false;
      }
    },
    onHit(target, source) {
  ...
  skillswap: {
    ...
    onHit(target, source, move) {
      ...
      if (target.isAlly(source)) {
        ...
        this.add("-activate", source, "move: Skill Swap", targetAbility1, sourceAbility1, `[of] ${target}`);
      }
  ...
  stockpile: {
    ...
    flags: { snatch: 1, metronome: 1 },
    onTry(source) {
      if (source.volatiles["stockpile"] && source.volatiles["stockpile"].layers >= 6) return false;
    },
  ...
  telekinesis: {
    ...
    onAfterMove(source, target, move) {
      ...
      if (terrain && terrain.id === "psychicterrain") {
        terrain.boostedpsyparticle = true;
  ...
  toxicthread: {
    ...
    condition: {
      onSideStart(side) {
        this.add("-sidestart", side, "move: Toxic Thread");
      },
      onSwitchIn(pokemon) {
        if (!pokemon.isGrounded()) return;
        ...
        if (this.field.isTerrain("toxicterrain")) {
          pokemon.trySetStatus("tox", pokemon.side.foe.active[0]);
        } else {
          pokemon.trySetStatus("psn", pokemon.side.foe.active[0]);
        }
  ...
  worryseed: {
    ...
    flags: { protect: 1, reflectable: 1, mirror: 1, allyanim: 1, metronome: 1 },
    onTryImmunity(target) {
      if (target.hasAbility("truant") || target.hasAbility("insomnia")) {
        return false;
      }
    },
    onTryHit(target) {
  ...
  gmaxsteelsurge: {
    ...
    self: { onHit(source) {
      for (const side of source.side.foeSidesWithConditions()) {
        side.addSideCondition("steelspikes");
      }
    } },
    condition: {
      onSideStart(side) {
  ...
  luckychant: {
    ...
    condition: {
      ...
      // "The Lucky Chant shielded [side.name]'s team from critical hits!"
      onCriticalHit: false,
      onSideResidualOrder: 26,
      ...
      // "[side.name]'s team's Lucky Chant wore off!"
    },
    secondary: null,
    target: "allySide"
  },
