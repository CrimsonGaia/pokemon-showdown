...
if(loading){loading.then(function(){_this.initConnection();});}else
{this.initConnection();}
}var _proto=PSConnection.prototype;_proto.
initConnection=function initConnection(){if(!this.tryConnectInWorker())this.directConnect();};_proto.
canReconnect=function canReconnect(){
var uptime=Date.now()-PS.startTime;
if(uptime>24*60*60*1000){
PS.confirm("It's been over a day since you first connected. Please refresh.",{okButton:'Refresh'}).then(function(confirmed){var _PS$room;if(confirmed)(_PS$room=PS.room)==null||_PS$room.send("/refresh");});
return false;
}
...
catch(_unused2){this.socket=new WebSocket(url.replace('http','ws')+'/websocket');}
...
socket.onmessage=function(ev){PS.receive(''+ev.data);};
...
if(this.worker){this.worker.postMessage({type:'send',data:msg});}else
...
PS.isOffline=false;
if(!PS.connection){PS.connection=new PSConnection();}else
...
if(Config.testclient){return;}else
if(location.protocol+"//"+location.hostname===PSStorage.origin){var _Config;
...
this.loaded=new Promise(function(resolve){_this5.loader=resolve;});
return this.loaded;
};PSStorage.
...
try{return _PSStorage.frame.postMessage(data,_PSStorage.origin);}
catch(_unused6){}
return false;
};
;
...
query=function query(act){var data=arguments.length>1&&arguments[1]!==undefined?arguments[1]:{};return this.rawQuery(act,data).then(function(res){return res?JSON.parse(res.slice(1)):null;})["catch"](function(){return null;});};return _class;}())(
);var
...
function NetRequest(uri){this.uri=void 0;this.uri=uri;}var _proto3=NetRequest.prototype;_proto3.
