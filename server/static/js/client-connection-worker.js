...
if(socket&&socket.readyState===WebSocket.OPEN){socket.send(data);}else
{queue.push(data);}
...
catch(_unused){socket=new WebSocket(url.replace('http','ws')+'/websocket');}
if(socket){
...
socket.onmessage=function(e){postMessage({type:'message',data:e.data});};
socket.onclose=function(){postMessage({type:'disconnected'});};
