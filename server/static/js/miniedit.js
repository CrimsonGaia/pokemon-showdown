...
function MiniEdit(el,options){var _this=this;this.element=void 0;this._setContent=void 0;this.pushHistory=void 0;this.onKeyDown=function(ev){if(ev.keyCode===13){_this.replaceSelection('\n');ev.preventDefault();}};
this.element=el;
...
this.element.addEventListener('input',function(){_this.reformat();});
...
if(node.nodeType===3){if(callback(node))return true;}else
{for(var i=0,len=node.childNodes.length;i<len;++i){if(this.traverseText(node.childNodes[i],callback))return true;}}
return false;
};_proto.
...
reformat=function reformat(selection){this.setValue(this.getValue(),selection);};_proto.
...
select=function select(){this.setSelection({start:0,end:this.getValue().length});};return MiniEdit;}();MiniEdit.plugins=[];var
