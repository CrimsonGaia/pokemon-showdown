...
all:function(){
var moves=[];
var searchIndex=window.BattleSearchIndex||[];for(var _i2=0;_i2<
searchIndex.length;_i2++){var entry=searchIndex[_i2];
if(entry[1]==='move')moves.push(_this.moves.get(entry[0]));
}
return moves;
}
};this.
...
name=BattleAliases[id];
id=toID(name);
}else if(window.BattlePokedex&&!(id in BattlePokedex)&&window.BattleBaseSpeciesChart){for(var _i4=0,_BattleBaseSpeciesCha2=
BattleBaseSpeciesChart;_i4<_BattleBaseSpeciesCha2.length;_i4++){var baseSpeciesId=_BattleBaseSpeciesCha2[_i4];
...
if(species.cosmeticFormes){for(var _i6=0,_species$cosmeticForm2=
species.cosmeticFormes;_i6<_species$cosmeticForm2.length;_i6++){var forme=_species$cosmeticForm2[_i6];
...
statusNames=['par','psn','tox','brn','frz','slp','aura','bubbleblight','dragonblight','fear','frostbite','drowsy','confusion','flinch'];this.
...
fieldEffectNames=['sun','sunnyday','harshsunshine','desolateland','rain','raindance','heavyrain','primordialsea','sand','sandstorm','hail','snow','snowscape','fog','strongwinds','deltastream','electricterrain','grassyterrain','mistyterrain','psychicterrain'];}var _proto2=_class2.prototype;_proto2.mod=function mod(modid){if(modid==='gen9')return this;if(!window.BattleTeambuilderTable)return this;if(modid in this.moddedDexes){return this.moddedDexes[modid];}this.moddedDexes[modid]=new ModdedDex(modid);return this.moddedDexes[modid];};_proto2.forGen=function forGen(gen){if(!gen)return this;return this.mod("gen"+gen);};_proto2.formatGen=function formatGen(format){var formatid=toID(format);if(!formatid)return Dex.gen;if(!formatid.startsWith('gen'))return 6;return parseInt(formatid.charAt(3))||Dex.gen;};_proto2.forFormat=function forFormat(format){var dex=Dex.forGen(Dex.formatGen(format));var formatid=toID(format).slice(4);if(dex.gen===7&&formatid.includes('letsgo')){dex=Dex.mod('gen7letsgo');}if(dex.gen===8&&formatid.includes('bdsp')){dex=Dex.mod('gen8bdsp');}if(dex.gen===9&&(formatid.includes('indigostarstorm')||formatid.includes('isl'))){dex=Dex.mod('gen9indigostarstorm');}return dex;};_proto2.resolveAvatar=function resolveAvatar(avatar){var _window$Config;if(window.BattleAvatarNumbers&&avatar in BattleAvatarNumbers){avatar=BattleAvatarNumbers[avatar];}if(avatar.startsWith('#')){return Dex.resourcePrefix+'sprites/trainers-custom/'+toID(avatar.substr(1))+'.png';}if(avatar.includes('.')&&(_window$Config=window.Config)!=null&&(_window$Config=_window$Config.server)!=null&&_window$Config.registered){var protocol=Config.server.port===443?'https':'http';var server=protocol+"://"+Config.server.host+":"+Config.server.port;return server+"/avatars/"+encodeURIComponent(avatar).replace(/%3F/g,'?');}return Dex.resourcePrefix+'sprites/trainers/'+Dex.sanitizeName(avatar||'unknown')+'.png';};_proto2.sanitizeName=function sanitizeName(name){if(!name)return'';return(''+name).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').slice(0,50);};_proto2.prefs=function prefs(prop){var _window$Storage,_window$PS;return(_window$Storage=window.Storage)!=null&&_window$Storage.prefs?window.Storage.prefs(prop):(_window$PS=window.PS)==null||(_window$PS=_window$PS.prefs)==null?void 0:_window$PS[prop];};_proto2.getShortName=function getShortName(name){var shortName=name.replace(/[^A-Za-z0-9]+$/,'');if(shortName.includes('(')){shortName+=name.slice(shortName.length).replace(/[^()]+/g,'').replace(/\(\)/g,'');}return shortName;};_proto2.getEffect=function getEffect(name){name=(name||'').trim();if(name.substr(0,5)==='item:'){return Dex.items.get(name.substr(5).trim());}else if(name.substr(0,8)==='ability:'){return Dex.abilities.get(name.substr(8).trim());}else if(name.substr(0,5)==='move:'){return Dex.moves.get(name.substr(5).trim());}var id=toID(name);return new PureEffect(id,name);};_proto2.getGen3Category=function getGen3Category(type){return['Fire','Water','Grass','Electric','Ice','Psychic','Dark','Dragon'].includes(type)?'Special':'Physical';};_proto2.hasAbility=function hasAbility(species,ability){for(var i in species.abilities){if(ability===species.abilities[i])return true;}return false;};_proto2.loadSpriteData=function loadSpriteData(gen){if(this.loadedSpriteData[gen])return;this.loadedSpriteData[gen]=1;var path=$('script[src*="pokedex-mini.js"]').attr('src')||'';var qs='?'+(path.split('?')[1]||'');path=(/.+?(?=data\/pokedex-mini\.js)/.exec(path)||[])[0]||'';var el=document.createElement('script');el.src=path+'data/pokedex-mini-bw.js'+qs;document.getElementsByTagName('body')[0].appendChild(el);};_proto2.getSpriteData=function getSpriteData(pokemon,isFront){var options=arguments.length>2&&arguments[2]!==undefined?arguments[2]:{gen:6};var mechanicsGen=options.gen||6;if(pokemon instanceof Pokemon){if(pokemon.volatiles.transform){options.shiny=pokemon.volatiles.transform[2];options.gender=pokemon.volatiles.transform[3];}else{options.shiny=pokemon.shiny;options.gender=pokemon.gender;}pokemon=pokemon.getSpeciesForme();}var species=Dex.species.get(pokemon);var spriteData={gen:mechanicsGen,w:96,h:96,y:0,url:Dex.resourcePrefix+'sprites/',pixelated:true,isFrontSprite:false,cryurl:'',shiny:options.shiny};var name=species.spriteid;var dir;var facing;if(isFront){spriteData.isFrontSprite=true;dir='';facing='front';}else{dir='-back';facing='back';}var graphicsGen=mechanicsGen;if(Dex.prefs('nopastgens'))graphicsGen=6;if(Dex.prefs('bwgfx')&&graphicsGen>=6)graphicsGen=5;spriteData.gen=Math.max(graphicsGen,Math.min(species.gen,5));var baseDir=['','gen1','gen2','gen3','gen4','gen5','','','',''][spriteData.gen];var miscData=null;var speciesid=species.id;if(window.BattlePokemonSprites)miscData=BattlePokemonSprites[speciesid];if(!miscData&&window.BattlePokemonSpritesBW)miscData=BattlePokemonSpritesBW[speciesid];if(!miscData)miscData={};if(miscData.num!==0&&miscData.num>-5000){var baseSpeciesid=toID(species.baseSpecies);spriteData.cryurl='audio/cries/'+baseSpeciesid;var formeid=species.formeid;if(species.isMega||formeid&&(formeid==='-crowned'||formeid==='-eternal'||formeid==='-four'||formeid==='-hangry'||formeid==='-hero'||formeid==='-lowkey'||formeid==='-noice'||formeid==='-primal'||formeid==='-rapidstrike'||formeid==='-roaming'||formeid==='-school'||formeid==='-sky'||formeid==='-starter'||formeid==='-super'||formeid==='-therian'||formeid==='-unbound'||baseSpeciesid==='calyrex'||baseSpeciesid==='kyurem'||baseSpeciesid==='cramorant'||baseSpeciesid==='indeedee'||baseSpeciesid==='lycanroc'||baseSpeciesid==='necrozma'||baseSpeciesid==='oinkologne'||baseSpeciesid==='oricorio'||baseSpeciesid==='slowpoke'||baseSpeciesid==='tatsugiri'||baseSpeciesid==='zygarde')){spriteData.cryurl+=formeid;}spriteData.cryurl+='.mp3';}if(options.shiny&&mechanicsGen>1)dir+='-shiny';if(Dex.afdMode||options.afd){dir='afd'+dir;spriteData.url+=dir+'/'+name+'.png';return spriteData;}if(options.mod){spriteData.cryurl="sprites/"+options.mod+"/audio/"+toID(species.baseSpecies);spriteData.cryurl+='.mp3';}var animatedSprite=false;if(!Dex.prefs('noanim')&&!Dex.prefs('nogif')&&spriteData.gen>=5){var animationArray=[];if(baseDir===''&&window.BattlePokemonSprites){animationArray.push([BattlePokemonSprites[speciesid],'']);}if(window.BattlePokemonSpritesBW){animationArray.push([BattlePokemonSpritesBW[speciesid],'gen5']);}for(var _i8=0;_i8<animationArray.length;_i8++){var _ref2=animationArray[_i8];var animationData=_ref2[0];var animDir=_ref2[1];if(!animationData)continue;if(animationData[facing+'f']&&options.gender==='F')facing+='f';if(!animationData[facing])continue;if(facing.endsWith('f'))name+='-f';if(spriteData.gen>=6)spriteData.pixelated=false;dir=animDir+'ani'+dir;spriteData.w=animationData[facing].w;spriteData.h=animationData[facing].h;spriteData.url+=dir+'/'+name+'.gif';animatedSprite=true;break;}}if(!animatedSprite){dir=(baseDir||'gen5')+dir;if(spriteData.gen>=4&&miscData['frontf']&&options.gender==='F'){name+='-f';}spriteData.url+=dir+'/'+name+'.png';}if(!options.noScale){if(graphicsGen>4){}else if(spriteData.isFrontSprite){spriteData.w*=2;spriteData.h*=2;spriteData.y+=-16;}else{spriteData.w*=2/1.5;spriteData.h*=2/1.5;spriteData.y+=-11;}if(spriteData.gen<=2)spriteData.y+=2;}return spriteData;};_proto2.getPokemonIconNum=function getPokemonIconNum(id,isFemale,facingLeft){var _window$BattlePokemon,_window$BattlePokedex,_window$BattlePokemon2;var num=0;if((_window$BattlePokemon=window.BattlePokemonSprites)!=null&&(_window$BattlePokemon=_window$BattlePokemon[id])!=null&&_window$BattlePokemon.num){num=BattlePokemonSprites[id].num;}else if((_window$BattlePokedex=window.BattlePokedex)!=null&&(_window$BattlePokedex=_window$BattlePokedex[id])!=null&&_window$BattlePokedex.num){num=BattlePokedex[id].num;}if(num<0)num=0;if(num>1025)num=0;if((_window$BattlePokemon2=window.BattlePokemonIconIndexes)!=null&&_window$BattlePokemon2[id]){num=BattlePokemonIconIndexes[id];}if(isFemale){if(['unfezant','frillish','jellicent','meowstic','pyroar'].includes(id)){num=BattlePokemonIconIndexes[id+'f'];}}if(facingLeft){if(BattlePokemonIconIndexesLeft[id]){num=BattlePokemonIconIndexesLeft[id];}}return num;};_proto2.getPokemonIcon=function getPokemonIcon(pokemon,facingLeft){var _pokemon,_pokemon2,_pokemon3,_pokemon4,_pokemon5;if(pokemon==='pokeball'){return"background:transparent url("+Dex.resourcePrefix+"sprites/pokemonicons-pokeball-sheet.png) no-repeat scroll -0px 4px";}else if(pokemon==='pokeball-statused'){return"background:transparent url("+Dex.resourcePrefix+"sprites/pokemonicons-pokeball-sheet.png) no-repeat scroll -40px 4px";}else if(pokemon==='pokeball-fainted'){return"background:transparent url("+Dex.resourcePrefix+"sprites/pokemonicons-pokeball-sheet.png) no-repeat scroll -80px 4px;opacity:.4;filter:contrast(0)";}else if(pokemon==='pokeball-none'){return"background:transparent url("+Dex.resourcePrefix+"sprites/pokemonicons-pokeball-sheet.png) no-repeat scroll -80px 4px";}var id=toID(pokemon);if(!pokemon||typeof pokemon==='string')pokemon=null;if((_pokemon=pokemon)!=null&&_pokemon.speciesForme)id=toID(pokemon.speciesForme);if((_pokemon2=pokemon)!=null&&_pokemon2.species)id=toID(pokemon.species);if((_pokemon3=pokemon)!=null&&(_pokemon3=_pokemon3.volatiles)!=null&&_pokemon3.formechange&&!pokemon.volatiles.transform){id=toID(pokemon.volatiles.formechange[1]);}var num=this.getPokemonIconNum(id,((_pokemon4=pokemon)==null?void 0:_pokemon4.gender)==='F',facingLeft);var top=Math.floor(num/12)*30;var left=num%12*40;var fainted=(_pokemon5=pokemon)!=null&&_pokemon5.fainted?";opacity:.6;filter:grayscale(60%) brightness(.5)":"";return"background:transparent url("+Dex.resourcePrefix+"sprites/pokemonicons-sheet.png?v19) no-repeat scroll -"+left+"px -"+top+"px"+fainted;};_proto2.getTeambuilderSpriteData=function getTeambuilderSpriteData(pokemon){var dex=arguments.length>1&&arguments[1]!==undefined?arguments[1]:Dex;var gen=dex.gen;var id=toID(pokemon.species||pokemon);var species=Dex.species.get(id);var spriteid;if(typeof pokemon==='string'){spriteid=species.spriteid||id;}else{spriteid=pokemon.spriteid;if(pokemon.species&&!spriteid){spriteid=species.spriteid||id;}}if(species.exists===false)return{spriteDir:'sprites/gen5',spriteid:'0',x:10,y:5};if(Dex.afdMode){return{spriteid:spriteid,spriteDir:'sprites/afd',shiny:!!pokemon.shiny,x:10,y:5};}var spriteData={spriteid:spriteid,spriteDir:'sprites/dex',x:-2,y:-3};if(pokemon.shiny)spriteData.shiny=true;if(dex.modid==='gen7letsgo')gen=8;if(Dex.prefs('nopastgens'))gen=9;if(Dex.prefs('bwgfx')&&gen>5)gen=5;var homeExists=(!species.isNonstandard||!['CAP','Custom'].includes(species.isNonstandard)||species.id==="xerneasneutral")&&!["floetteeternal","pichuspikyeared","pikachubelle","pikachucosplay","pikachulibre","pikachuphd","pikachupopstar","pikachurockstar"].includes(species.id);if(gen>=8&&homeExists){spriteData.spriteDir='sprites/home-centered';spriteData.x=8;spriteData.y=10;spriteData.h=96;return spriteData;}var xydexExists=!species.isNonstandard||species.isNonstandard==='Past'||species.isNonstandard==='CAP'||["pikachustarter","eeveestarter","meltan","melmetal","pokestarufo","pokestarufo2","pokestarbrycenman","pokestarmt","pokestarmt2","pokestargiant","pokestarhumanoid","pokestarmonster","pokestarf00","pokestarf002","pokestarspirit"].includes(species.id);if(species.gen>=8&&species.isNonstandard!=='CAP')xydexExists=false;if(gen>=6&&xydexExists){if(species.gen>=7){spriteData.x=-6;spriteData.y=-7;}else if(id.substr(0,6)==='arceus'){spriteData.x=-2;spriteData.y=7;}else if(id==='garchomp'){spriteData.x=-2;spriteData.y=2;}else if(id==='garchompmega'){spriteData.x=-2;spriteData.y=0;}return spriteData;}spriteData.spriteDir='sprites/gen5';if(gen<=1&&species.gen<=1)spriteData.spriteDir='sprites/gen1';else if(gen<=2&&species.gen<=2)spriteData.spriteDir='sprites/gen2';else if(gen<=3&&species.gen<=3)spriteData.spriteDir='sprites/gen3';else if(gen<=4&&species.gen<=4)spriteData.spriteDir='sprites/gen4';spriteData.x=10;spriteData.y=5;return spriteData;};_proto2.getTeambuilderSprite=function getTeambuilderSprite(pokemon,dex){var xOffset=arguments.length>2&&arguments[2]!==undefined?arguments[2]:0;var yOffset=arguments.length>3&&arguments[3]!==undefined?arguments[3]:0;if(!pokemon)return'';var data=this.getTeambuilderSpriteData(pokemon,dex);var shiny=data.shiny?'-shiny':'';var resize=data.h?"background-size:"+data.h+"px":'';return"background-image:url("+Dex.resourcePrefix+data.spriteDir+shiny+"/"+data.spriteid+".png);background-position:"+(data.x+xOffset)+"px "+(data.y+yOffset)+"px;background-repeat:no-repeat;"+resize;};_proto2.getItemIcon=function getItemIcon(item){var _itemData;var scale=arguments.length>1&&arguments[1]!==undefined?arguments[1]:1/2;var itemData=item;if(typeof item==='string'){itemData=Dex.items.get(item);}var filename=encodeURIComponent(((_itemData=itemData)==null?void 0:_itemData.name)||item);var size=96*scale;return"background:transparent url("+Dex.resourcePrefix+"sprites/itemicons/"+filename+".png) no-repeat center center;background-size:"+size+"px "+size+"px;";};_proto2.getItemIconpixel=function getItemIconpixel(item){var _itemData2;var scale=arguments.length>1&&arguments[1]!==undefined?arguments[1]:1/2;var itemData=item;if(typeof item==='string'){itemData=Dex.items.get(item);}var filename=encodeURIComponent(((_itemData2=itemData)==null?void 0:_itemData2.name)||item);var size=96*scale;return"background:transparent url("+Dex.resourcePrefix+"sprites/itemiconspixel/"+filename+".png) no-repeat center center;background-size:"+size+"px "+size+"px;";};_proto2.getTypeIcon=function getTypeIcon(type,b,type2){var t1=type?this.types.get(type).name:'???';var t2=type2&&this.types.get(type2)?this.types.get(type2).name:null;var icon=function(t){var sanitized=t.replace(/\?/g,'%3f');return"<img src=\""+Dex.resourcePrefix+"sprites/types/"+sanitized+".png\" alt=\""+t+"\" height=\"14\" width=\"32\" class=\"pixelated"+(b?' b':'')+"\" />";};return icon(t1)+(t2?icon(t2):'');};_proto2.getFlagIcon=function getFlagIcon(flag){if(!flag)return"\u2014";var flagID=toID(flag);var sanitizedFlag='';switch(flagID){case'contact':sanitizedFlag='Contact';break;case'binding':case'bind':sanitizedFlag='Bind';break;case'bite':sanitizedFlag='Bite';break;case'bomb':sanitizedFlag='Bomb';break;case'bullet':sanitizedFlag='Bullet';break;case'drain':sanitizedFlag='Drain';break;case'explosive':sanitizedFlag='Explosive';break;case'fist':case'punch':sanitizedFlag='Punch';break;case'powder':sanitizedFlag='Powder';break;case'pulse':sanitizedFlag='Pulse';break;case'slicing':case'slice':sanitizedFlag='Slice';break;case'sound':sanitizedFlag='Sound';break;case'wind':sanitizedFlag='Wind';break;case'airborne':sanitizedFlag='Airborne';break;case'aura':sanitizedFlag='Aura';break;case'beam':sanitizedFlag='Beam';break;case'breath':sanitizedFlag='Breath';break;case'claw':sanitizedFlag='Claw';break;case'crash':sanitizedFlag='Crash';break;case'crush':sanitizedFlag='Crush';break;case'dance':sanitizedFlag='Dance';break;case'heal':sanitizedFlag='Heal';break;case'kick':sanitizedFlag='Kick';break;case'launch':sanitizedFlag='Launch';break;case'light':sanitizedFlag='Light';break;case'lunar':sanitizedFlag='Lunar';break;case'magic':sanitizedFlag='Magic';break;case'pierce':sanitizedFlag='Pierce';break;case'shadow':sanitizedFlag='Shadow';break;case'solar':sanitizedFlag='Solar';break;case'spin':sanitizedFlag='Spin';break;case'sweep':sanitizedFlag='Sweep';break;case'throw':sanitizedFlag='Throw';break;case'weapon':sanitizedFlag='Weapon';break;case'wing':sanitizedFlag='Wing';break;case'bypassprotect':sanitizedFlag='Bypass Protect';break;case'nonreflectable':sanitizedFlag='Non-Reflectable';break;case'nonmirror':sanitizedFlag='Non-Mirror';break;case'nonsnatchable':sanitizedFlag='Non-Snatchable';break;case'bypasssubstitute':sanitizedFlag='Bypass Substitute';break;default:sanitizedFlag='undefined';break;}var flagText=sanitizedFlag.charAt(0).toUpperCase()+sanitizedFlag.slice(1);return"<img src=\""+Dex.resourcePrefix+"sprites/flagicons/"+sanitizedFlag+".png\" alt=\""+flagText+"\" height=\"32\" width=\"132\" class=\"pixelated\" onerror=\"this.style.display='none'; this.nextSibling.style.display='inline';\" /><span style=\"display: none;\">"+flagText+"</span>";};_proto2.isStatusName=function isStatusName(name){if(!name)return false;return this.statusNames.indexOf(toID(name))!==-1;};_proto2.getStatusIcon=function getStatusIcon(status){if(!status)return"\u2014";var statusID=toID(status);var sanitizedStatus='';switch(statusID){case'par':sanitizedStatus='Paralysis';break;case'psn':sanitizedStatus='Poison';break;case'tox':sanitizedStatus='Toxic';break;case'brn':sanitizedStatus='Burn';break;case'frz':sanitizedStatus='Frozen';break;case'slp':sanitizedStatus='Sleep';break;case'aura':sanitizedStatus='Aura';break;case'bubbleblight':sanitizedStatus='Bubbleblight';break;case'dragonblight':sanitizedStatus='Dragonblight';break;case'fear':sanitizedStatus='Fear';break;case'frostbite':sanitizedStatus='Frostbite';break;case'drowsy':sanitizedStatus='Drowsy';break;case'confusion':sanitizedStatus='Confused';break;case'flinch':sanitizedStatus='Flinch';break;default:sanitizedStatus='undefined';break;}var statusText=sanitizedStatus.charAt(0).toUpperCase()+sanitizedStatus.slice(1);return"<img src=\""+Dex.resourcePrefix+"sprites/status-is/"+sanitizedStatus+"_IS.png\" alt=\""+statusText+"\" height=\"32\" width=\"132\" class=\"pixelated\" onerror=\"this.style.display='none'; this.nextSibling.style.display='inline';\" /><span style=\"display: none;\">"+statusText+"</span>";};_proto2.
isFieldEffectName=function isFieldEffectName(name){
if(!name)return false;
return this.fieldEffectNames.indexOf(toID(name))!==-1;
};_proto2.
getFieldEffectIcon=function getFieldEffectIcon(effect){
if(!effect)return"\u2014";
var effectID=toID(effect);
var sanitizedEffect='';
switch(effectID){
...
var filename=encodeURIComponent(sanitizedEffect);
...
this.pokeballs=[];
(_window=window).BattleItems||(_window.BattleItems={});for(var _i10=0,_Object$values2=
Object.values(BattleItems);_i10<_Object$values2.length;_i10++){var data=_Object$values2[_i10];
...
all:function(){
var moves=[];
var searchIndex=window.BattleSearchIndex||[];for(var _i12=0;_i12<
searchIndex.length;_i12++){var entry=searchIndex[_i12];
if(entry[1]==='move')moves.push(_this2.moves.get(entry[0]));
}
return moves;
}
};this.
...
if(rows){for(var _i14=0;_i14<
rows.length;_i14++){var row=rows[_i14];

if(typeof row==='string'){allowedIds.push(toID(row));}else
if(Array.isArray(row)&&row[0]==='pokemon'&&row[1]){allowedIds.push(toID(row[1]));}
}
}
...
if(modTable!=null&&modTable.overrideTypeChart){for(var _i16=0,_Object$keys2=
Object.keys(modTable.overrideTypeChart);_i16<_Object$keys2.length;_i16++){var typeId=_Object$keys2[_i16];
...
get:function(name){var _window$BattleTeambui12,_window$BattleTeambui13;
var id=toID(name);
...
if(this.pokeballs)return this.pokeballs;
this.pokeballs=[];
(_window2=window).BattleItems||(_window2.BattleItems={});for(var _i18=0,_Object$values4=
Object.values(BattleItems);_i18<_Object$values4.length;_i18++){var data=_Object$values4[_i18];
...
for(var k=0;k<9;k++){i=buf.indexOf('|',i)+1;}
i=buf.indexOf(']',i)+1;
...
if(set.name&&set.name!==set.species){text+=set.name+" ("+set.species+")";}else
{text+=""+set.species;}
...
for(var i=((_set$moves3=set.moves)==null?void 0:_set$moves3.length)||0;i<4;i++){var _set$moves3;text+="- \n";}
}
...
for(var _i11=((_set$moves4=set.moves)==null?void 0:_set$moves4.length)||0;_i11<4;_i11++){var _set$moves4;text+="- \n";}
}
text+="\n";
return text;
};_proto["export"]=
...
sets.length;_i13++){var set=sets[_i13];text+=Teams.exportSet(set,dex,newFormat);}
return text;
};_proto.
...
}else if(line.startsWith('Trait: ')){set.ability=line.slice(7);}else
if(line.startsWith('Ability: ')){set.ability=line.slice(9);}else
if(line.startsWith('Item: ')){set.item=line.slice(6);}else
if(line.startsWith('Nickname: ')){set.name=line.slice(10);}else
if(line.startsWith('Species: ')){set.species=line.slice(9);}else
if(line==='Shiny: Yes'||line==='Shiny'){set.shiny=true;}else
if(line.startsWith('Level: ')){set.level=+line.slice(7);}else
if(line.startsWith('Happiness: ')){set.happiness=+line.slice(11);}else
if(line.startsWith('Pokeball: ')){set.pokeball=line.slice(10);}else
if(line.startsWith('Tera Type: ')){set.teraType=line.slice(11);}else
if(line.startsWith('Size: ')){set.size=line.slice(6);}else
...
for(var i in BattleNatures){if(BattleNatures[i].plus===plus&&BattleNatures[i].minus===minus){return i;}}
return null;
};_proto["import"]=
...
if(lines.length===1&&lines[0].includes('|')){return Teams.unpack(lines[0]);}for(var _i19=0;_i19<
lines.length;_i19++){var line=lines[_i19];
line=line.trim();
if(line===''||line==='---'){curSet=null;}else
if(line.startsWith('===')){}else
if(line.includes('|')){return Teams.unpack(line);}else
if(!curSet){
...
function Ability(id,name,data){this.effectType='Ability';this.id=void 0;this.name=void 0;this.gen=void 0;this.exists=void 0;this.num=void 0;this.shortDesc=void 0;this.desc=void 0;this.rating=void 0;this.flags=void 0;this.forcedGuardAction=void 0;this.blocksGuardAction=void 0;this.isNonstandard=void 0;
if(!data||typeof data!=='object')data={};
if(data.name)name=data.name;
this.name=Dex.sanitizeName(name);
this.id=id;
...
function Species(id,name,data){this.effectType='Species';this.id=void 0;this.name=void 0;this.gen=void 0;this.exists=void 0;this.baseSpecies=void 0;this.forme=void 0;this.formeid=void 0;this.spriteid=void 0;this.baseForme=void 0;this.num=void 0;this.types=void 0;this.abilities=void 0;this.guardAction=void 0;this.baseStats=void 0;this.bst=void 0;this.weightkg=void 0;this.heightm=void 0;this.gender=void 0;this.color=void 0;this.genderRatio=void 0;this.eggGroups=void 0;this.tags=void 0;this.infusibleSlots=void 0;this.otherFormes=void 0;this.cosmeticFormes=void 0;this.evos=void 0;this.prevo=void 0;this.evoType=void 0;this.evoLevel=void 0;this.evoMove=void 0;this.evoItem=void 0;this.evoCondition=void 0;this.nfe=void 0;this.requiredItems=void 0;this.tier=void 0;this.isMega=void 0;this.isPrimal=void 0;this.requiredTeraType=void 0;this.battleOnly=void 0;this.isNonstandard=void 0;this.unreleasedHidden=void 0;this.changesFrom=void 0;
if(!data||typeof data!=='object')data={};
if(data.name)name=data.name;
this.name=Dex.sanitizeName(name);
this.id=id;
...
if((window.getSelection==null||(_window$getSelection=window.getSelection())==null?void 0:_window$getSelection.type)==='Range'){ev.preventDefault();}else
...
el.className=className;
el.innerHTML=innerHTML;
if(this.innerElem.childNodes.length){this.innerElem.insertBefore(el,this.innerElem.childNodes[0]);}else
{this.innerElem.appendChild(el);}
this.updateScroll();
};_proto.
addSpacer=function addSpacer(){this.addDiv('spacer battle-history','<br />');};_proto.
...
this.innerElem.childNodes;_i4<_this$innerElem$child2.length;_i4++){var node=_this$innerElem$child2[_i4];if(node.className&&(' '+node.className+' ').includes(classContains)){elements.push(node);}}
if(this.preemptElem){for(var _i6=0,_this$preemptElem$chi2=this.preemptElem.childNodes;_i6<_this$preemptElem$chi2.length;_i6++){var _node=_this$preemptElem$chi2[_i6];if(_node.className&&(' '+_node.className+' ').includes(classContains)){elements.push(_node);}}}
if(htmlSrc&&elements.length&&!forceAdd){for(var _i8=0;_i8<
elements.length;_i8++){var element=elements[_i8];element.innerHTML=BattleLog.sanitizeHTML(htmlSrc);}
this.updateScroll();
return;
}for(var _i10=0;_i10<
elements.length;_i10++){var _element=elements[_i10];_element.parentElement.removeChild(_element);}
if(!htmlSrc)return;
if(forceAdd){this.addDiv('notice uhtml-'+id,BattleLog.sanitizeHTML(htmlSrc));}else
...
this.innerElem.childNodes;_i12<_this$innerElem$child4.length;_i12++){var node=_this$innerElem$child4[_i12];if(node.className&&(node.className+' ').startsWith(classStart)){nodes.push(node);}}
if(this.preemptElem){for(var _i14=0,_this$preemptElem$chi4=this.preemptElem.childNodes;_i14<_this$preemptElem$chi4.length;_i14++){var _node2=_this$preemptElem$chi4[_i14];if(_node2.className&&(_node2.className+' ').startsWith(classStart)){nodes.push(_node2);}}}
...
linkNode.childNodes;_i20<_linkNode$childNodes2.length;_i20++){var childNode=_linkNode$childNodes2[_i20];parent.insertBefore(childNode,linkNode);}
...
if(name.startsWith("[Gen "+Dex.gen+"] ")){name=name.slice(("[Gen "+Dex.gen+"] ").length);}else
...
hashColor=function hashColor(name){return"color:"+this.usernameColor(name)+";";};BattleLog.
usernameColor=function usernameColor(name){
if(this.colorCache[name])return this.colorCache[name];
var hash;
if(Config.customcolors[name]){hash=MD5(Config.customcolors[name]);}else
{hash=MD5(name);}
...
parseChatMessage=function parseChatMessage(message,name,timestamp,isHighlighted){var _BattleLog$prefs,_window$app6,_window$PS5;
...
if(message.charAt(1)==='/'){message=message.slice(1);}else
{
...
for(var i=0;i<attribs.length-1;i+=2){if(attribs[i]===key){return attribs[i+1];}}
return undefined;
}
...
if(tagName==='a'){if(getAttrib('target')==='replace'){targetReplace=true;}}else
if(tagName==='img'){
var src=getAttrib('src')||'';
if(src.startsWith('data:image/')){dataUri=src;}
if(src.startsWith('//')){if(location.protocol!=='http:'&&location.protocol!=='https:'){setAttrib('src','https:'+src);}}
...
if(Number(_width)<200){_width=window.innerWidth>=400?'400':'320';}
if(Number(_height)<200){_height=window.innerWidth>=400?'225':'200';}
...
}else if(iconType==='type'){tagName=Dex.getTypeIcon(iconValue).slice(1,-3);}else
if(iconType==='category'){tagName=Dex.getCategoryIcon(iconValue).slice(1,-3);}
}
}
...
if((_Intl=window.Intl)!=null&&_Intl.DateTimeFormat){formattedTime=new Intl.DateTimeFormat(undefined,{month:'long',day:'numeric',hour:'numeric',minute:'numeric'}).format(parsedTime);}else
...
this.ensureYoutube().then(function(){setTimeout(function(){return loadPlayer();},300);});
};BattleLog.
...
if(!((_window$YT=window.YT)!=null&&_window$YT.Player)){setTimeout(function(){return loopCheck();},300);}else
{resolve();}
};
loopCheck();
};
...
if(!((_window$Config2=window.Config)!=null&&_window$Config2.server.registered)){replayid='unregisteredserver-'+replayid;}else
{replayid=Config.server.id+'-'+replayid;}
}
}else if(room.fragment){replayid=room.fragment;}else
{replayid=battle.id;}
...
str=str.replace(/\bhttps?:\/\/[^\s<>"']+/g,function(url){return"<a href=\""+url+"\" target=\"_blank\" rel=\"noopener noreferrer\">"+url+"</a>";});
...
function getBattleTextDict(){var _bt$default;
var bt=globalThis.BattleText;
if(!bt)return null;
return(_bt$default=bt["default"])!=null?_bt$default:bt;
}
function getBattleTextDefault(){var _dict$default;
var dict=getBattleTextDict();
if(!dict)return null;
return(_dict$default=dict["default"])!=null?_dict$default:null;
}var
...
side=side.slice(0,2);
if(side===this.perspective||side===BattleTextParser.allyID(side)){return!isFar?BattleText["default"].team:BattleText["default"].opposingTeam;}
...
side=side.slice(0,2);
if(side===this.perspective||side===BattleTextParser.allyID(side)){return BattleText["default"].party;}
return BattleText["default"].opposingParty;
};BattleTextParser.
effectId=function effectId(effect){
if(!effect)return'';
if(effect.startsWith('item:')||effect.startsWith('move:')){effect=effect.slice(5);}else
if(effect.startsWith('ability:')){effect=effect.slice(8);}
return toID(effect);
};_proto.
effect=function effect(_effect){
if(!_effect)return'';
if(_effect.startsWith('item:')||_effect.startsWith('move:')){_effect=_effect.slice(5);}else
if(_effect.startsWith('ability:')){_effect=_effect.slice(8);}
return _effect.trim();
};_proto.
...
if(side==='p1'&&name){this.p1=BattleTextParser.escapeReplace(name);}else
if(side==='p2'&&name){this.p2=BattleTextParser.escapeReplace(name);}else
if(side==='p3'&&name){this.p3=BattleTextParser.escapeReplace(name);}else
if(side==='p4'&&name){this.p4=BattleTextParser.escapeReplace(name);}
return'';
}
case'gen':{
var num=args[1];
...
case'win':case'tie':{
var _name=args[1];
if(cmd==='tie'||!_name){return this.template('tieBattle').replace('[TRAINER]',this.p1).replace('[TRAINER]',this.p2);}
...
if(kwArgs.zeffect){_line2=this.template('zEffect').replace('[POKEMON]',this.pokemon(_pokemon8));}
...
if(templateId==='start'&&(_kwArgs$from=kwArgs.from)!=null&&_kwArgs$from.startsWith('item:')){templateId+='FromItem';}
...
var _templateId='end';
var _template17='';
if((_kwArgs$from2=kwArgs.from)!=null&&_kwArgs$from2.startsWith('item:')){_template17=this.template('endFromItem',_effect4);}
...
var _target='';
if(['magician','pickpocket'].includes(_id8)){var _ref2=[kwArgs.of,''];_target=_ref2[0];kwArgs.of=_ref2[1];}
...
case'-sidestart':{
var _side4=args[1],_effect6=args[2];
...
return _template42.replace('[TEAM]',this.team(_side4)).replace('[PARTY]',this.party(_side4));
}
case'-sideend':{
...
if(BattleTextParser.effectId(kwArgs.from)==='hadronengine'){return _line14+this.template('start','hadronengine').replace('[POKEMON]',this.pokemon(kwArgs.of));}
...
if(_id11==='celebrate'){return this.template('activate','celebrate').replace('[TRAINER]',this.trainer(_pokemon19.slice(0,2)));}
if(!_target2&&['hyperdrill','hyperspacefury','hyperspacehole','phantomforce','shadowforce','feint'].includes(_id11)){var _ref3=
...
if(_id11==='forewarn'&&_pokemon19===_target2){_templateId4='activateNoTarget';}
if((_id11==='protosynthesis'||_id11==='quarkdrive')&&kwArgs.fromitem){_templateId4='activateFromItem';}
if(_id11==='orichalcumpulse'&&kwArgs.source){_templateId4='start';}
...
if(_id11==='brickbreak'){_template51=_template51.replace('[TEAM]',this.team(_target2.slice(0,2)));}
if(kwArgs.ability){_line15+=this.ability(kwArgs.ability,_pokemon19);}
if(kwArgs.ability2){_line15+=this.ability(kwArgs.ability2,_target2);}
...
if(_template53){return _line16+_template53.replace('[POKEMON]',this.pokemon(_pokemon21));}
if(!kwArgs.from){
...
var _template63=this.template('block',_effect14);
...
}else if(blocker==='uproar'&&kwArgs.msg){_templateId9='blockSelf';}
var _template64=this.template(_templateId9,kwArgs.from);
if(_template64){return _line25+_template64.replace('[POKEMON]',this.pokemon(_pokemon31));}
if(_id14==='unboost'){
...
_templateId9='fail';
if(['brn','frz','par','psn','slp','substitute','shedtail'].includes(_id14)){_templateId9='alreadyStarted';}
if(kwArgs.heavy)_templateId9='failTooHeavy';
if(kwArgs.weak)_templateId9='fail';
if(kwArgs.forme)_templateId9='failWrongForme';
_template64=this.template(_templateId9,_id14);
...
_template65=this.template(_pokemon32?_templateId10:'immuneNoPokemon',kwArgs.from);
}
...
var _template68=this.template(_templateId11,_id15);
...
return _template68.replace('[POKEMON]',pokemonName).replace('[ITEM]',_item2).replace('[TRAINER]',this.trainer(_side6));
}
case'-terastallize':{
var _pokemon35=args[1],type=args[2];
var _id16='';
var _templateId12=cmd.slice(1);
var _template69=this.template(_templateId12,_id16);
...
case'-hitcount':{
var _num5=args[2];
if(_num5==='1'){return this.template('hitCountSingular');}
...
return _template71.replace('[POKEMON]',this.pokemon(_pokemon37)).replace('[TARGET]',this.pokemon(_target6));
}
case'-anim':{return'';}
default:{return null;}
}
