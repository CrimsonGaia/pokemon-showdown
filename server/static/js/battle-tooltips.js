...
function ModifiableValue(battle,pokemon,serverPokemon){this.value=0;this.maxValue=0;this.comment=void 0;this.battle=void 0;this.pokemon=void 0;this.serverPokemon=void 0;this.itemName=void 0;this.abilityName=void 0;this.abilityName2=void 0;this.weatherName=void 0;this.isAccuracy=false;
this.comment=[];
this.battle=battle;
...
var ability2=serverPokemon.ability2||(pokemon==null?void 0:pokemon.ability2)||serverPokemon.baseAbility2;
...
var ignoreKlutz=["Macho Brace","Power Anklet","Power Band","Power Belt","Power Bracer","Power Lens","Power Weight"];
...
var slot=abilityName===this.abilityName?1:abilityName===this.abilityName2?2:0;
...
if(slot===1){var _this$pokemon3;if(!((_this$pokemon3=this.pokemon)!=null&&_this$pokemon3.effectiveAbility(this.serverPokemon)))return false;}else
{var _this$pokemon4;if(!((_this$pokemon4=this.pokemon)!=null&&_this$pokemon4.effectiveAbility2(this.serverPokemon)))return false;}
return true;
};_proto.
...
round=function round(value){return value?Number(value.toFixed(2)):0;};_proto.
toString=function toString(){
var valueString;
if(this.isAccuracy){valueString=this.value?this.round(this.value)+"%":"can't miss";}else
...
if(isClick){$('#tooltipwrapper').addClass('tooltip-locking-click');}else
...
case'guardaction':{
var _move=this.battle.dex.moves.get(args[1]);
var _teamIndex=parseInt(args[2],10);
var _pokemon=this.battle.nearSide.active[_teamIndex+this.battle.pokemonControlled*Math.floor(this.battle.mySide.n/2)];
if(!_pokemon)return false;
var _serverPokemon=this.battle.myPokemon[_teamIndex];
...
if(side===this.battle.mySide&&this.battle.myPokemon){_serverPokemon2=this.battle.myPokemon[pokemonIndex];}
if(side===this.battle.mySide.ally&&this.battle.myAllyPokemon){_serverPokemon2=this.battle.myAllyPokemon[pokemonIndex];}
if(args[3]==='illusion'){
buf='';
...
}else{buf=this.showPokemonTooltip(_pokemon2,_serverPokemon2);}
break;
}
case'teambar':{var _s$fullTeam,_s$sidebarPokemon;
var _sideIndex=parseInt(args[1],10);
var _side=this.battle.sides[_sideIndex];
var _pokemonIndex=parseInt(args[2],10);
var s=_side;
var fullTeam=((_s$fullTeam=s.fullTeam)!=null&&_s$fullTeam.length?s.fullTeam:null)||((_s$sidebarPokemon=s.sidebarPokemon)!=null&&_s$sidebarPokemon.length?s.sidebarPokemon:null)||_side.pokemon||[];
var _pokemon3=fullTeam[_pokemonIndex];
if(!_pokemon3){buf='';break;}
buf=this.showPokemonTooltip(_pokemon3,null,false,undefined,true);
break;
}
case'activepokemon':{


var _sideIndex2=parseInt(args[1],10);
var _side2=this.battle.sides[+this.battle.viewpointSwitched^_sideIndex2];
var activeIndex=parseInt(args[2],10);
var _pokemonIndex2=activeIndex;
if(activeIndex>=1&&this.battle.sides.length>2){
_pokemonIndex2-=1;
_side2=this.battle.sides[_side2.n+2];
}
var _pokemon4=_side2.active[activeIndex];
var _serverPokemon3=null;
if(_side2===this.battle.mySide&&this.battle.myPokemon){_serverPokemon3=this.battle.myPokemon[_pokemonIndex2];}
if(_side2===this.battle.mySide.ally&&this.battle.myAllyPokemon){_serverPokemon3=this.battle.myAllyPokemon[_pokemonIndex2];}
if(!_pokemon4)return false;
buf=this.showPokemonTooltip(_pokemon4,_serverPokemon3,true);
break;
}
case'switchpokemon':{
...
var _serverPokemon4=this.battle.myPokemon[_activeIndex];
...
var _serverPokemon5=this.battle.myAllyPokemon?this.battle.myAllyPokemon[_activeIndex2]:null;
buf=this.showPokemonTooltip(_pokemon6,_serverPokemon5);
break;
}
case'field':{
...
var $elem;
if(hoveredElem){$elem=$(hoveredElem);}else
{
...
hideTooltip=function hideTooltip(){BattleTooltips.hideTooltip();};_proto2.
...
showMoveTooltip=function showMoveTooltip(move,isZOrMax,pokemon,serverPokemon,gmaxMove,cooldownText){
var text='';
var zEffect='';
var foeActive=pokemon.side.foe.active;
if(this.battle.gameType==='freeforall'){foeActive=[].concat(foeActive,pokemon.side.active).filter(function(active){return active!==pokemon;});}
...
if(this.battle.hasPseudoWeather('Electric Terrain')){calls='Thunderbolt';}else
if(this.battle.hasPseudoWeather('Grassy Terrain')){calls='Energy Ball';}else
if(this.battle.hasPseudoWeather('Misty Terrain')){calls='Moonblast';}else
if(this.battle.hasPseudoWeather('Psychic Terrain')){calls='Psychic';}else
{calls='Tri Attack';}
}else if(this.battle.gen>3){calls='Earthquake';}else
{calls='Swift';}
...
if(this.battle.hardcoreMode){text+="<p class=\"tooltip-section\">"+move.shortDesc+"</p>";}else
{
text+='<p class="tooltip-section">';
if(move.priority>1){text+="Nearly always moves first <em>(priority +"+move.priority+")</em>.</p><p>";}else
if(move.priority<=-1){text+="Nearly always moves last <em>(priority &minus;"+-move.priority+")</em>.</p><p>";}else
if(move.priority===1){text+="Usually moves first <em>(priority +"+move.priority+")</em>.</p><p>";}else
{if(move.id==='grassyglide'&&this.battle.hasPseudoWeather('Grassy Terrain')){text+='Usually moves first <em>(priority +1)</em>.</p><p>';}}
text+=''+(move.desc||move.shortDesc||'')+'</p>';
if(this.battle.gameType==='doubles'||this.battle.gameType==='multi'){
if(move.target==='allAdjacent'){text+='<p>&#x25ce; Hits both foes and ally.</p>';}else
if(move.target==='allAdjacentFoes'){text+='<p>&#x25ce; Hits both foes.</p>';}
}else if(this.battle.gameType==='triples'){
if(move.target==='allAdjacent'){text+='<p>&#x25ce; Hits adjacent foes and allies.</p>';}else
if(move.target==='allAdjacentFoes'){text+='<p>&#x25ce; Hits adjacent foes.</p>';}else
if(move.target==='any'){text+='<p>&#x25ce; Can target distant Pok&eacute;mon in Triples.</p>';}
}else if(this.battle.gameType==='freeforall'){
if(move.target==='allAdjacent'||move.target==='allAdjacentFoes'){text+='<p>&#x25ce; Hits all foes.</p>';}else
if(move.target==='adjacentAlly'){text+='<p>&#x25ce; Can target any foe in Free-For-All.</p>';}
}
...
showGuardActionCDTooltip=function showGuardActionCDTooltip(move,pokemon,serverPokemon,cur,max){
var remaining=Math.max(0,max-cur);
var cooldownText=cur>=max?"Guard Action Cooldown: Ready":"Guard Action Cooldown: "+
...
showEffectivenessTooltip=function showEffectivenessTooltip(payload){
var esc=function(s){return BattleLog.escapeHTML(s);};
var POS_COLOR='#3d7dca';
var NEG_COLOR='#c43b3b';
...
var cardBg=function(t1,t2){
var c1=TYPE_COLORS[t1]||'#888';
var c2=t2?TYPE_COLORS[t2]||'#888':c1;
return"background:linear-gradient(135deg, "+c1+"55, "+c2+"55);";
};
var KNOWN_FLAG_ICONS=new Set(['contact','binding','bind','bite','bomb','bullet','drain',
...
var typeIcon=function(type){return"<div class=\"typechart-icon-wrap\" style=\"width:32px;height:14px;display:inline-flex;align-items:center;\">"+Dex.getTypeIcon(type)+"</div>";};
var flagIcon=function(flag){
if(!KNOWN_FLAG_ICONS.has(flag.toLowerCase()))return null;
var resized=Dex.getFlagIcon(flag).replace(/width="\d+"/,'width="64"').replace(/height="\d+"/,'height="16"');
...
var badgeFor=function(mod){
var positive=mod>0;
var absMod=Math.abs(mod);
var icon;
if(absMod>=2)icon='quatro.png';else
if(absMod>=1.5)icon='tres.png';else
if(absMod>=1)icon='dos.png';else
icon='uno.png';
var iconUrl="https://play.pokemonshowdown.com/sprites/misc/"+icon;
var style=positive?
...
var multText=function(mult,mod){
var label='×'+(mult>=10?mult.toFixed(0):mult.toFixed(2)).replace(/\.?0+$/,'');
var color=mod>0?POS_COLOR:mod<0?NEG_COLOR:'inherit';
return"<strong style=\"color:"+color+";\">"+label+"</strong>";
};
var stepMult=function(mod){return mod===0?1:Math.pow(2,mod);};
var tierText=function(mult){
if(mult>1){
...
var _split=(payload||'').split('##'),headerStr=_split[0],entriesStr=_split[1];
var _split2=(headerStr||'').split('~'),moveName=_split2[0],moveType=_split2[1],moveType2=_split2[2],moveCategory=_split2[3],moveFlagsStr=_split2[4],targetName=_split2[5],targetSpecies=_split2[6],targetType1=_split2[7],targetType2=_split2[8],totalModStr=_split2[9];var totalMod=parseFloat(totalModStr)||0;
var moveFlags=(moveFlagsStr?moveFlagsStr.split(',').filter(Boolean):[]).
map(function(f){return flagIcon(f);}).filter(function(html){return html!==null;});
var cardStyle=function(t1,t2){var basis=arguments.length>2&&arguments[2]!==undefined?arguments[2]:'50%';return"position:relative;border:1px solid #999;border-radius:5px;padding:4px 6px;flex:1 1 "+basis+";min-width:0;"+cardBg(t1,t2);};var flagsWrap='display:flex;flex-wrap:wrap;gap:3px;max-width:132px;margin-top:2px;';
var header="<div style=\"display:flex;align-items:stretch;gap:6px;\">";
...
header+="</div></div>";
var entries=(entriesStr||'').split(';').filter(Boolean).map(function(entry){
var _entry$split=entry.split('~'),kind=_entry$split[0],label=_entry$split[1],defendingType=_entry$split[2],modStr=_entry$split[3],effectLabel=_entry$split[4],immuneStr=_entry$split[5];
return{kind:kind,label:label,defendingType:defendingType,mod:parseFloat(modStr)||0,effectLabel:effectLabel||'',immune:immuneStr==='1'};
}).sort(function(a,b){return(b.immune?-100:b.mod)-(a.immune?-100:a.mod);});
var rows='';for(var _i10=0;_i10<
entries.length;_i10++){var e=entries[_i10];
if(!e.mod&&!e.immune)continue;
var flagHtml=e.kind==='flag'?flagIcon(e.label):null;
var sourceIcon=e.kind==='type'?typeIcon(e.label):flagHtml;


var iconStack=sourceIcon?"<span style=\"position:relative;display:inline-block;\">"+
sourceIcon+(e.immune?'':badgeFor(e.mod))+"</span>":"<span style=\"font-size:10px;\">"+
esc(e.label)+"</span>";
var targetIcon=e.defendingType?typeIcon(e.defendingType):'';
var rowMult=e.immune?multText(0,-1):
...
var overallMult=stepMult(totalMod);
var overall="<div style=\"display:flex;align-items:center;gap:8px;margin-top:6px;padding-top:6px;border-top:1px solid #ccc;\">"+"<strong style=\"font-size:11px;\">Overall</strong>"+"<div style=\"width:1px;align-self:stretch;background:#ccc;\"></div>"+("<span style=\"font-size:11px;\">"+


tierText(overallMult)+"</span>")+("<span style=\"margin-left:auto;flex:0 0 auto;font-size:16px;display:flex;align-items:center;\">"+
multText(overallMult,totalMod)+"</span>")+"</div>";
...
showPokemonTooltip=function showPokemonTooltip(clientPokemon,serverPokemon,isActive,illusionIndex,restricted){var _this3=this;
...
if(clientPokemon.volatiles.transform){text+="<small>(Transformed into "+clientPokemon.volatiles.formechange[1]+")</small><br />";}else
...
if(pokemon.terastallized){text+="<small>(Terastallized)</small><br />";}else
if(clientPokemon!=null&&clientPokemon.volatiles.typechange||clientPokemon!=null&&clientPokemon.volatiles.typeadd){text+="<small>(Type changed)</small><br />";}
...
if(serverPokemon){
var auraTurns=pokemon.status==='aura'?Number(clientPokemon==null?void 0:clientPokemon.auraTime)||0:0;
...
var exacthp='';
if(serverPokemon){exacthp=" ("+serverPokemon.hp+"/"+serverPokemon.maxhp+")";}else
if(pokemon.maxhp===48){exacthp=" <small>("+pokemon.hp+"/"+pokemon.maxhp+" pixels)</small>";}
var _auraTurns=pokemon.status==='aura'?Number(clientPokemon==null?void 0:clientPokemon.auraTime)||0:0;
var _status=pokemon.status?" "+
this.renderStatusIcon(pokemon.status)+(_auraTurns?" <small>- "+_auraTurns+" Turn"+(_auraTurns===1?'':'s')+"</small>":''):
'';
text+="<p><small>HP:</small> "+Pokemon.getHPText(pokemon,this.battle.reportExactHP)+exacthp+_status;
if(clientPokemon){
if(pokemon.status==='tox'){
if(pokemon.ability==='Poison Heal'||pokemon.ability==='Magic Guard'){text+=" <small>Would take if ability removed: "+Math.floor(100/16*Math.min(clientPokemon.statusData.toxicTurns+1,15))+"%</small>";}else
...
var knownItem=restricted?clientPokemon.revealedItem:pokemon.item;
if(knownItem){_item=this.battle.dex.items.get(knownItem).name;resolvedItemId=knownItem;}
...
var itemIcon=resolvedItemId?iconOf(resolvedItemId):'';
...
text+="<p class=\"tooltip-section\">";
var battlePokemon=clientPokemon||this.battle.findCorrespondingPokemon(pokemon);
var currentGuardActionId=clientPokemon!=null&&clientPokemon.guardActionMoveId?toID(clientPokemon.guardActionMoveId):null;for(var _i12=0,_serverPokemon$moves2=
...
clientPokemon.moveTrack;_i16<_clientPokemon$moveTr2.length;_i16++){var _row=_clientPokemon$moveTr2[_i16];text+=this.getPPUseText(_row)+"<br />";}
...
if(this.pokemonHasClones(clientPokemon)){text+="(Your opponent has two indistinguishable Pok\xE9mon, making it impossible for you to tell which one has which moves/ability/item.) ";}
text+="</p>";
}
return text;
};_proto2.
...
side.pokemon;_i20<_side$pokemon4.length;_i20++){var ally=_side$pokemon4[_i20];if(pokemon!==ally&&pokemon.searchid===ally.searchid){return true;}}
return false;
};_proto2.
...
if(boostLevel>0){stats[statName]*=boostTable[boostLevel];}else
{
...
var ability=toID((_clientPokemon$effect=clientPokemon==null?void 0:clientPokemon.effectiveAbility(serverPokemon))!=null?_clientPokemon$effect:serverPokemon.ability||serverPokemon.baseAbility);

if(pokemon.status){
if(this.battle.gen>2&&ability==='guts'){stats.atk=Math.floor(stats.atk*1.5);}else
if(this.battle.gen<2&&pokemon.status==='brn'){stats.atk=Math.floor(stats.atk*0.5);}
...
Dex.statNamesExceptHP;_i24<_Dex$statNamesExceptH4.length;_i24++){var _statName=_Dex$statNamesExceptH4[_i24];if(stats[_statName]>999)stats[_statName]=999;}
return stats;
}
...
if(item==='choiceband'&&!(clientPokemon!=null&&clientPokemon.volatiles['dynamax'])){stats.atk=Math.floor(stats.atk*1.5);}
...
if(ability==='unburden'&&clientPokemon.volatiles['itemremoved']&&!item){speedModifiers.push(2);}for(var _i28=0,_Dex$statNamesExceptH6=
...
if(_statName2==='spe'){speedModifiers.push(1.5);}else
...
if(item==='choicespecs'&&!(clientPokemon!=null&&clientPokemon.volatiles['dynamax'])){stats.spa=Math.floor(stats.spa*1.5);}
...
if(item==='choicescarf'&&!(clientPokemon!=null&&clientPokemon.volatiles['dynamax'])){speedModifiers.push(1.5);}
...
speedModifiers.length;_i32++){var modifier=speedModifiers[_i32];chainedSpeedModifier*=modifier;}
...
if(this.battle.gen>6){stats.spe=Math.floor(stats.spe*0.5);}else
...
var _buf='';
_buf+=this.renderTypeMatchups(clientPokemon,null,short);
_buf+="<p><small>Spe</small> "+min+" to "+max+" <small>(before items/abilities/modifiers)</small></p>";
return _buf;
}
...
Dex.statNamesExceptHP;_i34<_Dex$statNamesExceptH8.length;_i34++){var _BattleText,_BattleText2,_ref2,_entry$statShortName;var statName=_Dex$statNamesExceptH8[_i34];
...
var fallbackShort={hp:'HP',atk:'Atk',def:'Def',spa:'SpA',spd:'SpD',spe:'Spe',spc:'Spc'};
var entry=((_BattleText=globalThis.BattleText)==null?void 0:_BattleText[statLabel])||((_BattleText2=globalThis.BattleText)==null||(_BattleText2=_BattleText2.stats)==null?void 0:_BattleText2[statLabel]);
var shortName=(_ref2=(_entry$statShortName=entry==null?void 0:entry.statShortName)!=null?_entry$statShortName:fallbackShort[statLabel])!=null?_ref2:statLabel.toUpperCase();
buf+=shortName+"&nbsp;</small>";
buf+=""+stats[statName];
...
Dex.statNamesExceptHP;_i36<_Dex$statNamesExceptH10.length;_i36++){var _BattleText3,_BattleText4,_ref3,_entry$statShortName2;var _statName3=_Dex$statNamesExceptH10[_i36];
...
var _fallbackShort={hp:'HP',atk:'Atk',def:'Def',spa:'SpA',spd:'SpD',spe:'Spe',spc:'Spc'};
var _entry=((_BattleText3=globalThis.BattleText)==null?void 0:_BattleText3[_statLabel])||((_BattleText4=globalThis.BattleText)==null||(_BattleText4=_BattleText4.stats)==null?void 0:_BattleText4[_statLabel]);
var _shortName=(_ref3=(_entry$statShortName2=_entry==null?void 0:_entry.statShortName)!=null?_entry$statShortName2:_fallbackShort[_statLabel])!=null?_ref3:_statLabel.toUpperCase();
buf+=_shortName+"&nbsp;</small>";
if(modifiedStats[_statName3]===stats[_statName3]){buf+=""+modifiedStats[_statName3];}else
if(modifiedStats[_statName3]<stats[_statName3]){buf+="<strong class=\"stat-lowered\">"+modifiedStats[_statName3]+"</strong>";}else
{buf+="<strong class=\"stat-boosted\">"+modifiedStats[_statName3]+"</strong>";}
}
buf+='</p>';
return buf;
};_proto2.
renderWeaponState=function renderWeaponState(clientPokemon,serverPokemon){
var pokemon=clientPokemon||serverPokemon;
var current=(pokemon==null?void 0:pokemon.weaponDurability)||0;
var max=(pokemon==null?void 0:pokemon.maxWeaponDurability)||0;
var recoveryLeft=(pokemon==null?void 0:pokemon.weaponRecoveryLeft)||0;
if(!pokemon||max<=0)return'';
var text="<p><small>Weapon:</small> "+current+"/"+max;
if(current<=0&&recoveryLeft>0){text+=" <small>(recovers in "+recoveryLeft+" turn"+(recoveryLeft===1?'':'s')+")</small>";}
return text+'</p>';
};_proto2.
renderGuardActionBadge=function renderGuardActionBadge(clientPokemon){
var cur=(clientPokemon==null?void 0:clientPokemon.guardActionCur)||0;
...
renderLightChargeBadge=function renderLightChargeBadge(clientPokemon){
if(!clientPokemon)return'';
var speciesId=toID(clientPokemon.speciesForme);
if(!['necrozma','necrozmadawnwings','necrozmaduskmane'].includes(speciesId))return'';
var max=clientPokemon.maxLightCharge||3;
var remaining=Math.max(0,max-(clientPokemon.lightCharge||0));
return"<p><small>Light Charge:</small> "+("<span class=\"tooltip-light-badge"+(remaining===0?' ready':'')+"\">")+"<i class=\"fa fa-sun-o\"></i>"+("<span class=\"tooltip-light-badge-count\">"+
remaining+"</span>")+"</span></p>";
};_proto2.
renderTypeMatchups=function renderTypeMatchups(clientPokemon,serverPokemon,compact){var _this4=this;

var pokemon=clientPokemon||serverPokemon;
if(!pokemon)return'';
var types=serverPokemon!=null&&serverPokemon.terastallized?[serverPokemon.teraType]:this.getPokemonTypes(pokemon);

var attackTypes=this.battle.dex.types.names().
map(function(n){return n;});

var allFlags=new Set();for(var _i38=0,_this$battle$dex$move2=
this.battle.dex.moves.all();_i38<_this$battle$dex$move2.length;_i38++){var move=_this$battle$dex$move2[_i38];
if(!move.flags)continue;
for(var flagId in move.flags){if(move.flags[flagId])allFlags.add(flagId);}
}
var weaknesses4x=[];
var weaknesses2x=[];
var resistsQuarter=[];
var resistsHalf=[];
var immunities=[];
var flagWeaknesses4x=[];
var flagWeaknesses2x=[];
var flagResistsQuarter=[];
var flagResistsHalf=[];
var flagImmunities=[];
var damageTakenOf=function(defType){var _types;return(_types=_this4.battle.dex.types)==null||_types.get==null||(_types=_types.get(defType))==null?void 0:_types.damageTaken;};for(var _i40=0;_i40<
attackTypes.length;_i40++){var atkType=attackTypes[_i40];
var mult=1;for(var _i42=0;_i42<
types.length;_i42++){var _dt$key;var defType=types[_i42];
var dt=damageTakenOf(defType);
if(!dt)continue;

var key=toID(atkType);
var val=(_dt$key=dt[key])!=null?_dt$key:dt[atkType];

if(val===1)mult*=2;else
if(val===2)mult*=0.5;else
if(val===3){mult=0;break;}
}
if(mult===0)immunities.push(atkType);else
if(mult===4)weaknesses4x.push(atkType);else
if(mult===2)weaknesses2x.push(atkType);else
if(mult===0.25)resistsQuarter.push(atkType);else
if(mult===0.5)resistsHalf.push(atkType);
}for(var _i44=0;_i44<

allFlags.length;_i44++){var flag=allFlags[_i44];
var _mult=1;for(var _i46=0;_i46<
types.length;_i46++){var _defType=types[_i46];
var _dt=damageTakenOf(_defType);
if(!_dt)continue;
var _val=_dt[flag];
if(_val===1)_mult*=2;else
if(_val===2)_mult*=0.5;else
if(_val===3){_mult=0;break;}
}
if(_mult===1)continue;
if(_mult===0)flagImmunities.push(flag);else
if(_mult===4)flagWeaknesses4x.push(flag);else
if(_mult===2)flagWeaknesses2x.push(flag);else
if(_mult===0.25)flagResistsQuarter.push(flag);else
if(_mult===0.5)flagResistsHalf.push(flag);
}
var renderIcons=function(list){var flagList=arguments.length>1&&arguments[1]!==undefined?arguments[1]:[];return list.length||flagList.length?"<span class=\"textaligned-typeicons\">"+
[].concat(list.map(function(t){return Dex.getTypeIcon(t);}),flagList.map(function(f){return Dex.getFlagIcon(f);})).join(' ')+"</span>":"<small>(none)</small>";};

var weakParts=[];
if(weaknesses4x.length||flagWeaknesses4x.length)weakParts.push("<small>4\xD7</small> "+renderIcons(weaknesses4x,flagWeaknesses4x));
if(weaknesses2x.length||flagWeaknesses2x.length)weakParts.push("<small>2\xD7</small> "+renderIcons(weaknesses2x,flagWeaknesses2x));
var weakLine=weakParts.length?weakParts.join('&nbsp; '):'<small>(none)</small>';
var resistParts=[];
if(resistsQuarter.length||flagResistsQuarter.length)resistParts.push("<small>\xBC\xD7</small> "+renderIcons(resistsQuarter,flagResistsQuarter));
if(resistsHalf.length||flagResistsHalf.length)resistParts.push("<small>\xBD\xD7</small> "+renderIcons(resistsHalf,flagResistsHalf));
var resistLine=resistParts.length?resistParts.join('&nbsp; '):'<small>(none)</small>';
var immuneLine=immunities.length||flagImmunities.length?renderIcons(immunities,flagImmunities):'<small>(none)</small>';
if(compact){


var parts=[];
if(weaknesses4x.length||flagWeaknesses4x.length)parts.push("<small>4\xD7</small> "+renderIcons(weaknesses4x,flagWeaknesses4x));
if(weaknesses2x.length||flagWeaknesses2x.length)parts.push("<small>2\xD7</small> "+renderIcons(weaknesses2x,flagWeaknesses2x));
if(resistsQuarter.length||flagResistsQuarter.length)parts.push("<small>\xBC\xD7</small> "+renderIcons(resistsQuarter,flagResistsQuarter));
if(resistsHalf.length||flagResistsHalf.length)parts.push("<small>\xBD\xD7</small> "+renderIcons(resistsHalf,flagResistsHalf));
if(immunities.length||flagImmunities.length)parts.push("<small>0\xD7</small> "+renderIcons(immunities,flagImmunities));
if(!parts.length)return'';
return"<p>"+parts.join('&nbsp; ')+"</p>";
}
return(
"<p><small>Weaknesses:</small> "+weakLine+"</p>"+("<p><small>Resistances:</small> "+
resistLine+"</p>")+("<p><small>Immunities:</small> "+
immuneLine+"</p>"));
...
var bullet=moveName.startsWith('*')?'<span style="color:#888">&#8226;</span>':'&#8226;';
...
pokemon.moveTrack;_i48<_pokemon$moveTrack2.length;_i48++){var _ref4=_pokemon$moveTrack2[_i48];var moveName=_ref4[0];var _ppUsed=_ref4[1];
...
if(this.battle.hasPseudoWeather('Electric Terrain')){moveType='Electric';}else
if(this.battle.hasPseudoWeather('Grassy Terrain')){moveType='Grass';}else
if(this.battle.hasPseudoWeather('Misty Terrain')){moveType='Fairy';}else
if(this.battle.hasPseudoWeather('Psychic Terrain')){moveType='Psychic';}else
if(this.battle.hasPseudoWeather('Toxic Terrain')){moveType='Poison';}
}
if(move.id==='terablast'&&pokemon.terastallized){moveType=pokemon.terastallized;}
...
var noTypeOverride=['judgment','multiattack','naturalgift','revelationdance','struggle','technoblast','terrainpulse','weatherball'];
...
for(var i=0;i<types.length;i++){if(serverPokemon.moves[i]&&move.id===toID(serverPokemon.moves[i])){moveType=types[i];}}
}
if(category!=='Status'){
if(moveType==='Normal'){
...
}for(var _i52=0,_ref6=[].concat(
pokemon.side.active,pokemon.side.foe.active);_i52<_ref6.length;_i52++){var _active=_ref6[_i52];
if(!_active||_active.fainted)continue;
if(_active.ability==='Illuminate'){
accuracyModifiers.push(4506);
...
accuracyModifiers.length;_i54++){var mod=accuracyModifiers[_i54];if(mod!==4096){chain=chain*mod+2048>>12;}}
...
if(pokemon.boosts.accuracy>0){value.set(Math.floor(value.value*(pokemon.boosts.accuracy+3)/3));}else
...
getMoveBasePower=function getMoveBasePower(move,moveType,value){var _move$flags;var target=arguments.length>3&&arguments[3]!==undefined?arguments[3]:null;
var pokemon=value.pokemon;
var serverPokemon=value.serverPokemon;
...
if(['hex','infernalparade'].includes(move.id)&&target!=null&&target.status){value.modify(2,move.name+' + status');}
...
Object.values(target.boosts);_i56<_Object$values2.length;_i56++){var boost=_Object$values2[_i56];if(boost>0)boostCount+=boost;}
...
Object.values(pokemon.boosts);_i58<_Object$values4.length;_i58++){var _boost=_Object$values4[_i58];if(_boost>0)_boostCount+=_boost;}
...
if(target.volatiles['dynamax']){value.set(0,'blocked by target\'s Dynamax');}else
...
if((_move$flags=move.flags)!=null&&_move$flags.light){value.abilityModify(1.5,"Illuminate");}
...
for(var i=1;i<=5&&i<=pokemon.side.faintCounter;i++){if(pokemon.volatiles["fallen"+i]){value.abilityModify(1+0.1*i,"Supreme Overlord");}}
if(target){
if(["MF","FM"].includes(pokemon.gender+target.gender)){value.abilityModify(0.75,"Rivalry");}else
if(["MM","FF"].includes(pokemon.gender+target.gender)){value.abilityModify(1.25,"Rivalry");}
}
var noTypeOverride=['judgment','multiattack','naturalgift','revelationdance','struggle','technoblast','terrainpulse','weatherball'];
var allowTypeOverride=!noTypeOverride.includes(move.id)&&(move.id!=='terablast'||!pokemon.terastallized);
...
if(!ally||ally.fainted)continue;
var allyAbility=this.getAllyAbility(ally);
if(moveType==='Fairy'&&allyAbility==='Fairy Aura'){auraBoosted='Fairy Aura';}else
if(moveType==='Dark'&&allyAbility==='Dark Aura'){auraBoosted='Dark Aura';}else
if(allyAbility==='Aura Break'){auraBroken=true;}else
if(allyAbility==='Battery'&&ally!==pokemon&&move.category==='Special'){value.modify(1.3,'Battery');}else
if(allyAbility==='Power Spot'&&ally!==pokemon){value.modify(1.3,'Power Spot');}else
if(allyAbility==='Steely Spirit'&&moveType==='Steel'){value.modify(1.5,'Steely Spirit');}
...
if(foe.ability==='Fairy Aura'&&moveType==='Fairy'){auraBoosted='Fairy Aura';}else
if(foe.ability==='Dark Aura'&&moveType==='Dark'){auraBoosted='Dark Aura';}else
if(foe.ability==='Aura Break'){auraBroken=true;}
}
if(auraBoosted){
if(auraBroken){value.modify(0.75,auraBoosted+' + Aura Break');}else
...
}else if(this.battle.hasPseudoWeather('Misty Terrain')&&moveType==='Dragon'){if(target?target.isGrounded():true){value.modify(0.5,'Misty Terrain + grounded target');}}else
if(this.battle.hasPseudoWeather('Grassy Terrain')&&['earthquake','bulldoze','magnitude'].includes(move.id)){if(target?target.isGrounded():true){value.modify(0.5,'Grassy Terrain + grounded target');}}
if(
...
if(move.id==='risingvoltage'&&this.battle.hasPseudoWeather('Electric Terrain')&&target!=null&&target.isGrounded()){value.modify(2,'Rising Voltage + Electric Terrain boost');}
...
if(!pokemon.getTypes){return this.battle.dex.species.get(pokemon.speciesForme).types;}
return pokemon.getTypeList(undefined,preterastallized);
};_proto2.
pokemonHasType=function pokemonHasType(pokemon,type,types){
if(!types)types=this.getPokemonTypes(pokemon);for(var _i64=0,_types3=
types;_i64<_types3.length;_i64++){var curType=_types3[_i64];if(curType===type)return true;}
return false;
};_proto2.
getAllyAbility=function getAllyAbility(ally){
var serverPokemon;
if(this.battle.myAllyPokemon){serverPokemon=this.battle.myAllyPokemon[ally.slot];}else
if(this.battle.myPokemon){serverPokemon=this.battle.myPokemon[ally.slot];}
return ally.effectiveAbility(serverPokemon);
};_proto2.
getPokemonAbilityData=function getPokemonAbilityData(clientPokemon,serverPokemon,restricted){
var abilityData=
...
var _cp=clientPokemon;
if(_cp.ability2)abilityData.ability2=_cp.ability2;
if(_cp.baseAbility2)abilityData.baseAbility2=_cp.baseAbility2;
}else{
var _speciesForme=clientPokemon.getSpeciesForme()||(serverPokemon==null?void 0:serverPokemon.speciesForme)||'';
var _species=this.battle.dex.species.get(_speciesForme);
if(_species.exists&&_species.abilities){
abilityData.possibilities=Object.values(_species.abilities);
if(this.battle.rules['Frantic Fusions Mod']){
var fusionSpecies=this.battle.dex.species.get(clientPokemon.name);
if(fusionSpecies.exists&&fusionSpecies.name!==_species.name){for(var _i66=0,_Object$values6=
Object.values(fusionSpecies.abilities);_i66<_Object$values6.length;_i66++){var newAbility=_Object$values6[_i66];
...
var sp=serverPokemon;
if(serverPokemon.ability||serverPokemon.baseAbility){abilityData.ability=serverPokemon.ability||serverPokemon.baseAbility;}else
if(!abilityData.ability){abilityData.ability=serverPokemon.baseAbility;}
...
{var _this5=this;
var text='';
var abilityData=this.getPokemonAbilityData(clientPokemon,serverPokemon,restricted);
var tier=this.battle.tier;
var abilityData0=abilityData;
...
var isOwnPokemon=!restricted&&!!clientPokemon&&(clientPokemon.side===this.battle.mySide||clientPokemon.side===this.battle.mySide.ally);
var sets=[];
for(var i=0;i<_abilityData.possibilities.length;i+=2){
var a1=_abilityData.possibilities[i];
var a2=_abilityData.possibilities[i+1];
var set=[];
if(a1)set.push(a1);
if(a2)set.push(a2);
...
var s=possible[0];
var _out="<span class=\"abilityset-title set-known\">Ability Set:</span><br />";for(var _i68=0;_i68<
s.length;_i68++){var id=s[_i68];_out+="<span class=\"ability-line\">"+boldIfRevealed(id)+"</span><br />";}
...
_out2+="<span class=\"abilityset-title "+setClass+"\">Set "+setNum+"</span><br />";for(var _i70=0,_possible$_s2=
possible[_s];_i70<_possible$_s2.length;_i70++){var _id=_possible$_s2[_i70];_out2+="<span class=\"ability-line\">"+boldIfRevealed(_id)+"</span><br />";}
}
return _out2;
}
}
...
for(var stat in evs){evs[stat]=comboEVs[stat]||0;}
...
hasMove[move.id]=1;
if(move.category==='Status'){
if(['batonpass','healingwish','lunardance'].includes(move.id)){moveCount['Support']++;}else
if(['metronome','assist','copycat','mefirst','photongeyser','shellsidearm'].includes(move.id)){
moveCount['Physical']+=0.5;
moveCount['Special']+=0.5;
}else
if(move.id==='naturepower'){moveCount['Special']++;}else
if(['protect','detect','spikyshield','kingsshield'].includes(move.id)){moveCount['Stall']++;}else
if(move.id==='wish'){
...
}else if(['dragondance','swordsdance','coil','bulkup','curse','bellydrum'].includes(move.id)){moveCount['PhysicalSetup']++;}else
if(['nastyplot','tailglow','quiverdance','calmmind','geomancy'].includes(move.id)){moveCount['SpecialSetup']++;}
...
}else if(['counter','endeavor','metalburst','mirrorcoat','rapidspin'].includes(move.id)){moveCount['Support']++;}else
if(['nightshade','seismictoss','psywave','superfang','naturesmadness','foulplay','endeavor','finalgambit','bodypress'].includes(move.id)){moveCount['Offense']++;}else
...
physicalBulk*=1.3;
if(hasMove['scald']){moveCount['SpecialStall']++;}else
{moveCount['PhysicalStall']++;}
}
...
physicalBulk*=1.5;
specialBulk*=1.5;
}else if(hasMove['painsplit']&&hasMove['substitute']){moveCount['Stall']--;}else
if(hasMove['painsplit']||hasMove['rest']){
physicalBulk*=1.4;
specialBulk*=1.4;
}
...
{isFast=true;}else
if(abilityid==='unburden'||abilityid==='speedboost'||abilityid==='motordrive'){
isFast=true;
...
var ultrafast=hasMove['agility']||hasMove['shellsmash']||hasMove['autotomize']||hasMove['shiftgear']||hasMove['rockpolish'];
...
if(statChart[role][1]==='atk'||statChart[role][1]==='spa'){plusStat=statChart[role][1];}else
if(moveCount['Physical']>=3){plusStat='atk';}else
if(stats.spd>stats.def){plusStat='spd';}else
{plusStat='def';}
}
if(this.supportsAVs){
...
if(SRweaknesses.includes(species.types[0])){SRweak++;}else
if(SRresistances.includes(species.types[0])){SRweak--;}
if(SRweaknesses.includes(species.types[1])){SRweak++;}else
if(SRresistances.includes(species.types[1])){SRweak--;}
}
var hpDivisibility=0;
var hpShouldBeDivisible=false;
var hp=evs['hp']||0;
stat=this.getStat('hp',set,hp,1);
if((set.item==='Leftovers'||set.item==='Black Sludge')&&hasMove['substitute']&&stat!==404){hpDivisibility=4;}else
if(set.item==='Leftovers'||set.item==='Black Sludge'){hpDivisibility=0;}else
if(hasMove['bellydrum']&&(set.item||'').endsWith('Berry')){
hpDivisibility=2;
hpShouldBeDivisible=true;
}else
if(hasMove['substitute']&&(set.item||'').endsWith('Berry')){
hpDivisibility=4;
hpShouldBeDivisible=true;
}else
if(SRweak>=2||hasMove['bellydrum']){hpDivisibility=2;}else
if(SRweak>=1||hasMove['substitute']||hasMove['transform']){hpDivisibility=4;}else
if(set.ability!=='Magic Guard'){hpDivisibility=8;}
if(hpDivisibility){
...
if(species.id==='tentacruel'){evTotal=this.ensureMinEVs(evs,'spe',16,evTotal);}else
if(species.id==='skarmory'){evTotal=this.ensureMinEVs(evs,'spe',24,evTotal);}else
if(species.id==='jirachi'){evTotal=this.ensureMinEVs(evs,'spe',32,evTotal);}else
if(species.id==='celebi'){evTotal=this.ensureMinEVs(evs,'spe',36,evTotal);}else
if(species.id==='volcarona'){evTotal=this.ensureMinEVs(evs,'spe',52,evTotal);}else
if(species.id==='gliscor'){evTotal=this.ensureMinEVs(evs,'spe',72,evTotal);}else
if(species.id==='dragonite'&&evs['hp']){evTotal=this.ensureMaxEVs(evs,'spe',220,evTotal);}
if(evTotal<508){
var remaining=508-evTotal;
if(remaining>252)remaining=252;
secondaryStat=null;
if(!evs['atk']&&moveCount['PhysicalAttack']>=1){secondaryStat='atk';}else
if(!evs['spa']&&moveCount['SpecialAttack']>=1){secondaryStat='spa';}else
if(stats.hp===1&&!evs['def']){secondaryStat='def';}else
if(stats.def===stats.spd&&!evs['spd']){secondaryStat='spd';}else
if(!evs['spd']){secondaryStat='spd';}else
if(!evs['def']){secondaryStat='def';}
if(secondaryStat){
ev=remaining;
...
if(hasMove['gyroball']||hasMove['trickroom']){minusStat='spe';}else
if(!moveCount['PhysicalAttack']){minusStat='atk';}else
if(moveCount['SpecialAttack']<1&&!evs['spa']){
if(moveCount['SpecialAttack']<moveCount['PhysicalAttack']){minusStat='spa';}else
if(!evs['atk']){minusStat='atk';}
}else if(moveCount['PhysicalAttack']<1&&!evs['atk']){minusStat='atk';}else
if(stats.def>stats.spe&&stats.spd>stats.spe&&!evs['spe']){minusStat='spe';}else
if(stats.def>stats.spd){minusStat='spd';}else
{minusStat='def';}
...
if(natureOverride){val*=natureOverride;}else
if(((_BattleNatures=BattleNatures[set.nature])==null?void 0:_BattleNatures.plus)===stat){val*=1.1;}else
if(((_BattleNatures2=BattleNatures[set.nature])==null?void 0:_BattleNatures2.minus)===stat){val*=0.9;}
if(!this.supportsEVs){
...
var val=~~(~~(2*baseStat+iv+~~(ev/4))*level/100+5);
if(nature.plus===stat){val*=1.1;}else
if(nature.minus===stat){val*=0.9;}
return~~val;
};
...
var origLeftoverEVs=508;for(var _i72=0,_Dex$statNames2=
Dex.statNames;_i72<_Dex$statNames2.length;_i72++){var _origSpread$evs;var stat=_Dex$statNames2[_i72];origLeftoverEVs-=((_origSpread$evs=origSpread.evs)==null?void 0:_origSpread$evs[stat])||0;}
...
if(!minusTooLow){for(var _i74=0,_Dex$statNamesExceptH12=
Dex.statNamesExceptHP;_i74<_Dex$statNamesExceptH12.length;_i74++){var _stat=_Dex$statNamesExceptH12[_i74];
...
if(origNature.minus){savedEVs+=(origSpread.evs[origNature.minus]||0)-getMinEVs(origNature.minus,{minus:_stat});}
bestMinus=_stat;
bestMinusMinEVs=minEVs;
}
}
}
if(!plusTooHigh){for(var _i76=0,_Dex$statNamesExceptH14=
Dex.statNamesExceptHP;_i76<_Dex$statNamesExceptH14.length;_i76++){var _stat2=_Dex$statNamesExceptH14[_i76];
...
if(bestPlus&&bestPlus!==bestMinus){plusEVsSaved+=bestPlusMinEVs-getMinEVs(bestPlus,{plus:_stat2,minus:bestMinus});}
...
if(origNature.minus&&origNature.minus!==bestPlus&&origNature.minus!==bestMinus){newSpread.evs[origNature.minus]=getMinEVs(origNature.minus,newSpread);}for(var _i78=0,_Dex$statNames4=
Dex.statNames;_i78<_Dex$statNames4.length;_i78++){var _stat3=_Dex$statNames4[_i78];if(!newSpread.evs[_stat3])delete newSpread.evs[_stat3];}
...
Dex.statNames;_i80<_Dex$statNames6.length;_i80++){var _stat4=_Dex$statNames6[_i80];if(!newSpread.evs[_stat4])delete newSpread.evs[_stat4];}
return Object.assign({},newSpread,{savedEVs:savedEVs});
}
}
}
